...
static void import_file(instance i, xdbcache xc, xht std_namespace_prefixes, const char* filename, const char* domain, const char* user) {
	...
	namespace = xmlnode_get_namespace(data_element);
	if (j_strcmp(namespace, NS_LAST) == 0
		|| j_strcmp(namespace, NS_AUTH) == 0
		|| j_strcmp(namespace, NS_JABBERD_STOREDPRESENCE) == 0
		|| j_strcmp(namespace, NS_REGISTER) == 0
		|| j_strcmp(namespace, NS_ROSTER) == 0
		|| j_strcmp(namespace, NS_BROWSE) == 0
		|| j_strcmp(namespace, NS_VCARD) == 0) {
	    /* this is data we can just set to the configured storage */
	    xdb_set(xc, userid, namespace, data_element);
	    continue;
	}
	...
	if (j_strcmp(namespace, NS_JABBERD_STOREDREQUEST) == 0) {
	...
	/* offline messages */
	if (j_strcmp(namespace, NS_OFFLINE) == 0) {
	...
	/* message history */
	if (j_strcmp(namespace, NS_JABBERD_HISTORY) == 0) {
	    xmlnode message = NULL;
	...
	/* private data */
	if (j_strcmp(namespace, NS_PRIVATE) == 0) {
	    xmlnode item = NULL;
	...
	/* privacy lists */
	if (j_strcmp(namespace, NS_PRIVACY) == 0) {
	    xmlnode list = NULL;
	...
	if (j_strcmp(namespace, NS_XDBNSLIST) == 0)
	    continue;
...
static result base_importspool_config(instance id, xmlnode x, void *arg) {
    ...
    conf_data = pmalloc(id->p, sizeof(_base_importspool_st));
    conf_data->id = id;
    ...
    xhash_put(conf_data->std_namespace_prefixes, "", NS_SERVER);
    xhash_put(conf_data->std_namespace_prefixes, "private", NS_PRIVATE);
    xhash_put(conf_data->std_namespace_prefixes, "jabberd", NS_JABBERD_WRAPPER);
    xhash_put(conf_data->std_namespace_prefixes, "privacy", NS_PRIVACY);
