...
static mreturn mod_dynamic_server_command(mapi m) {
    ...
    xmlnode_list_item form = xmlnode_get_tags(m->packet->iq, "data:x", m->si->std_namespace_prefixes);
    ...
    char const* type = xmlnode_get_attrib_ns(form->node, "type", NULL);

    // is user submitting the form?
    if (j_strcmp(type, "submit") == 0) {
	// get the hostname to add/remove
	xmlnode_list_item hostname_node = xmlnode_get_tags(form->node, "data:field[@var='host']/data:value/text()", m->si->std_namespace_prefixes);
	if (hostname_node) {
	    char const* hostname = xmlnode_get_data(hostname_node->node);
