...
static int mod_privacy_list_in_use_by_other(session s, const char* list) {
	...
	if (j_strcmp(xhash_get(cur->aux_data, "mod_privacy_active"), list) == 0)
	    found++;
    }
...
static void mod_privacy_free_current_list_definitions(session s) {
    ...
    /* free the lists */
    list = xhash_get(s->aux_data, "mod_privacy_list_message");
    if (list != NULL) {
	pool_free(list->p);
    }
    ...
    list = xhash_get(s->aux_data, "mod_privacy_list_iq");
    if (list != NULL) {
	pool_free(list->p);
    }
...
static void mod_privacy_free_current_offline_list_definitions(udata user) {
    ...
    /* free the lists */
    list = xhash_get(user->aux_data, "mod_privacy_list_message");
    if (list != NULL) {
	pool_free(list->p);
    }
    list = xhash_get(user->aux_data, "mod_privacy_list_presence-in");
    if (list != NULL) {
	pool_free(list->p);
    }
    list = xhash_get(user->aux_data, "mod_privacy_list_presence-out");
    if (list != NULL) {
	pool_free(list->p);
    }
    list = xhash_get(user->aux_data, "mod_privacy_list_iq");
    if (list != NULL) {
	pool_free(list->p);
    }
...
static void mod_privacy_insert_list_item(struct mod_privacy_compiled_list_item** list, long order, const char *jid_str, const char *subscription, int do_deny) {
    ...
    /* create the new item */
    new_item = pmalloco(p, sizeof(struct mod_privacy_compiled_list_item));
    new_item->p = p;
...
static int mod_privacy_activate_list(session s, xmlnode list) {
    ...
    if (j_strcmp(list_name, xhash_get(s->aux_data, "mod_privacy_active")) != 0)
...
static void mod_privacy_load_offline_list(udata user) {
	...
	xhash_put(user->aux_data, "mod_privacy_lists_loaded", "loaded");
    }
...
static mreturn mod_privacy_out_iq_get(mapi m) {
	...
	active_list = xhash_get(m->s->aux_data, "mod_privacy_active");
	if (active_list != NULL) {
...
static mreturn mod_privacy_filter(mapi m, void* arg) {
	...
	case JPACKET_IQ:
	    do_bounce = 1;
	    affected_list = xhash_get(m->s ? m->s->aux_data : m->user->aux_data, "mod_privacy_list_iq");
	    break;
	case JPACKET_MESSAGE:
	    do_bounce = 1;
	    affected_list = xhash_get(m->s ? m->s->aux_data : m->user->aux_data, "mod_privacy_list_message");
	    break;
	case JPACKET_PRESENCE:
	    ...
	    affected_list = xhash_get(m->s ? m->s->aux_data : m->user->aux_data, arg ? "mod_privacy_list_presence-out" : "mod_privacy_list_presence-in");
	    break;
    }
