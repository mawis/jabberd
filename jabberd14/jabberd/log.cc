 ...
 *
 * Generated log messages are routed to a logging component using the XML
 * router. Generated debug log messages are not routed but either displayed to
 * the standard error output or sent to the syslog (Depending on the jabberd
 * configuration).
 */
...
int debug_flag = 0; /**< the active debugging mask (this is a bitmask of ORed
                       LOGT_* constents) */
int cmdline_debug_flag =
    0; /**< the debug mask given at the command line - this is ORed with the
          mask given in the configuration file */
 ...
 *
 * @param zone the zone (file) the function is called from. __ZONE__ should be
 * used here.
 * @param msgfmt the format string for the log message, parameters like for
 * printf() are given afterwards
 */
void debug_log(char const *zone, char const *msgfmt, ...) {
    ...
    if (_debug_facility == -1) {
        snprintf(message, sizeof(message), "%s %s ", debug_log_timestamp(),
                 zone);
 ...
 *
 * @param type the type of log message (one of "notice"+/"record", "warn"+,
 * "alert", "stat"+, "info", "emerg", "crit", or "err"+ - in jabberd only the
 * marked ones are used - "debug" should not be used as debugging messages
 * should not be routed on the XML router)
 * @param host the sending host (domain) of the message, or NULL if no sending
 * host is known (the message are than logged as jabberd internal)
...
void logger(char const *type, char const *host, char const *message) {
    ...
    log_debug2(ZONE, LOGT_DELIVER, "%s",
               xmlnode_serialize_string(log, xmppd::ns_decl_list(), 0));
 ...
 *
 * @param host the sending host (domain) of the log message - NULL if the host
 * is not known, in that case the message is logged as jabberd internal
 * @param msgfmt the format string for the message, parameters are passed
 * afterwards like for the printf() function
 */
 ...
 */
inline int get_debug_flag() { return debug_flag; }

/**
 * set the debugging mask, if 0 no debugging is requested
 * for other values debugging is enabled, the different bits in the value enable
 * different log message types the value set with set_cmdline_debug_flag() (what
 * the user specified on the command line) is always ORed to this value
 *
 * @param v the new debugging mask
 */
void set_debug_flag(int v) { debug_flag = v | cmdline_debug_flag; }

/**
 * set the value of the cmdline_debug_flag which is always ORed to the value
 * that is set by set_debug_flag() this will reset the mask set with
 * set_debug_flag(), so set_cmdline_debug_flag() should be called first.
 *
 * @param v the debug mask given on the command line
 */
void set_cmdline_debug_flag(int v) { debug_flag = cmdline_debug_flag = v; }

/**
 * set the facility used to write debugging messages to syslog
 *
 * @param facility the facility to use, -1 if writing to the standard output is
 * requested
 */
...
#else
    if (facility != -1) {
        log_warn(NULL, PACKAGE " configured to debug to syslog, but compiled "
...
logmessage::logmessage(logging &log_entity, loglevel level)
    : log_entity(log_entity), level(level) {}

logmessage::logmessage(const logmessage &orig)
    : log_entity(orig.log_entity), level(orig.level) {}
...
void logging::write(loglevel level_to_use, Glib::ustring log_message) {
    logger(level_to_use == xmppd::alert
               ? "alert"
               : level_to_use == xmppd::error
                     ? "error"
                     : level_to_use == xmppd::warn
                           ? "warn"
                           : level_to_use == xmppd::notice ? "notice"
                                                           : "unknown",
           identity.c_str(), log_message.c_str());
}
