...
/**
 * @file mio_tls.cc
 * @brief MIO read/write functions to read/write on TLS encrypted sockets and
 * handling for TLS in general (using the GNU TLS implementation)
 */
...
static void mio_tls_close(mio m, bool close_read) {
    ...
    gnutls_bye(static_cast<gnutls_session_t>(m->ssl),
               close_read ? GNUTLS_SHUT_RDWR : GNUTLS_SHUT_WR);
    if (close_read) {
        close(m->fd);
    } else {
...
/**
 * process a configuration element configuring a set of credentials in the
 * configuration file
 *
 * @param x the xmlnode containing the set
 * @param default_cacertfile file containing ca certificates to use if the set
 * does not contain its own definition
 * @param mio_tls_dh_params DH params to use (for performance reasons the same
 * params are used for all sets
 * @param crl_files_pem the CRL files in PEM format to load
 * @param crl_files_der the CRL files in DER format to load
 */
static void mio_tls_process_credentials(
    xmlnode x, const std::list<std::string> &default_cacertfile_pem,
    ...
    if (ret < 0) {
        log_error(NULL, "Error initializing credentials: %s",
                  gnutls_strerror(ret));
        return;
    }

    // iterate the child elements
    for (xmlnode cur = xmlnode_get_firstchild(x); cur != NULL;
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "domain") == 0) {
            ...
            if (domain == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <domain/> element inside the "
                    "TLS configuration, that does not contain a domain name.");
                continue;
            }

            // add it to the set for domains for which we have to add the
            // credentials after we loaded them
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "pem") == 0) {
            ...
            char const *privfile =
                xmlnode_get_attrib_ns(cur, "private-key", NULL);

            // there needs to be a filename for the public key
            if (pubfile == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <pem/> element inside the TLS "
                    "configuration, that does not contain a file-name.");
                continue;
            }

            // if there is no filename for the private key, use the same as for
            // the public key (file containing both keys)
            if (privfile == NULL)
                privfile = pubfile;

            // load the X.509 certificate
            ret = gnutls_certificate_set_x509_key_file(
                current_credentials, pubfile, privfile, GNUTLS_X509_FMT_PEM);
            if (ret < 0) {
                log_error(
                    NULL,
                    "Error loading X.509 certificate (PEM) pub=%s/priv=%s: %s",
                    pubfile, privfile, gnutls_strerror(ret));
                continue;
            }
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "der") == 0) {
            ...
            if (pubfile == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <der/> element inside the TLS "
                    "configuration, that does not contain a file-name.");
                continue;
            }
            ...
            // load the X.509 certificate
            ret = gnutls_certificate_set_x509_key_file(
                current_credentials, pubfile, privfile, GNUTLS_X509_FMT_DER);
            if (ret < 0) {
                log_error(
                    NULL,
                    "Error loading X.509 certificate (DER) pub=%s/priv=%s: %s",
                    pubfile, privfile, gnutls_strerror(ret));
                continue;
            }
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "ca") == 0) {
            bool format_der =
                j_strcmp(xmlnode_get_attrib_ns(cur, "type", NULL), "der") == 0;
            ...
            if (file == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <ca/> element inside the TLS "
                    "configuration, that does not contain a file-name.");
                continue;
            }
            ...
            ret = gnutls_certificate_set_x509_trust_file(
                current_credentials, file,
                format_der ? GNUTLS_X509_FMT_DER : GNUTLS_X509_FMT_PEM);
            if (ret < 0) {
                log_error(NULL, "Error loading certificate of CA (%s) %s: %s",
                          format_der ? "DER" : "PEM", file,
                          gnutls_strerror(ret));
                continue;
            }
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "openpgp") == 0) {
            ...
            if (pubfile == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <openpgp/> element inside the "
                    "TLS configuration, that does not contain a file-name.");
                continue;
            }
            if (privfile == NULL) {
                log_warn(NULL, "Initializing TLS subsystem: <openpgp/> element "
                               "inside the TLS configuration, that does not "
                               "contain a private-key file-name.");
                continue;
            }

            // load OpenPGP key/certificate
            ret = gnutls_certificate_set_openpgp_key_file(
                current_credentials, pubfile, privfile,
                GNUTLS_OPENPGP_FMT_BASE64);
            if (ret < 0) {
                log_error(NULL, "Error loading OpenPGP key pub=%s/priv=%s: %s",
                          pubfile, privfile, gnutls_strerror(ret));
                continue;
            }
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "keyring") == 0) {
            ...
            if (file == NULL) {
                log_warn(
                    NULL,
                    "Initializing TLS subsystem: <keyring/> element inside the "
                    "TLS configuration, that does not contain a file-name.");
                continue;
            }

            // load the OpenPGP keyring
            ret = gnutls_certificate_set_openpgp_keyring_file(
                current_credentials, file, GNUTLS_OPENPGP_FMT_BASE64);
            if (ret < 0) {
                log_error(NULL, "Error loading OpenPGP keyring %s: %s", file,
                          gnutls_strerror(ret));
                continue;
            }
    ...
    for (crl_p = crl_files_pem.begin(); crl_p != crl_files_pem.end(); ++crl_p) {
        ...
        ret = gnutls_certificate_set_x509_crl_file(
            current_credentials, file_to_load, GNUTLS_X509_FMT_PEM);
        if (ret < 0) {
            log_error(NULL, "Error loading CRL file %s (PEM): %s", file_to_load,
                      gnutls_strerror(ret));
        }
    }
    for (crl_p = crl_files_der.begin(); crl_p != crl_files_der.end(); ++crl_p) {
        ...
        ret = gnutls_certificate_set_x509_crl_file(
            current_credentials, file_to_load, GNUTLS_X509_FMT_DER);
        if (ret < 0) {
            log_error(NULL, "Error loading CRL file %s (DER): %s", file_to_load,
                      gnutls_strerror(ret));
        }
    }
    ...
    if (!loaded_cacerts) {
        ...
        for (p = default_cacertfile_pem.begin();
             p != default_cacertfile_pem.end(); ++p) {
            ret = gnutls_certificate_set_x509_trust_file(
                current_credentials, p->c_str(), GNUTLS_X509_FMT_PEM);
            if (ret < 0) {
                log_error(NULL,
                          "Error loading default CA certificate %s (PEM): %s",
                          p->c_str(), gnutls_strerror(ret));
            }
        }
        for (p = default_cacertfile_der.begin();
             p != default_cacertfile_der.end(); ++p) {
            ret = gnutls_certificate_set_x509_trust_file(
                current_credentials, p->c_str(), GNUTLS_X509_FMT_DER);
            if (ret < 0) {
                log_error(NULL,
                          "Error loading default CA certificate %s (DER): %s",
                          p->c_str(), gnutls_strerror(ret));
            }
        }
    }
    ...
    for (p = domains.begin(); p != domains.end(); ++p) {
        if (mio_tls_credentials.find(*p) != mio_tls_credentials.end()) {
            log_warn(NULL,
                     "Redefinition of TLS credentials for domain %s. Ignoring "
                     "redefinition.",
                     p->c_str());
            continue;
        }
    ...
    if (!credentials_used) {
        log_warn(
            NULL,
            "Found credentials definition, that is not used for any domain.");
 ...
 * @param default_cacertfile_pem list of PEM files containing the CA
 * certificates to use
 * @param default_cacertfile_der list of DER files containing the CA
 * certificates to use
 * @param mio_tls_dh_params the Diffie-Hellman parameters to be used
 */
static void
mio_tls_process_key(xmlnode x,
    ...
    if (no_ssl_v2 != NULL || no_ssl_v3 != NULL || no_tls_v1 != NULL ||
        ciphers != NULL) {
        log_notice(id, "Warning: ignoring a attribute when loading X.509 "
                       "certificate. Not supported anymore are: no-ssl-v2, "
                       "no-ssl-v3, no-tls-v1 and ciphers");
    }
    ...
    ret = gnutls_certificate_allocate_credentials(&current_credentials);
    if (ret < 0) {
        log_error(id, "Error initializing credentials: %s",
                  gnutls_strerror(ret));
        return;
    }
    ret = gnutls_certificate_set_x509_key_file(
        current_credentials, file_to_use, private_key_file, certificate_type);
    if (ret < 0) {
        log_error(id, "Error loading key file cert=%s/priv=%s: %s", file_to_use,
                  private_key_file, gnutls_strerror(ret));
        gnutls_certificate_free_credentials(current_credentials);
        return;
    }
    ...
    std::list<std::string>::const_iterator p;
    for (p = default_cacertfile_pem.begin(); p != default_cacertfile_pem.end();
         ++p) {
        ret = gnutls_certificate_set_x509_trust_file(
            current_credentials, p->c_str(), GNUTLS_X509_FMT_PEM);
        if (ret < 0) {
            log_error(id, "Error loading CA certificates %s (PEM): %s",
                      p->c_str(), gnutls_strerror(ret));
        }
    }
    for (p = default_cacertfile_der.begin(); p != default_cacertfile_der.end();
         ++p) {
        ret = gnutls_certificate_set_x509_trust_file(
            current_credentials, p->c_str(), GNUTLS_X509_FMT_DER);
        if (ret < 0) {
            log_error(id, "Error loading CA certificates %s (DER): %s",
                      p->c_str(), gnutls_strerror(ret));
        }
    }
...
bool mio_tls_early_init() {
    ...
    if (ret != 0) {
        std::cerr << "Error initializing GnuTLS library: "
    ...
    if (ret != ASN1_SUCCESS) {
        std::cerr << "Error preparing the libtasn1 library: "
                  << ::asn1_strerror(ret) << std::endl;
        return false;
        /* XXX we have to delete the structure on shutdown using
         * asn1_delete_structure(&mio_tls_asn1_tree) */
    }
 ...
 *
 * @param x xmlnode containing the configuration information (the io/tls
 * element)
 */
void mio_ssl_init(xmlnode x) {
    ...
    for (cur = xmlnode_get_firstchild(x); cur != NULL;
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "cacertfile") == 0) {
            ...
            if (cacertfile_data != NULL) {
                bool der_format =
                    j_strcmp(xmlnode_get_attrib_ns(cur, "type", NULL), "der") ==
                    0;
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "crlfile") == 0) {
            ...
            char const *const crlfile_type =
                xmlnode_get_attrib_ns(cur, "type", NULL);
    ...
    if (ret < 0) {
        log_error(ZONE, "Error initializing DH params: %s",
                  gnutls_strerror(ret));
    }
    ...
    if (dhparams != "") {
        ...
        if (filehandle == -1) {
            log_warn(NULL, "Cannot open %s for reading dhparams: %s",
                     dhparams.c_str(), strerror(errno));
        } else {
            ...
            pkcs3_data.data = const_cast<unsigned char *>(
                reinterpret_cast<const unsigned char *>(filecontent.c_str()));
            ret = gnutls_dh_params_import_pkcs3(
                mio_tls_dh_params, &pkcs3_data,
                dhparams_der ? GNUTLS_X509_FMT_DER : GNUTLS_X509_FMT_PEM);
            if (ret > 0) {
                log_warn(NULL, "Error importing dhparams (%s) %s: %s",
                         dhparams_der ? "DER" : "PEM", dhparams.c_str(),
                         gnutls_strerror(ret));
            } else {
                dhparams_set = true;
            }
    ...
    if (!dhparams_set) {
        ...
        if (ret < 0) {
            log_error(ZONE, "Error generating DH params: %s",
                      gnutls_strerror(ret));
        }
    }
    ...
    for (cur = xmlnode_get_firstchild(x); cur != NULL;
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "credentials") == 0 &&
            j_strcmp(xmlnode_get_namespace(cur), NS_JABBERD_CONFIGFILE) == 0) {
            /* it's a credentials group (new format) */
            mio_tls_process_credentials(
                cur, default_cacertfile_pem, default_cacertfile_der,
                mio_tls_dh_params, crl_files_pem, crl_files_der);
        } else if (j_strcmp(xmlnode_get_localname(cur), "key") == 0 &&
                   j_strcmp(xmlnode_get_namespace(cur),
                            NS_JABBERD_CONFIGFILE) == 0) {
            /* it's a key - old format */
            mio_tls_process_key(cur, default_cacertfile_pem,
                                default_cacertfile_der, mio_tls_dh_params);
        }
    }
 ...
 *
 * The m->flags.recall_read_when_readable and
 * m->flags.recall_read_when_writeable is updated by this function.
 *
...
ssize_t _mio_ssl_read(mio m, void *buf, size_t count) {
    ...
    log_debug2(ZONE, LOGT_IO,
               "Trying to read up to %i B from socket %i using GnuTLS", count,
               m->fd);
    ...
    /* trying to read */
    read_return = gnutls_record_recv(static_cast<gnutls_session_t>(m->ssl),
                                     (char *)buf, count);
    ...
    if (read_return == GNUTLS_E_INTERRUPTED || read_return == GNUTLS_E_AGAIN) {
        if (gnutls_record_get_direction(
                static_cast<gnutls_session_t>(m->ssl)) == 0) {
    ...
    if (read_return == GNUTLS_E_FATAL_ALERT_RECEIVED) {
        log_notice(NULL, "Received fatal TLS alert on socket %i: %s", m->fd,
                   gnutls_alert_get_name(gnutls_alert_get(
                       static_cast<gnutls_session_t>(m->ssl))));
    } else if (read_return == GNUTLS_E_WARNING_ALERT_RECEIVED) {
        log_debug2(ZONE, LOGT_IO, "Received warning TLS alert on socket %i: %s",
                   m->fd,
                   gnutls_alert_get_name(gnutls_alert_get(
                       static_cast<gnutls_session_t>(m->ssl))));
    }

    log_debug2(ZONE, LOGT_IO, "Error case after gnutls_record_recv(): %s",
               gnutls_strerror(read_return));
 ...
 *
 * The m->flags.recall_write_when_readable and
...
ssize_t _mio_ssl_write(mio m, const void *buf, size_t count) {
    ...
    log_debug2(ZONE, LOGT_IO,
               "Trying to write up to %i B to socket %i using GnuTLS", count,
               m->fd);
    ...
    write_return =
        gnutls_record_send(static_cast<gnutls_session_t>(m->ssl), buf, count);

    if (write_return > 0) {
        log_debug2(ZONE, LOGT_IO, "Wrote %i B on socket %i", write_return,
                   m->fd);
        return write_return;
    }
    if (write_return == GNUTLS_E_INTERRUPTED ||
        write_return == GNUTLS_E_AGAIN) {
        if (gnutls_record_get_direction(
                static_cast<gnutls_session_t>(m->ssl)) == 0) {
    ...
    log_debug2(ZONE, LOGT_IO, "Error case after gnutls_record_send(): %s",
               gnutls_strerror(write_return));

    return -1;
}

/**
 * continue a TLS handshake (as server side) when new data is available or data
 * can be written now
 *
...
int _mio_tls_cont_handshake_server(mio m) {
    ...
    if (ret >= 0) {
        ...
        if (gnutls_record_get_direction(
            ...
            log_debug2(
                ZONE, LOGT_IO,
                "TLS layer needs to read data to complete handshake (fd #%i)",
                m->fd);
            m->flags.recall_handshake_when_readable = 1;
        } else {
            log_debug2(
                ZONE, LOGT_IO,
                "TLS layer needs to write data to complete handshake (fd #%i)",
                m->fd);
            m->flags.recall_handshake_when_writeable = 1;
        }
        return 0;
    } else {
        log_debug2(ZONE, LOGT_IO, "TLS handshake failed for fd #%i: %s", m->fd,
                   gnutls_strerror(ret));
        return -1;
    }
}
...
/**
 * accepted a new incoming connection, where we have to start the TLS layer
 * without a STARTTLS command, e.g. on port 5223
 *
 ...
 * @return 0 if the connection is unprotected, 1 if the connection is integrity
 * protected, >1 if the connection is encrypted
 */
int mio_is_encrypted(mio m) {
    return m->ssl == NULL ? 0
                          : 8 * gnutls_cipher_get_key_size(gnutls_cipher_get(
                                    static_cast<gnutls_session_t>(m->ssl)));
}
...
int mio_ssl_starttls_possible(mio m, const char *identity) {
    /* if the connection is already TLS protected, we cannot start a second TLS
     * layer */
    if (m->ssl != NULL) {
        return 0;
    }
    ...
    if (identity != NULL &&
        mio_tls_credentials.find(identity) != mio_tls_credentials.end()) {
        return 1;
    }
...
/**
 * start a TLS layer on a connection and set the appropriate mio handlers for
 * SSL/TLS
 *
...
int mio_ssl_starttls(mio m, int originator, const char *identity) {
    ...
    if (m->ssl != NULL) {
        log_debug2(
            ZONE, LOGT_EXECFLOW | LOGT_IO,
            "cannot start TLS layer on an already encrapted socket (mio=%X)",
            m);
        return 1;
    }
    ...
    if (used_credentials == NULL) {
        log_error(identity,
                  "Cannot start TLS layer for %s - no credentials available, "
                  "even no default ones",
                  identity);
        return 1;
    }

    log_debug2(ZONE, LOGT_IO,
               "Establishing TLS layer for %s connection (we=%s, peer=%s, "
               "identity=%s)",
               originator ? "outgoing" : "incoming", m->our_ip, m->peer_ip,
               identity);

    /* GNU TLS setup for this connection */
    ret = gnutls_init(&session, originator ? GNUTLS_CLIENT : GNUTLS_SERVER);
    if (ret != 0) {
        log_debug2(ZONE, LOGT_IO, "Error initializing session for fd #%i: %s",
                   m->fd, gnutls_strerror(ret));
        return 1;
    }
    ...
    if (ret != 0) {
        log_debug2(ZONE, LOGT_IO,
                   "Error setting default priorities for fd #%i: %s", m->fd,
                   gnutls_strerror(ret));
    }
    ...
    if (ret != 0) {
        log_debug2(ZONE, LOGT_IO,
                   "Error setting certificate credentials for fd #%i: %s",
                   m->fd, gnutls_strerror(ret));
    }
    if (!originator) {
    ...
    if (ret < 0) {
        if (ret == GNUTLS_E_INTERRUPTED || ret == GNUTLS_E_AGAIN) {
            if (gnutls_record_get_direction(session) == 0) {
                m->flags.recall_handshake_when_readable = 1;
                log_debug2(ZONE, LOGT_IO,
                           "TLS layer needs to read data to complete handshake "
                           "(mio %X, fd #%i)",
                           m, m->fd);
            } else {
                m->flags.recall_handshake_when_writeable = 1;
                log_debug2(ZONE, LOGT_IO,
                           "TLS layer needs to write data to complete "
                           "handshake (mio %X, fd #%i)",
                           m, m->fd);
            }
        ...
        mio_close(m);
        gnutls_deinit(session);
        log_debug2(ZONE, LOGT_IO, "TLS handshake failed on socket #%i: %s",
                   m->fd, gnutls_strerror(ret));
        return 1;
    }
    ...
    m->k.val = 100;
    m->ssl = session;
    log_debug2(ZONE, LOGT_EXECFLOW, "m->ssl is now %X, session=%X", m->ssl,
               session);

    pool_cleanup(m->p, _mio_ssl_cleanup, (void *)session);

    log_debug2(ZONE, LOGT_IO, "Established TLS layer on socket %d, mio %X",
               m->fd, m);
...
static int mio_tls_cert_match(pool p, const char *cert_dom,
    ...
    if (strchr(cert_dom, '*') == NULL) {
        ...
        /* to match the matching part has to be bigger than the domain we are
         * expecting */
 ...
 * @param id_on_xmppAddr the xmpp address the peer claims to be, and that should
 * be checked
 ...
 */
static int mio_tls_check_x509(mio m, char const *id_on_xmppAddr,
    ...
    if (cert_list == NULL || cert_list_size <= 0) {
        log_notice(log_id.c_str(),
                   "Problem verifying certificate: No certificate was found!");
        return 0;
    }

    log_debug2(ZONE, LOGT_AUTH, "We have to verify %i certificates for %s",
               cert_list_size, id_on_xmppAddr);
    ...
    for (unsigned int crt_index = 0; crt_index < cert_list_size; crt_index++) {
        ...
        if (ret < 0) {
            log_warn(log_id.c_str(),
                     "Problem initializing the certificate var. Therefore I "
                     "cannot verify the certificate.");
            return 0;
        }

        /* get this certificate */
        ret = gnutls_x509_crt_import(cert, &cert_list[crt_index],
                                     GNUTLS_X509_FMT_DER);
        if (ret < 0) {
            log_warn(log_id.c_str(), "Error in loading certificate %i: %s",
                     crt_index, gnutls_strerror(ret));
        ...
        if (ret < 0) {
            log_warn(log_id.c_str(), "Error accessing DN of certificate %i: %s",
                     crt_index, gnutls_strerror(ret));
        } else {
            cert_subject = name;
        }
        log_debug2(ZONE, LOGT_AUTH, "verifying certificate: %s",
                   cert_subject.c_str());
        ...
        if (crt_index == 0 && id_on_xmppAddr != NULL) {
            ...
            log_debug2(ZONE, LOGT_AUTH,
                       "verifying first certificate in chain ...");

            /* only pure domains may be matched against dNSName */
            if (id_on_xmppAddr == NULL || strchr(id_on_xmppAddr, '@') != NULL ||
                strchr(id_on_xmppAddr, '/') != NULL) {
                may_match_dNSName = 0;
            }
            ...
            do {
                ...
                ret = gnutls_x509_crt_get_extension_by_oid(
                    cert, "2.5.29.17", ext_count, subjectAltName,
                    &subjectAltName_size, &is_critical);
                if (ret < 0) {
                    if (ret == GNUTLS_E_REQUESTED_DATA_NOT_AVAILABLE) {
                        log_debug2(ZONE, LOGT_AUTH,
                                   "no more subjectAltName extensions (%i)",
                                   ext_count);
                    } else {
                        log_warn(
                            log_id.c_str(),
                            "error requesting %i-th subjectAltName: %s (%s)",
                            ext_count, gnutls_strerror(ret),
                            cert_subject.c_str());
                    }
                } else {
                    ...
                    log_debug2(ZONE, LOGT_AUTH,
                               "got a%s subjectAltName extension",
                    ...
                    ret = asn1_create_element(mio_tls_asn1_tree,
                                              "PKIX1.SubjectAltName",
                                              &subjectAltName_element);
                    if (ret != ASN1_SUCCESS) {
                        log_warn(log_id.c_str(),
                                 "error creating asn1 element for "
                                 "PKIX1.SubjectAltName: %s (%s)",
                                 asn1_strerror(ret), cert_subject.c_str());
                        break;
                    }

                    /* decode the extension */
                    ret = asn1_der_decoding(&subjectAltName_element,
                                            subjectAltName, subjectAltName_size,
                                            NULL);
                    if (ret != ASN1_SUCCESS) {
                        log_warn(log_id.c_str(),
                                 "error DER decoding subjectAltName extension: "
                                 "%s (%s)",
                                 asn1_strerror(ret), cert_subject.c_str());
                    ...
                    for (cnt = 1; cnt < 1024 && !found_matching_subjectAltName;
                         cnt++) {
                        char cnt_string[12];
                        ...
                        log_debug2(ZONE, LOGT_AUTH,
                                   "accessing subjectAltName element %s",
                                   cnt_string);

                        ret =
                            asn1_read_value(subjectAltName_element, cnt_string,
                                            address_type, &address_type_len);
                        if (ret == ASN1_ELEMENT_NOT_FOUND) {
                            log_debug2(ZONE, LOGT_AUTH,
                                       "no more values in subjectAltName (%s)",
                                       cnt_string);
                            break;
                        }
                        if (ret != ASN1_SUCCESS) {
                            log_notice(log_id.c_str(),
                                       "error accessing type for %s in "
                                       "subjectAltName: %s (%s)",
                                       cnt_string, asn1_strerror(ret),
                                       cert_subject.c_str());
                            break;
                        }

                        log_debug2(ZONE, LOGT_AUTH, "... it is of type %s",
                                   address_type);
                        ...
                        if (j_strncmp(address_type, "dNSName", 8) == 0) {
                            if (!may_match_dNSName) {
                                ...
                                snprintf(access_string, sizeof(access_string),
                                         "%s.dNSName", cnt_string);

                                ret = asn1_read_value(subjectAltName_element,
                                                      access_string, dNSName,
                                                      &dNSName_len);
                                if (ret != ASN1_SUCCESS) {
                                    log_notice(log_id.c_str(),
                                               "error accessing %s in "
                                               "subjectAltName: %s (%s)",
                                               access_string,
                                               asn1_strerror(ret),
                                               cert_subject.c_str());
                                ...
                                log_debug2(ZONE, LOGT_AUTH, "found dNSName: %s",
                                           dNSName);
                                ...
                                /* compare the dNSName */
                                if (mio_tls_cert_match(compare_pool, dNSName,
                                                       id_on_xmppAddr) == 0) {
                                    found_matching_subjectAltName = 1;
                                    log_debug2(ZONE, LOGT_AUTH,
                                               "match on dNSName: %s", dNSName);
                                }
                            ...
                            snprintf(access_string_type,
                                     sizeof(access_string_type),
                                     "%s.otherName.type-id", cnt_string);
                            snprintf(access_string_value,
                                     sizeof(access_string_value),
                                     "%s.otherName.value", cnt_string);

                            /* get the OID of the otherName */
                            ret = asn1_read_value(
                                subjectAltName_element, access_string_type,
                                otherNameType, &otherNameType_len);
                            if (ret != ASN1_SUCCESS) {
                                log_notice(log_id.c_str(),
                                           "error accessing type information "
                                           "%s in subjectAltName: %s (%s)",
                                           access_string_type,
                                           asn1_strerror(ret),
                                           cert_subject.c_str());
                                break;
                            ...
                            if (j_strncmp(otherNameType, "1.3.6.1.5.5.7.8.5",
                                          18) != 0) {
                                log_notice(log_id.c_str(),
                                           "ignoring unknown otherName in "
                                           "subjectAltName (%s)",
                                           cert_subject.c_str());
                                break;
                            }
                            ...
                            ret = asn1_read_value(
                                subjectAltName_element, access_string_value,
                                otherNameValue, &otherNameValue_len);
                            if (ret != ASN1_SUCCESS) {
                                log_notice(log_id.c_str(),
                                           "error accessing value of othername "
                                           "%s in subjectAltName: %s (%s)",
                                           access_string_value,
                                           asn1_strerror(ret),
                                           cert_subject.c_str());
                            ...
                            /* okay we now have an UTF8String ... get the
                             * content */
                            {
                                ASN1_TYPE directoryString_element =
                                    ASN1_TYPE_EMPTY;
                                char thisIdOnXMPPaddr[3072];
                                int thisIdOnXMPPaddr_len =
                                    sizeof(thisIdOnXMPPaddr);
                                pool jid_pool = NULL;
                                jid cert_jid = NULL;

                                ret = asn1_create_element(
                                    mio_tls_asn1_tree, "PKIX1.DirectoryString",
                                    &directoryString_element);
                                if (ret != ASN1_SUCCESS) {
                                    log_notice(log_id.c_str(),
                                               "error creating DirectoryString "
                                               "element: %s (%s)",
                                               asn1_strerror(ret),
                                               cert_subject.c_str());
                                ...
                                ret = asn1_der_decoding(
                                    &directoryString_element, otherNameValue,
                                    otherNameValue_len, NULL);
                                if (ret != ASN1_SUCCESS) {
                                    log_notice(log_id.c_str(),
                                               "error decoding "
                                               "DirectoryString: %s (%s)",
                                ...
                                ret = asn1_read_value(
                                    directoryString_element, "utf8String",
                                    thisIdOnXMPPaddr, &thisIdOnXMPPaddr_len);
                                if (ret != ASN1_SUCCESS) {
                                    log_notice(log_id.c_str(),
                                               "error accessing utf8String of "
                                               "DirectoryString: %s (%s)",
                                               asn1_strerror(ret),
                                               cert_subject.c_str());
                                    asn1_delete_structure(
                                ...
                                if ((size_t)thisIdOnXMPPaddr_len >=
                                    sizeof(thisIdOnXMPPaddr)) {
                                    log_notice(log_id.c_str(),
                                               "id-on-xmppAddr is %i B long "
                                               "... ignoring (%s)",
                                               thisIdOnXMPPaddr_len,
                                               cert_subject.c_str());
                                    asn1_delete_structure(
                                        &directoryString_element);
                                    break;
                                ...
                                log_debug2(ZONE, LOGT_AUTH,
                                           "found id-on-xmppAddr: %s",
                                           jid_full(cert_jid));

                                /* compare */
                                if (j_strcmp(id_on_xmppAddr,
                                             jid_full(cert_jid)) == 0) {
                                    found_matching_subjectAltName = 1;
                                    log_debug2(ZONE, LOGT_AUTH,
                                               "match on id-on-xmppAddr: %s",
                                               thisIdOnXMPPaddr);
                                }
                            ...
                            log_notice(log_id.c_str(),
                                       "ignoring %s in subjectAltName (%s)",
                                       address_type, cert_subject.c_str());
                        }
                    }
            ...
            if (found_any_subjectAltName) {
                if (!found_matching_subjectAltName) {
                    log_notice(log_id.c_str(),
                               "Found subjectAltName, but non matched (%s)",
                               cert_subject.c_str());
                    verification_result = 0;
                    gnutls_x509_crt_deinit(cert);
                    cert = NULL;
                ...
                if (!gnutls_x509_crt_check_hostname(cert, id_on_xmppAddr)) {
                    log_notice(log_id.c_str(),
                               "Certificate subject does not match. (%s)",
                               cert_subject.c_str());
                    verification_result = 0;
                    gnutls_x509_crt_deinit(cert);
                    cert = NULL;
        ...
        if (gnutls_x509_crt_get_expiration_time(cert) < time(NULL)) {
            log_notice(log_id.c_str(), "Certificate %i has expired (%s)",
                       crt_index, cert_subject.c_str());
            verification_result = 0;
            gnutls_x509_crt_deinit(cert);
            cert = NULL;
            break;
        ...
        if (gnutls_x509_crt_get_activation_time(cert) > time(NULL)) {
            log_notice(log_id.c_str(), "Certificate %i not yet active (%s)",
                       crt_index, cert_subject.c_str());
            verification_result = 0;
            gnutls_x509_crt_deinit(cert);
            cert = NULL;
 ...
 * @param id_on_xmppAddr the JabberID, that the certificate should be checked
 * for, if NULL it is only checked if the certificate is valid and trusted
...
int mio_ssl_verify(mio m, const char *id_on_xmppAddr) {
    ...
    ret = gnutls_certificate_verify_peers2(
        static_cast<gnutls_session_t>(m->ssl), &status);
    if (ret != 0) {
        log_notice(log_id.c_str(), "TLS cert verification failed: %s",
                   gnutls_strerror(ret));
        return 0;
    }
    if (status != 0) {
        ...
        if (gnutls_certificate_type_get(
                static_cast<gnutls_session_t>(m->ssl)) == GNUTLS_CRT_X509) {
            cert_list = gnutls_certificate_get_peers(
                static_cast<gnutls_session_t>(m->ssl), &cert_list_size);
            if (cert_list != NULL && cert_list_size > 0) {
                gnutls_x509_crt_t cert = NULL;
        ...
        log_notice(log_id.c_str(), "Certificate verification failed: %s (%s)",
                   got_a_message ? messages.str().c_str() : "unknown reason",
                   cert_subject.c_str());
        return 0;
    }
    ...
    gnutls_certificate_type_t cert_type =
        gnutls_certificate_type_get(static_cast<gnutls_session_t>(m->ssl));
    switch (cert_type) {
        case GNUTLS_CRT_X509:
            return mio_tls_check_x509(m, id_on_xmppAddr, log_id);
        default:
            /* no ... we cannot handle other certificates here yet ... declare
             * as invalid */
            log_notice(
                log_id.c_str(),
                "Rejecting certificate as it is no supported certificate "
                "format: %s",
...
void mio_tls_get_characteristics(mio m, char *buffer, size_t len) {
    ...
    characteristics << gnutls_protocol_get_name(
        gnutls_protocol_get_version(session));
    characteristics << "/";
    characteristics << gnutls_cipher_suite_get_name(gnutls_kx_get(session),
