...
/**
 * @mainpage jabberd14 - implementation of an instant messaging server using the
 * Jabber/XMPP protocols in C
 *
 * @section BigPicture The big picture
 *
 * jabberd14 consists of a base executable (the jabberd binary) implemented
 * inside in the directory @link jabberd jabberd.@endlink This base executable
 * implements a router for XML fragments (called packets or stanzas). The
 * routing at this level is in general not visible to the user of the Jabber
 * server, but used by the components, that form the Jabber server to exchange
 * such XML fragments with each other.
 *
 * The other parts of jabberd14 are the components, which are all implemented in
 * their own directories. These components are:
 * - The client connection manager, which task is to accept incoming TCP/IP
 * connections from clients and forwards incoming stanzas from the client to an
 * other component called session manager (JSM) and forwards packets from the
 * session manager back to the client on the TCP/IP connection. The exchange of
 * the stanzas between the client connection manager and the session manager is
 * done using the routing functionallity provided by the jabberd executable. The
 * client connection manager (historically sometimes called pthsock_client) is
 * implemented inside the directory @link pthsock pthsock.@endlink
 * - The Jabber session manager, which task is to implement the visible business
 * logic. It implements what is afterwards seen as the Jabber server and
 * addressed by the domain used by this server. It maintains presence
 * subscriptions, the presence of the current sessions of a user and forwards
 * messages or stores them offline. The session manager is implemented inside
 * the directory @link jsm jsm.@endlink
 * - The DNS resolver (@link dnsrv dnsrv@endlink) typically handles all packets
 * on the XML router that have no configured destination on the router (which is
 * as you remember implemented by the jabberd binary). It will try to resolve
 * the destination domain using DNS and if a DNS entry is found, it tags the XML
 * fragment with a list of resolved IP addresses and resends the XML fragment
 * using the XML router to one (or many in case of clustering) component, that
 * is then responsible for sending the XML fragment to another host. This other
 * component is the dialback component as described below.
 * - The dialback component implements the interconnection between the local
 * Jabber server and other Jabber servers using the server to server protocol of
 * XMPP/Jabber. It establishes connections to other servers and accepts incoming
 *   connections from them. This component is implemented inside the directory
 * @link dialback dialback.@endlink
 * - There is typically at least one other component plugged into the XML router
 * which is responsible for the persistant storing of data. This component
 * handles special XML fragments router on the XML router. This special
 * fragments are instructions for such storrage components and their replies to
 * the requestor. There are two implementations of such a component inside the
 * base package of jabberd14. The one implementation implements storing data to
 *   XML files and is implemented inside the directory @link xdb_file
 * xdb_file,@endlink the other implementation uses SQL databases to store data
 * and is implemented inside the directory @link xdb_sql xdb_sql.@endlink
 *
 * The components are compiled as shared objects (*.so files) and are loaded by
 * the jabberd base executable on startup as configured inside the configuration
 * file using the &lt;load/&gt; element inside &lt;section/&gt;s. Each section
 * inside the configuration file defines one component.
 */

/**
 * @dir jabberd
 * @brief Containing the base executable (the jabberd binary) implementing an
 * XML router and helpers
 *
 * In this directory you find the implementation of the jabberd executable,
 * which is just something, that is able to route XML fragments between the base
 * handlers (they are implemented in files in the directory
 * @link jabberd/base jabberd/base@endlink.)
 * The base handlers (which are a part of the jabberd binary) are then able to
 * connect targets and sources to this XML routing. The most known base handler
 * might be the handler for the &lt;load/&gt; target in the configuration file.
 * This handler loads a shared object file containing a component and connects
 * this component to the XML routing. Other important base handlers are the
 * handlers implemented in base_accept.cc and base_connect.cc, which implement
 * the &lt;accept/&gt; and &lt;connect/&gt; targets used to connect two (or
 * more) instances of jabberd running to build a single server.
 *
 * In addition to the XML routing, you find the implementation of the managed
 * threads (mtq.cc), a scedular to invoke regularly tasks (heartbeat.cc),
 * logging services (log.cc), the handling of the configuration file
 * (config.cc), the handling of network sockets (mio.cc, mio_raw.cc, mio_tls.cc,
 * mio_xml.cc), the XML database interface (xdb.cc), and access control lists
 * (acl.cc).
 *
 * The XML routing itself is implemented in the file deliver.cc. Routines used
 * for the startup of the server can be found in jabberd.cc.
 *
 * The jabberd executable also contains a library of functions used either by
 * the base executable itself or that are of general use for components (which
 * are implemented as loadable objects). This jabberd library is implemented in
 * @link jabberd/lib jabberd/lib.@endlink
 */
...
/**
 * the stages of packet delivery - the order in which packet handlers gets
 * called.
 *
 * Delivery starts with o_PRECOND handlers, then o_COND handlers, then
 * o_PREDELIVER handlers, and o_DELIVER handlers last
 */
typedef enum {
    o_PRECOND, /**< o_PRECOND handlers always get called first - used by xdb.cc
                  to filter out xdb results */
    o_COND,    /**< currently not used by jabberd14 */
    o_PREDELIVER, /**< handlers that should get called before o_DELIVER handlers
                     - used by base_format.cc to reformat messages */
 ...
 * of the other values is returned, the data has not been consumed and the
 * caller must free it if necessary.
 */
typedef enum {
    r_UNREG, /**< unregister the packet handler and keep delivering the packet
              */
    r_NONE,  /**< unused value ... might get removed */
    r_PASS,  /**< packet has not been handled nor did it cause an error, keep
                delivering the packet to later handlers */
    r_LAST,  /**< can only be returned by o_COND handlers in which case delivery
                is not continued with later handlers -- currently not used by
                jabberd14 */
    r_ERR, /**< final delivery error, create error bounce and do not call later
              handlers */
    r_DONE /**< packet has been handled, if o_DELIVER handler other handlers are
              called as well, else delivery is stopped */
} result;
...
/** Callback function that gets notified of registering/unregistering hosts for
 * an instance */
typedef void (*register_notify)(instance i, const char *destination,
...
typedef struct register_notifier_struct {
    ...
    struct register_notifier_struct
        *next; /**< pointer to further functions, that want to get the notify */
...
struct instance_struct {
    ...
    register_notifier
        routing_update_callbacks; /**< list of callback functions, that should
                                     be called on a routing update */
    std::map<std::string, void *>
        *module_init_funcs; /**< map with the modules of this instance, key is
                               name, value is init function */
    std::set<Glib::ustring> *static_hosts; /**< set of host that are fixed
...
void register_config(
    pool p, char const *node, cfhandler f,
    void *arg); /* register a function to handle that node in the config file */
void register_beat(
    int freq, beathandler f,
    void *arg); /* register the function to be called from the heartbeat, freq
                   is how often, <= 0 is ignored */
typedef void (*shutdown_func)(void *arg);
void register_shutdown(
    shutdown_func f,
    void *arg); /* register to be notified when the server is shutting down */

// functions in deliver.cc
void register_instance(instance i,
                       char const *host); /* associate an id with a hostname for
                                             that packet type */
void unregister_instance(instance i,
                         char const *host); /* disassociate an id with a
                                               hostname for that packet type */
void register_routing_update_callback(
    instance i, register_notify f,
    void *arg); /**< register a function that gets called on
                   registering/unregistering a host for an instance */
void register_phandler(
    instance id, order o, phandler f,
    void *arg); /* register a function to handle delivery for this instance */
dpacket
dpacket_new(xmlnode x); /* create a new delivery packet from source xml */
...
void deliver_fail(dpacket p,
                  const char *err); /* bounce a packet intelligently */
// void deliver_instance(instance i, dpacket p); /* deliver packet TO the
// instance, if the result != r_DONE, you have to handle the packet! */
bool deliver_is_delivered_to(
    Glib::ustring const &host,
    _instance const *i); /* util that returns the instance handling this
                            hostname for normal packets */
bool deliver_is_uplink(
    instance i); // checks if an instance is configured to be the uplink
...
void logger(
    char const *type, char const *host,
    char const *message); /* actually creates and delivers the log message */
void log_record(char const *id, char const *type, char const *action,
                char const *msgfmt,
                ...); /* for generic logging support, like
                         log_record("jer@jabber.org","session","end","...") */
void log_generic(char const *logtype, char const *id, char const *type,
                 char const *action, char const *msgfmt, ...);
...
typedef struct xdbcache_struct {
    ...
    jid owner;
    int sent;
    int preblock; /**< thread that created the query is waiting for a
                     pth_cond_notify() on ::cond */
    pth_cond_t cond;
    pth_mutex_t mutex;
...
xmlnode xdb_get(xdbcache xc, jid owner,
                const char *ns); /**< blocks until namespace is retrieved,
                                    returns xmlnode or NULL if failed */
int xdb_act(xdbcache xc, jid owner, char const *ns, char const *act,
            char const *match, xmlnode data);
int xdb_act_path(
    xdbcache xc, jid owner, const char *ns, char const *act,
    char const *matchpath, xht namespaces,
    xmlnode data); /**< sends new xml action, returns non-zero if failure */
int xdb_set(xdbcache xc, jid owner, const char *ns,
            xmlnode data); /**< sends new xml to replace old, returns non-zero
                              if failure */

/* Error messages */
#define SERROR_NAMESPACE                                                       \
    "<stream:error><invalid-namespace "                                        \
    "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "                      \
    "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>Invalid "       \
    "namespace specified.</text></stream:error>"
#define SERROR_INVALIDHOST                                                     \
    "<stream:error><invalid-from "                                             \
    "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "                      \
    "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>Invalid "       \
    "hostname used.</text></stream:error>"
...
/** Managed thread queue. Has a pointer to the currently assigned thread for
 * this queue. */
...
/** Managed thread queue. Has the message port for the running thread, and the
 * current queue it's processing. */
...
mtq mtq_new(pool p); /**< Creates a new queue, is automatically cleaned up when
                        p frees */

void mtq_send(
    mtq q, pool p, mtq_callback f,
    void *arg); /**< appends the arg to the queue to be run on a thread */
...
struct mio_st;
typedef void (*mio_std_cb)(mio_st *m, int state, void *arg, xmlnode x,
...
typedef struct mio_st {
    ...
    mio_state state; /**< state of this manages socket, used to flag a socket
                        that it needs to be closed */

    mio_wbq queue; /**< write buffer queue */
    mio_wbq tail;  /**< the last buffer queue item */

    struct mio_st *prev, *next; /**< pointers to the previous and next item, if
                                   a list of mio_st elements is build */

    void *cb_arg;  /**< MIO event callback argument (do not modify directly) */
    mio_std_cb cb; /**< MIO event callback (do not modify directly) */
    struct mio_handlers_st *mh; /**< MIO internal handlers (for reading,
                                   writing, setting up TLS layers) */

    xstream xs;        /* XXX kill me, I suck */
    XML_Parser parser; /**< sax instance used for this mio socket if we layer an
                          XML stream on top of it */
    xmlnode stacknode; /**< the stanza that is currently received */
    void *ssl; /**< TLS layer instance for this managed socket (either GNU TLS
                  or OpenSSL) */
    struct {
        int root : 1;  /**< 0 = waiting for the stream root tag, 1 = stream root
                          tag already received */
        int rated : 1; /**< 0 = no rating for this socket, 1 = socket is rate
                          limited (see ::rate) */
        int reset_stream : 1; /**< set to 1, if stream has to be resetted */
        int recall_read_when_readable : 1;  /**< recall the read function, when
                                               the socket has data available for
                                               reading */
        int recall_read_when_writeable : 1; /**< recall the read function, when
                                               the socket has data available for
                                               writing */
        int recall_write_when_readable : 1; /**< recall the write function, when
                                               the socket has data available for
                                               reading */
        int recall_write_when_writeable : 1;     /**< recall the write function,
                                                    when the socket allows writing
                                                    again */
        int recall_handshake_when_readable : 1;  /**< recall the handshake
                                                    function, when the socket has
                                                    data available for reading */
        int recall_handshake_when_writeable : 1; /**< recall the handshake
                                                    function, when the socket
                                                    allows writing again */
    } flags;

    struct karma k;     /**< karma for this socket, used to limit bandwidth of a
                           connection */
    ...
    char const *connect_errmsg; /**< error message on failed connects (don't
                                   free messages) */
    char
        *authed_other_side; /**< if the other side of the stream is
                               authenticated, the identity can be placed here */

    xmppd::ns_decl_list *out_ns;  /**< pointer to the namespaces declared on the
                                     outgoing stream root element */
    xmppd::ns_decl_list *in_root; /**< pointer to the namespaces declared on the
                                     incoming root element */
    xmppd::ns_decl_list *in_stanza; /**< pointer to the namespaces declared on
                                       the currently recevied stanza */
    const char *
        root_lang; /**< declared language of the incoming stream root element */
} * mio, _mio;
...
typedef struct mio_main_st {
    ...
    int zzz_active;  /**< if set to something else then 1, there has been sent a
                        signal already, that is not yet processed */
    struct karma *k; /**< default karma */
    int rate_t, rate_p;         /**< default rate, if any */
    char const *bounce_uri;     /**< where to bounce HTTP requests to */
    char const *webserver_path; /**< location where small HTTP requests are
                                   handled from */
...
/* Connects to an ip */
void mio_connect(char *host, int port, mio_std_cb cb, void *cb_arg, int timeout,
                 mio_handlers mh);

/* Starts listening on a port/ip, returns NULL if failed to listen */
mio mio_listen(int port, char const *sourceip, mio_std_cb cb, void *cb_arg,
               mio_handlers mh);
 ...
 * possible loglevels
 */
enum loglevel { notice, warn, error, alert };

/**
 * outstream used to generate a logging line
 *
 * A user may write as he writes to a normal stream. On destruction the written
 * data will be sent to the log.
 */
class logmessage : public std::ostringstream {
  public:
     ...
     * @return the stream on which the output has been done, can be used to
     * output more data using the operator<<()
     */
    ...
    /**
     * copy constructor: needed by the logging class to return a logmessage
     * instance for the level() member
     */
...
class instance_base {
  ...
  protected:
     ...
     *
     * The default implementation of this handler checks the type of packet that
     * has been received and calls the relevant handler: on_stanza_packet(),
     * on_xdb_packet(), on_log_packet(), or on_route_packet()
     *
     * @param dp packet that has to be handled
     * @return should normally return r_DONE if the packet is handled (for other
     * possible values see the result type)
     */
    virtual result on_packet(dpacket dp);

    /**
     * event that handles received packets, that contain a normal stanza (type
     * ::p_NORM)
     *
     * The default implementation of this handler is to forward the stanza to
     * the relevant handler: on_message_stanza(), on_presence_stanza(),
     * on_iq_stanza(), or on_subscription_stanza()
     *
     * @param dp the packet containing the stanza
     * @return should normally return r_DONE if the packet has been handled (for
     * other possible values see the ::result type)
     */
     ...
     * @return should normally return r_DONE if the stanza has been handled (for
     * other possible values see the ::result type)
     */
     ...
     *
     * @note This handler does not get notified for subscription stanzas,
     * on_subscription_stanza() is used for them instead.
     *
     * The default implementation is to bounce the stanza
     *
     * @param p the received message stanza
     ...
     *
     * If no heartbeat has been registered for this instance yet, it gets
     * registered
     *
     * @param interval the interval in seconds how often the on_heartbeat()
     * event should be fired, 0 for disabling heartbeat again
     */
     ...
     *
     * @note you should not bounce subscription stanzas. Bouncing them creates
     * presence errors, not subscription errors (which do not exist in XMPP)
     *
     ...
     *
     * @return the configuration root node of the instance, NULL if no
     * configuration (should NOT be freed)
     */
  ...
  private:
    ...
    /**
     * Method called as the beathandler that checks if the beat frequency should
     * be changed and calls the on_heartbeat() method
     */
    ...
    /**
     * the frequency with which heartbeat is requested to be done on this
     * instance
     *
