...
typedef struct mio_connect_st {
    ...
    mio_std_cb cb; /**< callback function that should be notified on the new
                      connection */
    void
        *cb_arg; /**< argument that should be passed to the callback function */
    mio_handlers mh; /**< mio internal handlers for different events, used to
                        switch between raw and TLS-protected connections, XML
                        streams or byte streams */
 ...
 */
static int _mio_compare_ipv6(const struct in6_addr *addr1,
                             const struct in6_addr *addr2, int netsize) {
    int i;
    u_int8_t mask;
 ...
 *
 * E.g. 255.255.255.0 is converted to 120 (the network is ::ffff:a.b.c.0 in this
 * case)
 *
 * @param netmask string containing the netmask in traditional IPv4 notation
 * @return number of bits in the network part of the address (range 96...128,
 * because the argument is IPv4)
 */
 ...
 * allow:
 * @return 1 if allowed by default, or IP inside an allowed network, 2 if
 * explicitly allowed IP address, 0 if not allowed deny:
 */
static int _mio_access_check(const char *address, int check_allow) {
    ...
    xmlnode io = xmlnode_get_list_item(
    ...
    if (xmlnode_get_list_item(
        ...
        return check_allow
                   ? 1
                   : 0; /* if there is no allow/deny section, allow all */
    }
    ...
    for (cur = xmlnode_get_firstchild(io); cur != NULL;
        ...
        if (j_strcmp(xmlnode_get_localname(cur),
                     check_allow ? "allow" : "deny") != 0 ||
            j_strcmp(xmlnode_get_namespace(cur), NS_JABBERD_CONFIGFILE) != 0)
            continue;

        ip = xmlnode_get_data(
            xmlnode_get_list_item(xmlnode_get_tags(cur, "ip", namespaces), 0));
        netmask = xmlnode_get_data(xmlnode_get_list_item(
            xmlnode_get_tags(cur, "mask", namespaces), 0));
        ...
        if (cur->k.dec != 0) {
            ...
            if (was_negative && cur->k.val >= 0) {
                log_debug2(ZONE, LOGT_IO,
                           "Punishment Over for socket %d: ", cur->fd);
                _wakeup_mio_loop();
            }
        }
    }
 ...
 *
 * The new socket is inserted as the first list element, but you must not rely
 * on this.
 *
...
int _mio_write_dump(mio m) {
    ...
    while (m->queue != NULL) {
        ...
        log_debug2(ZONE, LOGT_IO, "write_dump writing data: %.*s", cur->len,
                   cur->cur);

        /* try to write a queue item */
        len = (*m->mh->write)(m, cur->cur, cur->len);
        log_debug2(ZONE, LOGT_BYTES, "written %i of %i B on socket %i: %.*s",
                   len, cur->len, m->fd, len, cur->cur);
 ...
 * connection just accepted the connection. The handler for the OpenSSL
 * encrypted connection (typically on port 5223) accepted the connection and
 * started the SSL/TLS layer afterwards. As in any case we have to first accept
 * the connection, accepting is not done in a handler anymore, but in
 * _mio_accept() itself. After a new connection has been accepted, the accepted
 * handler is called. In case of a plain connection, this handler has to do
 * nothing. In case of a port 5223 connection, this handler can start the
 * SSL/TLS layer. This also cleans up the code of _mio_accept() where we had to
 * copy values the _mio_ssl_accept handler wrote to the ::mio passed to it,
 * which was the mio of the listening socket and not the structure for the new
 * connection, which is created after the accept handler had been called.
 *
...
static mio _mio_accept(mio m) {
    ...
    fd =
        pth_accept(m->fd, (struct sockaddr *)&serv_addr, (socklen_t *)&addrlen);
    if (fd <= 0) {
        log_debug2(ZONE, LOGT_IO, "pth_accept() failed to accept on socket #%i",
                   m->fd);
        return NULL;
    }
    ...
    if (fd >= FD_SETSIZE) {
        log_warn(NULL,
                 "could not accept incoming connection, maximum number of "
    ...
    /* is there an accepted handler? E.g. starting TLS layer at connection time
     * if using Jabber over TLS on port 5223 */
    if (m->mh->accepted != NULL) {
        ...
        if (ret < 0) {
            mio_close(m);
            return newm; /* return it to get it really closed and destroyed
                            again */
        }
    }

    /* call the application callback, that wants to get notified for newly
     * accepted sockets */
    if (m->cb != NULL) {
...
static result _mio_connect_timeout(void *arg) {
    ...
    log_debug2(
        ZONE, LOGT_IO,
        "mio_connect taking too long connecting to %s, signaling to stop",
        cd->ip);
    if (cd->t != NULL)
 ...
 */
static int _mio_connect_helper(mio m, struct sockaddr *serv_addr,
                               socklen_t addrlen) {
    sigset_t set;
    int sig;
    pth_event_t wevt;
...
static void *_mio_connect(void *arg) {
    ...
    /* create the new mio object, can't call mio_new.. don't want it in select
     * yet */
    p = pool_new();
    ...
    /* XXX pthreads race condition.. cd->connected may be checked in the
     * timeout, and cd freed before these calls */
...
static void _mio_read_from_socket(mio m) {
    ...
    do {
        ...
        log_debug2(ZONE, LOGT_BYTES, "IN (%i of max %i, fd#%i): %.*s", len,
                   maxlen, m->fd, len, buf);
...
/**
 * helper function for _mio_loop_process_a_socket, that recalls the handshake
 * function
 *
...
static void _mio_do_handshake(mio m) {
    ...
    if (!m->flags.recall_handshake_when_readable &&
        !m->flags.recall_handshake_when_writeable) {
        log_debug2(ZONE, LOGT_IO, "handshake for socket %i has finished",
                   m->fd);
    }
}
 ...
 * - Writing to sockets
 *
 * If one of these steps fails, the processing of this socket is stopped and the
 * function returns
 *
 ...
 */
static void _mio_loop_process_a_socket(mio m, int *maxfd, fd_set *rfds,
                                       fd_set *wfds, int select_retval) {
    log_debug2(ZONE, LOGT_IO, "processing mio %X (state %i)", m, m->state);

    /* find the max fd: during a full interation on the master__list we always
     * update maxfd as long as we find a higher fd  */
    if (m->fd > *maxfd)
        *maxfd = m->fd;
    ...
    if (m->type == type_LISTEN) {
        if (FD_ISSET(m->fd, rfds)) {
            ...
            log_debug2(ZONE, LOGT_IO, "Accepted socket on MIO object %X, fd %i",
                       accepted_m, accepted_m != NULL ? accepted_m->fd : -1);
    ...
    if (m->flags.recall_write_when_writeable) {
        ...
        if (!FD_ISSET(m->fd, wfds)) {
            log_debug2(ZONE, LOGT_IO,
                       "socket %i waits to become writeable again ...", m->fd);
            return;
        }
    ...
    if (m->flags.recall_write_when_readable) {
        ...
        if (!FD_ISSET(m->fd, rfds)) {
            log_debug2(ZONE, LOGT_IO,
                       "socket %i waits to become readable again for being "
                       "able to write ...",
                       m->fd);
            return;
        }
    ...
    if (m->flags.recall_read_when_writeable) {
        if (!FD_ISSET(m->fd, wfds)) {
            log_debug2(ZONE, LOGT_IO,
                       "socket %i waits to become writeable again for being "
                       "able to read ...",
                       m->fd);
            return;
        }
    ...
    if (m->flags.recall_read_when_readable) {
        if (!FD_ISSET(m->fd, rfds)) {
            log_debug2(ZONE, LOGT_IO,
                       "socket %i waits to become readable again ...", m->fd);
            return;
        }
...
static void *_mio_main(void *arg) {
    fd_set wfds; /* fd set containing fds that should be checked for/had a write
                    event */
    fd_set rfds; /* fd set containing fds that should be checked for/had a write
                    event */
    mio cur = NULL, next = NULL;
    char buf[8192]; /* max socket read buffer      */
    int retval, maxfd = 0;
    static xht namespaces = NULL;
    ...
    /* loop forever -- will only exit when mio__data->master__list is NULL and
     * mio__data->shutdown is 1*/
    while (1) {
        ...
        for (cur = mio__data->master__list; cur != NULL; cur = cur->next) {
            ...
            if (cur->queue != NULL || cur->flags.recall_write_when_writeable ||
                cur->flags.recall_read_when_writeable ||
                cur->flags.recall_handshake_when_writeable)
                FD_SET(cur->fd, &wfds);

            /* check if we want to get read events for this socket */
            if (cur->k.val > 0 || cur->flags.recall_write_when_readable ||
                cur->flags.recall_read_when_readable ||
                cur->flags.recall_handshake_when_readable)
                FD_SET(cur->fd, &rfds);
        }
        ...
        /* reset maxfd, in case it changes (more updates in
         * _mio_loop_process_a_socket() is it's updated when pth_select is
         * called again) */
        maxfd = mio__data->zzz[0];
        ...
        for (cur = mio__data->master__list; cur != NULL; cur = next) {
            next = cur->next; /* a mio might get deleted inside
                                 _mio_loop_process_a_socket() so that we cannot
                                 access cur afterwards! */
...
void mio_init(void) {
    ...
    namespaces = xhash_new(3);
    xhash_put(namespaces, "", const_cast<char *>(NS_JABBERD_CONFIGFILE));
    io = xmlnode_get_list_item(xmlnode_get_tags(greymatter__, "io", namespaces),
                               0);
    ...
    if (tls == NULL) {
        ...
        if (tls != NULL) {
            log_warn(NULL,
                     "Please update your configuration. The <ssl/> elements "
                     "have been renamed to <tls/>. Falling back to use <ssl/> "
                     "for now: %s",
                     xmlnode_serialize_string(tls, xmppd::ns_decl_list(), 0));
        }
    }
    if (tls != NULL) {
        mio_ssl_init(tls);
    ...
    if (mio__data == NULL) {
        ...
        if (res) {
            log_error(NULL, "MIO I/O will probably not work correctly. Could "
                            "not create pipe.");
        }
    ...
    // HTTP configuration
    mio__data->bounce_uri = pstrdup(
        mio__data->p, xmlnode_get_data(xmlnode_get_list_item(
                          xmlnode_get_tags(io, "bounce", namespaces), 0)));
    mio__data->webserver_path =
        pstrdup(mio__data->p,
                xmlnode_get_data(xmlnode_get_list_item(
                    xmlnode_get_tags(io, "mini-webserver", namespaces), 0)));
    mio__data->flash_policy =
        pstrdup(mio__data->p,
                xmlnode_get_data(xmlnode_get_list_item(
                    xmlnode_get_tags(io, "flash-policy", namespaces), 0)));

    if (karma != NULL) {
        mio__data->k->val =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "init", namespaces), 0)),
                   KARMA_INIT);
        mio__data->k->max =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "max", namespaces), 0)),
                   KARMA_MAX);
        mio__data->k->inc =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "inc", namespaces), 0)),
                   KARMA_INC);
        mio__data->k->dec =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "dec", namespaces), 0)),
                   KARMA_DEC);
        mio__data->k->penalty =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "penalty", namespaces), 0)),
                   KARMA_PENALTY);
        mio__data->k->restore =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "restore", namespaces), 0)),
                   KARMA_RESTORE);
        mio__data->k->reset_meter =
            j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(karma, "resetmeter", namespaces), 0)),
                   KARMA_RESETMETER);
    }
    mio__data->rate_t = j_atoi(
        xmlnode_get_attrib_ns(
            xmlnode_get_list_item(xmlnode_get_tags(io, "rate", namespaces), 0),
            "time", NULL),
        0);
    mio__data->rate_p = j_atoi(
        xmlnode_get_attrib_ns(
            xmlnode_get_list_item(xmlnode_get_tags(io, "rate", namespaces), 0),
            "points", NULL),
        0);
 ...
 * @param arg the argument MIO should pass to the callback function, when
 * calling it
 ...
 *
 * You can only write the xmlnode OR the buffer to the mio. If the buffer
 * argument is not equal to NULL, the buffer is used and the xmlnode is ignored.
 *
 * @param m the ::mio to write the data to
 * @param stanza ::xmlnode containing the stanza, that should be written to a
 * stream (gets freed after the data has been written)
 * @param buffer pointer to a buffer of characters, that should be written to
 * the connection
 * @param len number of bytes contained in the buffer, that should be written
 * (-1 to write a zero terminated string contained in the buffer)
 */
void mio_write(mio m, xmlnode stanza, char const *buffer, int len) {
    ...
    if (stanza == NULL && buffer == NULL) {
        log_debug2("mio", LOGT_IO | LOGT_STRANGE,
                   "[%s] mio_write called without x or buffer", ZONE);
        return;
    }
    ...
    if (buffer != NULL) {
        ...
        /* XXX more hackish code to print the stream header right on a NUL
         * xmlnode socket */
        ...
        newwbq->data = xmlnode_serialize_string(
            stanza, m->out_ns ? *m->out_ns : xmppd::ns_decl_list(), 0);
        if (!newwbq->data) {
            pool_free(p);
            return;
        }
    ...
    log_debug2(ZONE, LOGT_IO, "mio_write called on stanza: %X buffer: %.*s",
               stanza, len, buffer);
 ...
 *
 * @param m the mio of the stream where to write the start tag for the root
 * element to
 * @param root the root element (freed by this function)
 * @param stream_type type of stream: 0 for 'jabber:server', 1 for
 * 'jabber:client', 2 for 'jabber:component:accept'
 */
void mio_write_root(mio m, xmlnode root, int stream_type) {
    ...
    // xstream_header_char() might have added declarations for the default
    // namespace as well as for the namespace defined by the db prefix
    char const *default_namespace =
        xmlnode_get_attrib_ns(root, "xmlns", NS_XMLNS);
    if (default_namespace) {
        // we do handle NS_CLIENT and NS_COMPONENT_ACCEPT as NS_SERVER
        // internally
 ...
 *
 * This function removes the last xmlnode from a write queue. This is normally
 * used to get unsent stanzas from the queue in case of an error happens and the
 * stanzas cannot be delivered.
 *
 * The returned xmlnodes have to be freed by the caller.
 *
 * Elements in the write queue, that have no xmlnode attached, but are just
 * character data are deleted by this function and the memory associated with
 * these items is freed by mio_cleanup().
 *
 * @param m the mio stream to get the last xmlnode stanza in the write queue
 * for.
 * @return last stanza item in the write queue (has to be freed by the caller),
 * or NULL if no further stanzas
 */
 ...
 * @param timeout how long to wait for a connection to be established (0 for
 * using the default value)
 * @param mh the ::mio_handlers used to select the desired type of socket (e.g.
 * an XML stream or a TLS protected socket)
 */
 ...
 */
mio mio_listen(int port, char const *listen_host, mio_std_cb cb, void *arg,
               mio_handlers mh) {
    mio newm;
    int fd;
    ...
    if (fd < 0) {
        log_alert(NULL, "mio unable to create socket on %d [%s]: %s", port,
    ...
    if (listen(fd, 10) < 0) {
        log_alert(NULL, "mio unable to listen on %d [%s]: %s", port,
                  listen_host, strerror(errno));
        return NULL;
    }
    ...
    log_debug2(ZONE, LOGT_IO, "mio starting to listen on %d [%s]", port,
               listen_host);
...
/**
 * create a ::mio_handlers instance, that can be passed to mio_listen() or
 * mio_accept()
 *
 ...
 *
 * If you are requesting a TLS protected socket (using ::MIO_SSL_READ and
 * ::MIO_SSL_WRITE), you also have to modify the accepted function in the
 * returned ::mio_handlers afterwards!
 *
 * @param rf handler used for reading, NULL for default (may be ::MIO_RAW_READ
 * or ::MIO_SSL_READ)
 * @param wf handler used for writing, NULL for default (may be ::MIO_RAW_WRITE
 * or ::MIO_SSL_WRITE)
 * @param pf handler used for parsing, NULL for default (may be ::MIO_RAW_PARSER
 * or ::MIO_XML_PARSER)
