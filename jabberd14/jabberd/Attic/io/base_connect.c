...
/* register a handler w/ the idnode asap, start a thread to connect, when it does it puts it in the connection storage struct */
...
result base_connect_config(idnode id, xmlnode x, void *arg)
{
    if(id == NULL)
    {
...
void base_connect(void)
{
    ...
    cfreg("connect",base_connect_config,NULL);
}