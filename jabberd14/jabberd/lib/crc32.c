 ...
 * c(x) = 1+x+x^2+x^4+x^5+x^7+x^8+x^10+x^11+x^12+x^16+x^22+x^23+x^26+x^32
 * on a given string.
 * We are initializing with zero and we are not inverting the result
 *
 * @param str the string for which we calculate the CRC
 * @param crc32buf where to put the result
 */
