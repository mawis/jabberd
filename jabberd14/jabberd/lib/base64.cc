 ...
 *
 * The function to decode base64 data is base64_decode(), the general encoding
 * function is base64_encode(). If the data that should get encoded is not
 * binary data, but zero terminated character data, the str_b64decode() function
 * can be used instead, which does not require the caller to specify the length
