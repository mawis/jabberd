 ...
 *
 * xstream is a way to have a consistent method of handling incoming XML stream
 * based events ... if doesn't handle the generation of an XML stream, but
 * provides some facilities to help doing that
 *
 * Currently this is only used by base_stdout.c and the stream used by dnsrv to
 * communicate with the co-process. Other XML streams (c2s, s2s, components,
 ...
 */
static void _xstream_startElement(void *_xs, const char *name,
                                  const char **atts) {
    ...
    // if we do not have a ns declaration list for the stanza yet create one by
    // copying the root's ns_decl_list
    if (!xs->ns_stanza) {
        xs->ns_stanza = xs->ns_root ? new xmppd::ns_decl_list(*xs->ns_root)
    ...
    if (xs->node == NULL) {
        ...
        xs->node = xmlnode_new_tag_pool_ns(p, local_name.c_str(),
                                           prefix == "" ? NULL : prefix.c_str(),
                                           ns_iri.c_str());
        ...
        if (xs->status == XSTREAM_ROOT) {
            ...
            // NS_COMPONENT_ACCEPT, or NS_DIALBACK has been declared => add
            ...
            try {
                std::string prefix =
                    xs->ns_root->get_nsprefix(NS_COMPONENT_ACCEPT);
                xmlnode_put_attrib_ns(xs->node,
            ...
            xs->status =
                XSTREAM_NODE; /* flag status that we're processing nodes now */
            (xs->f)(XSTREAM_ROOT, xs->node,
                    xs->arg); /* send the root, f must free all nodes */
            xs->node = NULL;
        }
    } else {
        xs->node = xmlnode_insert_tag_ns(xs->node, local_name.c_str(),
                                         prefix.c_str(), ns_iri.c_str());
        xmlnode_put_expat_attribs(xs->node, atts, *xs->ns_stanza);
    }
...
static void _xstream_endElement(void *_xs, const char *name) {
    ...
    /* if it's already NULL we've received </stream>, tell the app and we're
     * outta here */
    if (xs->node == NULL) {
        ...
        /* we are the top-most node, feed to the app who is responsible to
         * delete it */
        if (parent == NULL) {
 ...
 */
static void _xstream_startNamespaceDecl(void *arg, const XML_Char *prefix,
                                        const XML_Char *iri) {
    xstream xs = static_cast<xstream>(arg);

    // create a new ns_decl_list if necessary, and copy what namespaces we
...
/**
 * internal function to be registered as pool cleaner, frees a stream if the
 * associated memory pool is freed
 *
...
xstream xstream_new(pool p, xstream_onNode f, void *arg) {
    ...
    if (p == NULL || f == NULL) {
        fprintf(stderr, "Fatal Programming Error: xstream_new() was improperly "
                        "called with NULL.\n");
        return NULL;
    }
    ...
    XML_SetElementHandler(newx->parser, _xstream_startElement,
                          _xstream_endElement);
    XML_SetCharacterDataHandler(newx->parser, _xstream_charData);
    XML_SetNamespaceDeclHandler(newx->parser, _xstream_startNamespaceDecl,
                                _xstream_endNamespaceDecl);
...
int xstream_eat(xstream xs, char *buff, int len) {
    ...
    if (xs == NULL) {
        fprintf(stderr, "Fatal Programming Error: xstream_eat() was improperly "
                        "called with NULL.\n");
        return XSTREAM_ERR;
    }
...
xmlnode xstream_header(const char *to, const char *from) {
    ...
    snprintf(id, sizeof(id), "%08X%08X%08X%08X%08X", rand(), rand(), rand(),
             rand(), rand());
 ...
 *
 * @note this function does ignore most explicit declarations of namespace
 * prefixes. The only exceptions are explicit declarations of the default
 * namespace, or the namespace defined by the prefix 'db'
 *
 * @param x the xmlnode
 * @param stream_type 0 for 'jabber:server', 1 for 'jabber:client', 2 for
...
char *xstream_header_char(xmlnode x, int stream_type) {
    if (xmlnode_has_children(x)) {
        std::cerr << "Fatal programming error: xstream_header_char() was sent "
                     "a header with children!"
                  << std::endl;
        return NULL;
    }
    ...
    if (default_namespace) {
        if (stream_type && std::string(default_namespace) == NS_SERVER) {
            default_namespace =
                stream_type == 1
                    ? NS_CLIENT
                    : stream_type == 2 ? NS_COMPONENT_ACCEPT : NS_SERVER;
        }
...
void xstream_format_error(std::ostream &out, streamerr errstruct) {
    ...
    switch (errstruct->reason) {
        ...
        case bad_namespace_prefix:
            ...
            break;
        case conflict:
            out << "new stream has been initiated that confilicts with the "
                   "existing one";
            break;
        ...
        case internal_server_error:
            ...
            break;
        case invalid_from:
            out << "from address does not match an authorized JID or validated "
                   "domain";
            break;
        case invalid_id:
            out << "stream or dialback id is invalid or does not match a "
                   "previous "
                   "one";
            break;
        ...
        case unsupported_stanza_type:
            out << "sent a first-level child element (stanza) that is not "
                   "supported";
            break;
...
streamerr_severity xstream_parse_error(pool p, xmlnode errnode,
    ...
    for (cur = xmlnode_get_firstchild(errnode); cur != NULL;
