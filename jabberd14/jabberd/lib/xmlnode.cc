 ...
 *
 * @param p existing memory pool to use, if NULL a new memory pool will be
 * created
 * @param name local name of the element to be created (ignored for NTYPE_CDATA)
 * @param prefix namespace prefix of the element (NULL for no prefix, ignored
 * for NTYPE_CDATA)
 * @param ns_iri namespace IRI of the element (ignored for NTYPE_CDATA)
 * @param type type of the element to be created (NTYPE_CDATA, NTYPE_TAG,
 * NTYPE_ATTRIB)
 * @return the new xmlnode, NULL on failure
 */
static xmlnode _xmlnode_new(pool p, const char *name, const char *prefix,
                            const char *ns_iri, unsigned int type) {
    xmlnode result = NULL;
    if (type > NTYPE_LAST)
        return NULL;
 ...
 *
 * @note the sibling given as lastsibling has to be the last sibling in the list
 * of siblings
 *
 * @param lastsibling last sibling in a list of siblings, where the new sibling
 * should be added
...
static xmlnode _xmlnode_append_sibling(xmlnode lastsibling, const char *name,
    ...
    result =
        _xmlnode_new(xmlnode_pool(lastsibling), name, prefix, ns_iri, type);
    if (result != NULL) {
 ...
 *
 * The xmlnode may already contain child nodes, in that case the new xmlnode is
 * added as a sibling to the existing childs.
 *
...
static xmlnode _xmlnode_insert(xmlnode parent, const char *name,
    ...
    /* If parent->firstchild is NULL, simply create a new node for the first
     * child */
    if (parent->firstchild == NULL) {
        ...
        result = _xmlnode_append_sibling(parent->lastchild, name, prefix,
                                         ns_iri, type);
    }
 ...
 * @param ns_iri namespace IRI of the sibling to search for (NULL to ignore
 * namespaces, or on NTYPE_ATTRIB to match prefix-less attributes)
    ...
    for (current = firstsibling; current != NULL; current = current->next) {
        if ((current->type == type) &&
            (j_strcmp(current->name, name) == 0 ||
             (current->name == NULL && name == NULL)) &&
            (j_strcmp(current->ns_iri, ns_iri) == 0 ||
             (ns_iri == NULL &&
              (type != NTYPE_ATTRIB || ns_iri == current->ns_iri))))
            return current;
    }
 ...
 * @param ns_replace 0 for no namespace IRI replacing, 1 for replacing
 * 'jabber:server' ns with 'jabber:client', 2 for replacing 'jabber:server' ns
 * with 'jabber:component:accept'
 * @param ns_number the number of the namespace, that should be declared next if
 * needed
 */
static void _xmlnode_serialize(std::ostream &s, xmlnode_t const *x,
                               xmppd::ns_decl_list nslist, int ns_replace,
                               int ns_number = 0) {
    ...
    if (x->type == NTYPE_ATTRIB) {
        // do we have to print a prefix? (if yes: hopefully it is defined, else
        // we get an exception)
        if (x->ns_iri) {
        ...
        s << x->name << "='"
          << strescape(xmlnode_get_data(const_cast<xmlnode>(x))) << "'";
    ...
    // We use the default namespace for everything but NS_STREAM, and
    // NS_DIALBACK
    ...
    if (stream_ns) {
        ...
        if (!nslist.check_prefix("", x->ns_iri ? x->ns_iri : "")) {
            ...
            if (ns_replace && std::string(ns_iri) == NS_SERVER) {
                ns_iri = ns_replace == 1 ? NS_CLIENT
                                         : ns_replace == 2 ? NS_COMPONENT_ACCEPT
                                                           : NS_SERVER;
            }
    ...
    for (xmlnode_t const *cur = xmlnode_get_firstattrib_const(x); cur != NULL;
         cur = xmlnode_get_nextsibling_const(cur)) {
        // does the attribute have a namespace IRI?
        if (cur->ns_iri) {
            // attributes that are just namespace declarations are not
            // serialized, they are created as needed automatically
    ...
    for (xmlnode_t const *cur = xmlnode_get_firstchild_const(x); cur != NULL;
         cur = xmlnode_get_nextsibling_const(cur)) {
 ...
 *
 * Appends xmlnode node, if the predicate matches. If there is a next_step, not
 * the xmlnode is added itself, but the result of an xmlnode_get_tags()
 * recursion is added
 *
 * @param result_first pointer to the pointer to the first list element of the
 * result (gets modified)
 * @param result_last pointer to the pointer to the first list element of the
 * result (gets modified)
 * @param node the node, that should be appended if there is no next_step, or
 * which should be the parent node for the next_step
 * @param _predicate the predicate for this step
 * @param next_step the next step for recursion
 */
static void _xmlnode_append_if_predicate(xmlnode_vector &result_vector,
                                         xmlnode node, char const *_predicate,
    ...
    if (_predicate != NULL) {
        ...
        /* we only support checking for attribute existence or attribute values
         * for now */
        if (_predicate[0] != '@') {
        ...
        if (pos != std::string::npos) {
            attrib_ns_iri = static_cast<char *>(
                xhash_get(namespaces, attrib_name.substr(0, pos).c_str()));
            attrib_name.erase(0, pos + 1);
        }

        /* iterate over the namespace attributes */
        for (iter = xmlnode_get_firstattrib(node); iter != NULL;
            ...
            if (j_strcmp(attrib_ns_iri, iter->ns_iri) != 0 &&
                !(attrib_ns_iri == NULL && iter->ns_iri == NULL)) {
                continue;
            }

            /* we have to check the value and it differs */
            if (attrib_value_present &&
                j_strcmp(attrib_value.c_str(), xmlnode_get_data(iter)) != 0) {
                continue;
            }
 ...
 * create a tag node
 *
 * @deprecated This function is not aware of namespaces, use
 * xmlnode_new_tag_ns() instead
 *
 ...
 *
 * @deprecated This function is not aware of namespaces, use
 * xmlnode_new_tag_pool_ns() instead
 *
...
xmlnode xmlnode_new_tag_pool_ns(pool p, const char *name, const char *prefix,
    ...
    /* 'jabber:client' and 'jabber:component:accept' are represented as
     * 'jabber:server' internally */
...
xmlnode xmlnode_insert_tag(xmlnode parent, const char *name) {
    ...
    result =
        _xmlnode_insert(parent, local_name, NULL, parent->ns_iri, NTYPE_TAG);
    if (result != NULL && local_name > name) {
        result->prefix = static_cast<char *>(
            pmalloco(xmlnode_pool(result), local_name - name));
 ...
 * @param size size of the string in CDATA, or -1 for auto-detection on
 ...
 *
 * @deprecated This function is not aware of namespaces, use xmlnode_get_tags(),
 * and xmlnode_get_list_item() instead
 *
 * @param parent pointer to the parent tag
 * @param name "name" for the child tag of that name, "name/name" for a sub
 * child (recurses), "?attrib" to match the first tag with that attrib defined,
 * "?attrib=value" to match the first tag with that attrib and value, "=cdata"
 * to match the text node contents of the child, or any combination:
 * "name/name/?attrib", "name=cdata", etc
 * @return a pointer to the tag matching search criteria, or NULL if search was
 * unsuccessfull
 */
xmlnode xmlnode_get_tag(xmlnode parent, const char *name) {
    ...
    if (equals != NULL && (slash == NULL || equals < slash) &&
        (qmark == NULL || equals < qmark)) {
        /* of type =cdata */
    ...
    if (qmark != NULL && (slash == NULL || qmark < slash)) {
        ...
        *qmark = '\0';
        ...
        for (step = parent->firstchild; step != NULL;
            ...
            if (equals != NULL &&
                j_strcmp(xmlnode_get_attrib(step, qmark), equals) != 0)
                continue;
 ...
 *
 * The only predicates we support is for existence of attributes, or for
 * attribute values, we only support steps in the axis child and the axis must
 * be ommited, we support text() as a step.
 *
 * Examples:
 * - foo/bar/text()
 ...
 * @return first item in the list of xmlnodes, or NULL if no xmlnode matched the
 * path
 */
xmlnode_vector xmlnode_get_tags(xmlnode context_node, const char *_path,
                                xht namespaces) {
    ...
    /* separate this step from the next one, and check for a predicate in this
     * step */
    ...
    if (start_predicate == std::string::npos &&
        start_next_step == std::string::npos) {
        // there is neither a predicate nor a next step in the path
        this_step = path;
    } else if (start_predicate == std::string::npos ||
               (start_next_step != std::string::npos &&
                start_predicate > start_next_step)) {
        ...
        predicate = path.substr(start_predicate + 1,
                                end_predicate - start_predicate - 1);
    ...
    if (end_prefix == std::string::npos) {
        ...
        ns_iri = axis == 2
                     ? NULL
                     : static_cast<const char *>(xhash_get(namespaces, ""));
    } else {
        ns_iri = static_cast<const char *>(
            xhash_get(namespaces, this_step.substr(0, end_prefix).c_str()));
        this_step.erase(0, end_prefix + 1);
    }

    /* iterate over all child nodes, checking if this step matches them */
    for (iter = axis == 0
                    ? xmlnode_get_firstchild(context_node)
                    : axis == 1
                          ? xmlnode_get_parent(context_node)
                          : axis == 2 ? xmlnode_get_firstattrib(context_node)
                                      : NULL;
         iter != NULL;
         iter = axis == 0
                    ? xmlnode_get_nextsibling(iter)
                    : axis == 1
                          ? NULL
                          : axis == 2 ? xmlnode_get_nextsibling(iter) : NULL) {
        if (this_step == "*") {
            /* matching all nodes */
            ...
            _xmlnode_append_if_predicate(
                result_vector, iter,
                predicate.length() > 0 ? predicate.c_str() : NULL,
                next_step.length() > 0 ? next_step.c_str() : NULL, namespaces);
        ...
        if (iter->type != NTYPE_CDATA &&
            ((ns_iri == NULL && iter->ns_iri == NULL) ||
             j_strcmp(ns_iri, iter->ns_iri) == 0) &&
            j_strcmp(this_step.c_str(), iter->name) == 0) {
 ...
 *
 * @deprecated This function is not aware of namespaces, use xmlnode_get_tags(),
 * and xmlnode_get_list_item_data() instead
 *
 * @param parent the element where to search for the element defined by the name
 * parameter
 * @param name search query for the element whichs textual content should be
 * returned
...
void xmlnode_change_namespace(xmlnode node, const char *ns_iri) {
    ...
    if (node->prefix == NULL) {
        ...
        if (xmlnode_get_attrib_ns(node, node->prefix, NS_XMLNS) != NULL)
            xmlnode_put_attrib_ns(node, node->prefix, "xmlns", NS_XMLNS,
                                  ns_iri);
    }
}
/**
...
void xmlnode_put_attrib_ns(xmlnode owner, const char *name, const char *prefix,
    ...
    if (owner->firstattrib == NULL) {
        ...
        attrib =
            _xmlnode_search(owner->firstattrib, name, ns_iri, NTYPE_ATTRIB);
        if (attrib == NULL) {
            attrib = _xmlnode_append_sibling(owner->lastattrib, name, prefix,
                                             ns_iri, NTYPE_ATTRIB);
            owner->lastattrib = attrib;
        } else {
        }
 ...
 * @param ns_iri namespace IRI of the attribute of which the value should be
 * returned
 * @return value of the attribute, or NULL if no such attribute
 */
 ...
 *
 * iteration on all attributes is possible by using xmlnode_get_nextsibling()
 * using the result of this function as the start
 *
 ...
 *
 * iteration on all childs is possible using xmlnode_get_nextsibling() using the
 * result of this function as the start
 *
 ...
 *
 * (backwards) iteration on all childs is possible using
 * xmlnode_get_prevsibling() using the result of this function as the start
 *
 ...
 *
 * this can be used together with xmlnode_get_firstchild() to iterate over the
 * childrens of a node
 *
...
static xmlnode_t const *
xmlnode_get_nextsibling_const(xmlnode_t const *sibling) {
 ...
 *
 * this can be used together with xmlnode_get_lastchild() to iterate backwards
 * over the childrens of a node
 *
 ...
 *
 * @deprecated This function mimics the jabberd 1.4.x xmlnode_get_name() where
 * the name including the prefix is returned. You probably do not want to use
 * this in a namespace aware application. You might want to use
 * xmlnode_get_localname(), and xmlnode_get_namespace() instead.
 *
 ...
 * namespace (xmlnode_get_namespace()) and the localname
 * (xmlnode_get_localname())
 *
...
char *xmlnode_get_data(xmlnode node) {
    if (xmlnode_get_type(node) ==
        NTYPE_TAG) /* loop till we find a CDATA in the children */
 ...
 * @return type of the node, one of ::NTYPE_TAG, ::NTYPE_ATTRIB, or
 * ::NTYPE_CDATA
 */
 ...
 *
 * @deprecated This function is not aware of namespaces. Use
 * xmlnode_hide_attrib_ns() instead.
 *
 ...
 */
void xmlnode_hide_attrib_ns(xmlnode parent, const char *name,
                            const char *ns_iri) {
    xmlnode attrib;
 ...
 *
 * This function can be used to serialize a stanza. As a stanza is typically
 * written to an XML stream, there might be namespaces, that are already
 * declared by the stream root element. These namespaces do not need to be
 * declared again, when the stanza is serialized. This is what nslist_first and
 * nslist_last is for. These two pointers point to the begin and end of a list
 * of already declared namespace prefixes. If all namespaces should be declared,
 * than you can just pass NULL as these two arguments.
 *
 * Do not pass the 'jabber:client', or 'jabber:component:accept' namespace in
 * the list of already declared namespaces, but pass always the namespace
 * 'jabber:server' instead. This is because xmlnode represents all these
 * namespaces as 'jabber:server'.
 *
 * @note while xmlnode_serialize_string() is running, the list passed by the
 * arguments nslist_first and nslist_last gets expanded, but when returning, the
 * list is restored. Keep this in mind in a multithreading environment.
 *
 * @note Internally xmlnode converts the namespaces 'jabber:client' and
 * 'jabber:component:accept' to the namespace 'jabber:server'. This is done to
 * be able to easily serialize the same stanza on client streams, server
 * streams, as well as streams where components are connected. When serializing
 * the xmlnode, the namespace 'jabber:server' is serialized as the right
 * namespace. This is controled by the stream_type argument.
 *
 * @param node the base xmlnode of the tree, that should be serialized
 * @param nslist list of already declared namespaces
 * @param stream_type 0 for a 'jabber:server' stream, 1 for a 'jabber:client'
 * stream, 2 for a 'jabber:component:accept' stream
...
xmlnode xmlnode_insert_tag_node(xmlnode parent, xmlnode node) {
    ...
    child =
        xmlnode_insert_tag_ns(parent, node->name, node->prefix, node->ns_iri);
...
void xmlnode_insert_node(xmlnode parent, xmlnode node) {
    ...
    while (node != NULL) {
        switch (xmlnode_get_type(node)) {
            case NTYPE_ATTRIB:
                xmlnode_put_attrib_ns(parent, node->name, node->prefix,
                                      node->ns_iri, xmlnode_get_data(node));
                break;
            case NTYPE_TAG:
 ...
 *
 * @note this is nearly the same as xmlnode_dup(), with the difference, that you
 * can specify which memory pool to use
 *
 * @param p memory pool to use
 ...
 *
 * this function creates a new element with the name specified as the wrapper
 * parameter, the xmlnode x becomes a child of this new element
 *
 * @deprecated This function is not aware of namespaces. Use xmlnode_wrap_ns()
 * instead.
 *
...
xmlnode xmlnode_wrap(xmlnode x, const char *wrapper) {
    ...
    if (local_name > wrapper) {
        result->prefix =
            static_cast<char *>(pmalloco(result->p, local_name - wrapper));
 ...
 *
 * No nodes inside the xmlnode tree can be used afterwards, as they are all
 * freed by this function
 *
 * @param node one of the elements inside a tree of xmlnodes that should be
 * freed
 */
 ...
 * @param lang language to prefere (if NULL, the first node without language is
 * prefered)
...
xmlnode xmlnode_select_by_lang(const xmlnode_vector &nodes, const char *lang) {
    ...
    for (iter = nodes.begin(); iter != nodes.end(); ++iter) {
        ...
        if (first_with_general_lang == NULL && this_nodes_lang != NULL &&
            !general_lang.empty() && general_lang == this_nodes_lang) {
    ...
    /* no match by language, either return node without language (prefered), or
     * first node */
...
ns_decl_list::ns_decl_list(const xmlnode node) {
    ...
    for (xmlnode iter = xmlnode_get_firstattrib(node); iter != NULL;
         iter = xmlnode_get_nextsibling(iter)) {
 ...
 */
void ns_decl_list::update(const std::string &prefix,
 ...
 *
 * @param iri the namespace IRI to search for
 * @param accept_default_prefix if set to false, the method will only search for
 * a non-default prefix
 * @return the latest prefix, that is bould to this namespace IRI
 * @throws std::invalid_argument if namespace is not declared
 */
char const *ns_decl_list::get_nsprefix(const std::string &iri,
                                       bool accept_default_prefix) const {
    ns_decl_list::const_reverse_iterator p;
    // iterate on all list items backwards
    for (p = rbegin(); p != rend(); ++p) {
        // is it the IRI we are looking for? if it's the default prefix, do we
        // accept it? isn't the prefix redeclared differently?
        if (p->second == iri && (accept_default_prefix || p->first != "") &&
            check_prefix(p->first, iri)) {
 ...
 * @return the namespace IRI the prefix is bound to, NULL if the prefix is not
 * declared
 ...
 */
bool ns_decl_list::check_prefix(const std::string &prefix,
...
void xmlnode_stat() {
    ...
    log_notice(own_pid,
               "%i xmlnodes in %i pools / biggest used pool: %i, xmlnodes in "
               "this pool: %i",
               number_of_xmlnodes, number_of_pools, biggest_pool,
               number_of_xmlnodes_in_biggest_pool);
    ...
    for (p = existing_xmlnodes.begin(); p != existing_xmlnodes.end(); ++p) {
        if (pool_size(p->first) == biggest_pool) {
            ...
            for (p2 = p->second.begin(); p2 != p->second.end(); ++p2) {
                log_notice(own_pid, "root element: %s in namespace %s",
                           xmlnode_get_localname(*p2),
                           xmlnode_get_namespace(*p2));
            }
        }
    }
}
