 ...
 *
 * This file implements the code needed to access lwresd (light-weight resolving daemon) used to do asyncronous DNS resolving)
 */
...
namespace xmppd {
    namespace lwresc {
	invalid_packet::invalid_packet(const std::string& __arg) : std::logic_error(__arg) {
	};
	...
	void lwquery::write_header(std::ostream& os, uint32_t opcode, size_t rdata_len) const {
	    write_uint32(os, 4+2+2+4+4+4+4+2+2+rdata_len);	// packet length
	...
	uint32_t lwquery::getSerial() const {
	    return serial;
	}
	...
	std::string lwresult::read_qname(std::istream& is) {
		...
		for (unsigned char bytes_read = 0; bytes_read < static_cast<unsigned char>(label_length); bytes_read++) {
		    char one_byte;
		    if (!is.get(one_byte)) {
			std::ostringstream exception_message;
			exception_message << "Could not read a qname while reading a character " << static_cast<unsigned>(bytes_read) << "/" << static_cast<unsigned>(label_length);
	...
	lwresult::lwresult(std::istream& is) : rdata(NULL) {
		...
		char one_byte;
		if (!is.get(one_byte)) {
		    // try to unread - may not work in many cases, caller has to check state of is after catching the exception
		    for (uint32_t i = 0; i < bytes_read; i++) {
			is.unget();
		    }
		    std::ostringstream exception_message;
		    exception_message << "Packet not yet fully available: Expecting " << length << " B / got " << bytes_read << " B";
		    throw std::runtime_error(exception_message.str());
		}
		buffer.put(one_byte);
	    }
		...
		switch (opcode) {
			...
			break;
		    default:
			for (uint32_t bytes_read = 0; bytes_read<rdata_length; bytes_read++) {
			    if (!buffer.get())
				throw std::runtime_error("Not enough data in buffer");
			}
		}
		...
		throw invalid_packet(std::string("Received corrupted packet: ")+re.what());
	    }
	}
	...
	uint32_t lwresult::getSerial() const {
	    return serial;
	}
	...
	lwresult_rdata const* lwresult::getRData() const {
	    return rdata;
	}
	...
	uint32_t lwresult_rrset::getTTL() const {
	    return ttl;
	}

	std::vector<rrecord *> lwresult_rrset::getRR() const {
	    return rr;
	}
	...
	lwresult_rrset::lwresult_rrset(std::istream& is, uint32_t rdata_len) {
		...
		try {
			...
			*p = NULL;
		    ...
		    std::ostringstream exception_message;
		    exception_message << "Error while creating record " << (rr_read+1) << "/" << number_rr << ": " << re.what();
		    throw std::runtime_error(exception_message.str());
		}
	...
	srv_record::srv_record(std::istream& is) {
		...
		throw std::runtime_error(std::string("Error reading qname: ")+re.what());
	    }
	}

	uint16_t srv_record::getPrio() const {
	    return prio;
	}

	uint16_t srv_record::getWeight() const {
	    return weight;
	}

	uint16_t srv_record::getPort() const {
	    return port;
	}

	const std::string& srv_record::getDName() const {
	    return dname;
	}

	const std::string& aaaa_record::getAddress() const {
	    return address;
	}
	...
	const std::string& a_record::getAddress() const {
	    return address;
	}
