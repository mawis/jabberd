 ...
 *
 * @param userdata pointer to the parent xmlnode instance (NULL if this function
 * is called for the root note)
...
void expat_startElement(void *userdata, const char *name, const char **atts) {
    ...
    expat_callback_data callback_data =
        static_cast<expat_callback_data>(userdata);
    ...
    if (xmlns_separator_pos != Glib::ustring::npos) {
        // expat found the namespace IRI for us (this should be the case for a
        // correct stream)
        ...
        if (colon_pos != Glib::ustring::npos) {
            ...
            // again: ... be liberal in what you accept ... (guessing it's
            // 'jabber:server')
            ns_iri = NS_SERVER;
            local_name = qname;
        }
    ...
    if (callback_data->x == NULL) {
        ...
        callback_data->x = xmlnode_new_tag_ns(
            local_name.c_str(), prefix.empty() ? NULL : prefix.c_str(),
            ns_iri.c_str());
    } else {
        /* insert as child node */
        callback_data->x = xmlnode_insert_tag_ns(
            callback_data->x, local_name.c_str(),
            prefix.empty() ? NULL : prefix.c_str(), ns_iri.c_str());
    }
...
/**
 * create an xmlnode instance (possibly including other xmlnode instances) by
 * parsing a string
 *
 * This function will parse a string containing an XML document and create an
 * xmlnode graph
 *
 * @param str the string containing the XML document (not necessarily zero
 * terminated)
 * @param len the length of the string (without the zero byte, if present)
 * @return the graph of xmlnodes that represent the parsed document, NULL on
 * failure
 */
xmlnode xmlnode_str(const char *str, int len) {
    ...
    XML_SetNamespaceDeclHandler(p, expat_startNamespaceDecl,
                                expat_endNamespaceDecl);
 ...
 * parsing a file
 *
 * This function will parse a file containing an XML document and create an
 * xmlnode graph
 *
 ...
 *
 * This function can be used to get a textual message why parsing an XML file
 * failed.
 *
 * @param file the filename
...
char const *xmlnode_file_borked(char const *file) {
    ...
    while (!done) {
        ...
        if (!XML_Parse(p, buf, len, done)) {
            snprintf(err, sizeof(err), "%s at line %ld and column %ld",
                     XML_ErrorString(XML_GetErrorCode(p)),
                     XML_GetErrorLineNumber(p), XML_GetErrorColumnNumber(p));
            XML_ParserFree(p);
            close(fd);
            return err;
        }
...
int xmlnode2file_limited(char const *file, xmlnode node, size_t sizelimit) {
    ...
    /* is it to big? (23 is the size of the XML declaration and the trailing
     * newline in the file) */
 ...
 * @param atts the attributes in expat format (even indexes are the attribute
 * names, odd indexes the values)
...
void xmlnode_put_expat_attribs(xmlnode owner, const char **atts,
    ...
    for (; atts[i] != NULL; i += 2) {
        ...
        // get prefix, iri, and local name of the element
        Glib::ustring::size_type xmlns_separator_pos =
            qname.find(XMLNS_SEPARATOR);
        if (xmlns_separator_pos != Glib::ustring::npos) {
            // expat found the namespace IRI for us (this should be the case for
            // a correct stream)
            ns_iri = qname.substr(0, xmlns_separator_pos);
            local_name = qname.substr(xmlns_separator_pos + 1);
            ...
            if (colon_pos != Glib::ustring::npos) {
                ...
                // some well known prefixes (but they would have to be
                // declared!)
                if (prefix == "stream") {
                    ns_iri = NS_STREAM;
                } else if (prefix == "db") {
        ...
        xmlnode_put_attrib_ns(
            owner, local_name.c_str(), prefix.empty() ? NULL : prefix.c_str(),
            ns_iri.empty() ? NULL : ns_iri.c_str(), attribute_value.c_str());
    }
}
