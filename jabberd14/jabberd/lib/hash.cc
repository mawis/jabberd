 ...
 *
 * This file currently only implements the SHA-1 hashing algorithm. The
...
void sha1::update(const std::string &data) {
    ...
    if (padded) {
        throw std::domain_error(
            "Updating the SHA-1 is not possible after it has been calculated.");
    }
    ...
    for (size_t i = 0; i < characters; i++) {
        ...
        // move the content of the relevant W[] 8 bits to the left so that we
        // have all at the right bit in this W[] after adding 4 bytes
        W[W_pos / 4] <<= 8;
...
uint32_t sha1::f_20_39(uint32_t B, uint32_t C, uint32_t D) { return B ^ C ^ D; }
...
std::vector<uint8_t> sha1::final() {
    ...
    if (!padded) {
        ...
        // pad with a single bit set first ... this is always the character 0x80
        // as we are only adding octects
        update("\x80");

        // pad with that many 0 bits, that we can just add the message length
        // and get a complete block
