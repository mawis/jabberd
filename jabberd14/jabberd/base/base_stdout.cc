 ...
 * (STDOUT) from the jabberd process, it also flags a thread to read on STDIN
 * for incoming packets - DEPRICATED
 *
 * @deprecated using this handler is depricated, it will be removed from future
 * versions of jabberd14
 */
...
static void base_stdin_packets(int type, xmlnode x, void *arg) {
    switch (type) {
        case XSTREAM_ROOT:
            ...
            break;
        case XSTREAM_NODE:
            /* deliver the packets coming on stdin... they aren't associated
             * with an instance really */
...
static result base_stdout_config(instance id, xmlnode x, void *arg) {
    ...
    log_debug2(ZONE, LOGT_INIT | LOGT_THREAD,
               "base_stdout_config performing configuration");
