 ...
 * @brief base module base_importspool: reads a filespool and sets all contained
 * data using the configured xdb handler.
 *
 * This module is used internally by jabberd14 if you request it to import data
 * from an existing filespool.
 */
...
typedef struct base_importspool_struct {
    ...
    xht std_namespace_prefixes; /**< namespace prefixes used by this base
                                   handler */
 ...
 *
 * @param steps_done how many steps of the expected number of steps have been
 * done
 * @param needed_steps how many steps are expected to be needed
 * @param yield_counter used to check when progress should be updated and when
 * we are cooperative
 */
static void print_progress(int steps_done, int needed_steps, int *yield_counter,
                           time_t importstart) {
    int bar_done = 0;
    int i = 0;
 ...
 *
 */
static void import_file(instance i, xdbcache xc, xht std_namespace_prefixes,
                        const char *filename, const char *domain,
                        const char *user) {
    ...
    if (userid == NULL || !userid->has_node()) {
        log_debug2(ZONE, LOGT_IO, "invalid user: %s@%s - skipping", user,
                   domain);
    ...
    for (data_element = xmlnode_get_firstchild(spoolfile); data_element != NULL;
        ...
        if (j_strcmp(ns, NS_LAST) == 0 || j_strcmp(ns, NS_AUTH) == 0 ||
            j_strcmp(ns, NS_JABBERD_STOREDPRESENCE) == 0 ||
            j_strcmp(ns, NS_REGISTER) == 0 || j_strcmp(ns, NS_ROSTER) == 0 ||
            j_strcmp(ns, NS_BROWSE) == 0 || j_strcmp(ns, NS_VCARD) == 0) {
        ...
        if (j_strcmp(ns, NS_JABBERD_STOREDREQUEST) == 0) {
            ...
            for (request = xmlnode_get_firstchild(data_element);
                ...
                xpath << "presence[@from='"
                      << xmlnode_get_attrib_ns(request, "from", NULL) << "']";
                xdb_act_path(xc, userid, NS_JABBERD_STOREDREQUEST, "insert",
                             xpath.str().c_str(), std_namespace_prefixes,
                             request);
            }
            continue;
        }
        ...
        if (j_strcmp(ns, NS_PRIVATE) == 0) {
            ...
            for (item = xmlnode_get_firstchild(data_element); item != NULL;
                ...
                xpath << "private:query[@jabberd:ns='"
                      << xmlnode_get_namespace(item) << "']";
                xdb_act_path(xc, userid, NS_PRIVATE, "insert",
                             xpath.str().c_str(), std_namespace_prefixes, item);
            }
            continue;
        }
        ...
        if (j_strcmp(ns, NS_PRIVACY) == 0) {
            ...
            for (list = xmlnode_get_firstchild(data_element); list != NULL;
                ...
                xpath << "privacy:list[@name='"
                      << xmlnode_get_attrib_ns(list, "name", NULL) << "']";
                xdb_act_path(xc, userid, NS_PRIVACY, "insert",
                             xpath.str().c_str(), std_namespace_prefixes, list);
            }
            continue;
        }
        ...
        std::ostringstream xpath;
        xpath << "private:query[@jabberd:ns='"
              << xmlnode_get_namespace(data_element) << "']";
        xdb_act_path(xc, userid, NS_PRIVATE, "insert", xpath.str().c_str(),
                     std_namespace_prefixes, data_element);
    }
...
static void *base_importspool_worker(void *arg) {
    ...
    if (basedir == NULL) {
        printf(
            "Could not open this directory: %s\nNot importing anything ...\n",
            strerror(errno));
        return NULL;
    }
    ...
    while ((basedir_entry = readdir(basedir))) {
        ...
        snprintf(cur_domaindir, sizeof(cur_domaindir), "%s/%s",
                 conf_data->importspool, basedir_entry->d_name);
        ...
        while ((domaindir_entry = readdir(domaindir))) {
            ...
            if (strlen(domaindir_entry->d_name) == 2) {
                ...
                if (!is_hexdigit(domaindir_entry->d_name[0]) ||
                    !is_hexdigit(domaindir_entry->d_name[1]))
                    continue;

                /* open the first subdir of the domaindir */
                snprintf(cur_domainsubdir1, sizeof(cur_domainsubdir1), "%s/%s",
                         cur_domaindir, domaindir_entry->d_name);
                domainsubdir1 = opendir(cur_domainsubdir1);
                if (domainsubdir1 == NULL)
                    continue;

                /* iterate the entries in the first subdirectory of a domain
                 * directory (should be second level subdirs) */
                while ((domainsubdir1_entry = readdir(domainsubdir1))) {
                    ...
                    if (!is_hexdigit(domainsubdir1_entry->d_name[0]) ||
                        !is_hexdigit(domainsubdir1_entry->d_name[1]) ||
                        domainsubdir1_entry->d_name[2] != 0)
                        continue;

                    /* open the second subdir of the domaindir */
                    snprintf(cur_domainsubdir2, sizeof(cur_domainsubdir2),
                             "%s/%s", cur_domainsubdir1,
                             domainsubdir1_entry->d_name);
                    domainsubdir2 = opendir(cur_domainsubdir2);
                    if (domainsubdir2 == NULL)
                        continue;

                    /* iterate the entries in the second subdirectory of a
                     * domain directory (should contain the spool files) */
                    while ((domainsubdir2_entry = readdir(domainsubdir2))) {
                        if (strlen(domainsubdir2_entry->d_name) <= 4)
                            continue;

                        if (strcmp(domainsubdir2_entry->d_name +
                                       strlen(domainsubdir2_entry->d_name) - 4,
                                   ".xml") == 0) {
                            char filename[2048];
                            char user[1028];
                            ...
                            snprintf(filename, sizeof(filename), "%s/%s",
                                     cur_domainsubdir2,
                                     domainsubdir2_entry->d_name);
                            snprintf(user, sizeof(user), "%s",
                                     domainsubdir2_entry->d_name);
                            user[strlen(user) - 4] = 0;
                            import_file(conf_data->id, conf_data->xc,
                                        conf_data->std_namespace_prefixes,
                                        filename, basedir_entry->d_name, user);

                            /* update output */
                            print_progress(steps_done, needed_steps,
                                           &yield_counter,
                                           conf_data->importstart);
                        }
                    }
            ...
            if (strlen(domaindir_entry->d_name) > 4) {
                if (strcmp(domaindir_entry->d_name +
                               strlen(domaindir_entry->d_name) - 4,
                           ".xml") == 0) {
                    char filename[2048];
                    char user[1028];

                    /* import this file */
                    snprintf(filename, sizeof(filename), "%s/%s", cur_domaindir,
                             domaindir_entry->d_name);
                    snprintf(user, sizeof(user), "%s", domaindir_entry->d_name);
                    user[strlen(user) - 4] = 0;
                    import_file(conf_data->id, conf_data->xc,
                                conf_data->std_namespace_prefixes, filename,
                                basedir_entry->d_name, user);

                    /* update output */
                    print_progress(steps_done, needed_steps, &yield_counter,
                                   conf_data->importstart);
                }
            }
        }
...
static result base_importspool_config(instance id, xmlnode x, void *arg) {
    ...
    log_debug2(ZONE, LOGT_INIT | LOGT_CONFIG,
               "base_importspool configuring instance %s", id->id);
    ...
    xhash_put(conf_data->std_namespace_prefixes, "",
              const_cast<char *>(NS_SERVER));
    xhash_put(conf_data->std_namespace_prefixes, "private",
              const_cast<char *>(NS_PRIVATE));
    xhash_put(conf_data->std_namespace_prefixes, "jabberd",
              const_cast<char *>(NS_JABBERD_WRAPPER));
    xhash_put(conf_data->std_namespace_prefixes, "privacy",
...
/**
 * load the base_importspool base module by registering a configuration handler
 * for &lt;importspool/&gt;
 *
