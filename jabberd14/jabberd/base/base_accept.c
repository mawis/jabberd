...
void base_accept_process_xml(mio m, int state, void* arg, xmlnode x)
{
    ...
    switch(state)
    {
        case MIO_XML_ROOT:
        {
            ...
            /* The default argument for this callback is an accept_listener struct, inherited from the parent MIO object */
            ...
            /* Search the acceptor hosts node for a host which matches the "to" attribute sent in the stream header */
            ...
            aio->io       = m;
            aio->instance = ai;
            mio_reset(m, (void*)base_accept_process_xml, (void*)aio);
        ...
        case MIO_XML_NODE:
        {
            ...
            if(j_strcmp(xmlnode_get_name(x), "handshake") == 0)
            {
                ...
                if(j_strcmp(hashbuf, xmlnode_get_data(x)) == 0)
                {
                    ...
                    mio_write(aio->io, NULL, "<handshake/>", -1);
                    ...
                    if(aio->state == A_DUPLEX)
                    {
                        ...
                        we need to "take over" the instance and deal with it's queue directly */
                        ...
                        while((entry = (accept_instance_queue) pth_msgport_get(aio->instance->write_queue)) != NULL)
        ...
        case MIO_CLOSED:
        {
            ...
            make sure that the instance is notified that the AIO object is leaving */
...
result base_accept_config(instance id, xmlnode x, void *arg)
{
    ...
    if(id == NULL)
    {
        ...
        if ((xmlnode_get_tag(x, "ip") == NULL) || (xmlnode_get_tag(x, "port") == NULL) || (xmlnode_get_tag(x,"secret") == NULL))
        {			
