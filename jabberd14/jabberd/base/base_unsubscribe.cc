 ...
 * @brief base module base_unsubscribe: bounces messages and iqs, on receiving
 * presences it sends unsubscribes
 *
 ...
 * telling about delivery failures, the admin can automatically create
 * unsubscribes to remove the contacts from the users' rosters and create
 * bounces for other stanzas.
 */
 ...
 * @param replied_stanza the stanza where sender and recipient address can be
 * get from
 * @param type of reply to generate, either "unsubscribe" or "unsubscribed"
 */
static void base_unsubscribe_bounce_presence(instance id,
                                             xmlnode replied_stanza,
                                             const char *type) {
    xmlnode reply = NULL;
...
static result base_unsubscribe_deliver(instance id, dpacket p, void *arg) {
    ...
    switch (packet_to_handle->type) {
        ...
        case JPACKET_IQ:
            deliver_fail(
                p,
                arg ? static_cast<char *>(arg)
                    : messages_get(
                          xmlnode_get_lang(p->x),
                          N_("Destination blocked by server administrator")));
            return r_DONE;
        ...
        case JPACKET_S10N:
            ...
            if (j_strcmp(xmlnode_get_attrib_ns(p->x, "type", NULL),
                         "subscribe") == 0) {
                base_unsubscribe_bounce_presence(id, p->x, "unsubscribed");
            }
            break;
...
static result base_unsubscribe_config(instance id, xmlnode x, void *arg) {
    ...
    log_debug2(ZONE, LOGT_INIT | LOGT_CONFIG,
               "base_unsubscribe configuring instance %s", id->id);

    if (id->type != p_NORM) {
        log_alert(NULL,
                  "ERROR in instance %s: <unsubscribe>..</unsubscribe> element "
                  "only allowed in service sections",
                  id->id);
        return r_ERR;
    }

    register_phandler(id, o_DELIVER, base_unsubscribe_deliver,
...
/**
 * load the base_unsubscribe base module by registering a configuration handler
 * for &lt;unsubscribe/&gt;
 *
