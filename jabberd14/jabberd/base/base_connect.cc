 ...
 *
 * This base handler implements connecting to another jabberd or other server
 * instance using the component protocol defined in XEP-0114.
 */
...
typedef struct {
    ...
    pth_msgport_t
        write_queue; /* Queue of write_buf packets which need to be written */
 ...
 */
static void base_connect_send_routingupdate_peer(conn_info ci, char const *host,
                                                 bool is_register) {
...
static result base_connect_deliver(instance i, dpacket p, void *arg) {
    ...
    /* Insert the message into the write_queue if we don't have an MIO socket
     * yet.. */
    if (ci->state != conn_AUTHD) {
        conn_write_buf entry = static_cast<conn_write_buf>(
            pmalloco(p->p, sizeof(_conn_write_buf)));
        entry->packet = p;
        ...
        if (ci->dplast == p) {
            ...
            // unregister being responsible for this domain in local router and
            // remote
...
static void base_connect_connect(void *arg) {
    ...
    mio_connect(ci->hostip, ci->hostport, base_connect_process_xml, ci,
                ci->timeout, mio_handlers_new(NULL, NULL, MIO_XML_PARSER));
}
...
static void base_connect_process_xml(mio m, int state, void *arg, xmlnode x,
    ...
    switch (state) {
        case MIO_NEW:
            ...
            log_debug2(ZONE, LOGT_IO, "base_connecting: %X, %X, %s", ci,
                       ci->inst, ci->inst->id);
        ...
        case MIO_XML_NODE:
            ...
            if (ci->state == conn_AUTHD) {
                ci->dplast =
                    dpacket_new(x); /* store the addr of the dpacket we're
                                       sending to detect circular delevieries */
                deliver(ci->dplast, ci->inst);
                ci->dplast = NULL;
                return;
            }

            /* If a handshake packet is recv'd from the server, we have
             * successfully auth'd -- go ahead and update the connection state
             */
            if (j_strcmp(xmlnode_get_localname(x), "handshake") == 0 &&
                ...
                conn_write_buf b;
                while ((b = (conn_write_buf)pth_msgport_get(ci->write_queue)) !=
                       NULL)
                    mio_write(ci->io, b->packet->x, NULL, 0);
                /* Update connection state flag */
                ci->state = conn_AUTHD;

                // if we are configured as uplink, request the routings to the
                // other instances of this process from our peer process
                if (deliver_is_uplink(ci->inst)) {
                    std::set<Glib::ustring> hosts_to_route =
                        deliver_routed_hosts(p_NORM, ci->inst);

                    for (std::set<Glib::ustring>::const_iterator p =
                             hosts_to_route.begin();
                         p != hosts_to_route.end(); ++p) {
                        log_debug2(ZONE, LOGT_DYNAMIC,
                                   "base_connect is uplink. Sending routing "
                                   "request: %s",
                                   p->c_str());
                        ...
                        xmlnode_put_attrib_ns(route_stanza, "from", NULL, NULL,
                                              ci->inst->id);
                        jid magic_jid = jid_new(xmlnode_pool(route_stanza),
                                                "host@-internal");
        ...
        case MIO_CLOSED:
            ...
            if (ci->tries_left == 0) {
                fprintf(
                    stderr,
                    "Base Connect Failed: service %s was unable to connect to "
                    "%s:%d, unrecoverable error, exiting",
            ...
            log_debug2(
                ZONE, LOGT_IO,
                "Base Connect Failed to connect to %s:%d Retry [%d] in 2 "
                "seconds...",
 ...
 */
static void base_connect_routingupdate(instance i, char const *destination,
    ...
    log_debug2(ZONE, LOGT_DYNAMIC,
               "base_connect is uplink and has to forward %s",
...
static result base_connect_config(instance id, xmlnode x, void *arg) {
    ...
    timeout = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                     ...
                     5);
    tries = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                       xmlnode_get_tags(x, "tries", namespaces), 0)),
                   -1);
    ...
    if (id == NULL) {
        ...
        if (port == 0 || (secret == NULL)) {
            xmlnode_put_attrib_ns(x, "error", NULL, NULL,
                                  "<connect> requires the following subtags: "
                                  "<port>, and <secret>");
            return r_ERR;
        }
        return r_PASS;
    ...
    register_routing_update_callback(NULL, base_connect_routingupdate,
                                     static_cast<void *>(ci));
 ...
 *
 * @param p memory pool used to register the handler for the &lt;connect/&gt;
 * configuration element (must be available for the livetime of jabberd)
 */
