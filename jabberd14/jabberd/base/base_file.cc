...
static result base_file_config(instance id, xmlnode x, void *arg) {
    ...
    if (id == NULL) {
        log_debug2(ZONE, LOGT_INIT | LOGT_CONFIG,
                   "base_file_config validating configuration");

        if (xmlnode_get_data(x) == NULL) {
            log_debug2(ZONE, LOGT_STRANGE | LOGT_CONFIG | LOGT_INIT,
    ...
    log_debug2(ZONE, LOGT_INIT | LOGT_CONFIG,
               "base_file configuring instance %s", id->id);

    if (id->type != p_LOG) {
        log_alert(NULL,
                  "ERROR in instance %s: <file>..</file> element only allowed "
                  "in log sections",
                  id->id);
        return r_ERR;
    }
    ...
    if (filehandle == NULL) {
        log_alert(NULL, "base_file_config error: error opening file (%d): %s",
