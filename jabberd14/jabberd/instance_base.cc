...
namespace xmppd {
instance_base::instance_base(instance i, xmlnode x)
    : i(i), current_heartbeat_frequency(0), requested_heartbeat_frequency(0) {
...
result instance_base::on_packet(dpacket dp) {
    ...
    switch (dp->type) {
        ...
        default:
            deliver_fail(
                dp, N_("This component does not know the type of packet."));
            return r_DONE;
    }
}
...
result instance_base::beathandler_wrapper() {
    ...
    if (current_heartbeat_frequency != requested_heartbeat_frequency) {
        ...
        if (requested_heartbeat_frequency > 0) {
            ::register_beat(requested_heartbeat_frequency,
                            instance_base::beathandler_helper, this);
        }
...
void instance_base::deliver(dpacket p) { ::deliver(p, i); }

void instance_base::deliver(xmlnode x) { ::deliver(::dpacket_new(x), i); }
...
logmessage instance_base::log(loglevel level) { return logger->level(level); }

xmlnode instance_base::get_instance_config() {
    // search for the first child element of the service element in a different
    // namespace
    for (xmlnode x = xmlnode_get_firstchild(i->x); x;
