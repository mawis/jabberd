...
#define MAX_INCLUDE_NESTING                                                    \
    20 /**< the maximum number of nexted &lt;jabberd:include/&gt; elements in  \
          the configuration */
xht instance__ids =
    NULL; /**< hash of all created XML routing target instances (key is the id
             of the instance, value is the ::instance) */
...
/**
 * check the parsed configuration file for include instructions, process these
 * instructions, and check again
 *
 * @param nesting_lefel nesting level of includes, used to stop recursion if
 * nesting_level is bigger than ::MAX_INCLUDE_NESTING
    ...
    for (; cur != NULL;) {
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "include") == 0 &&
            ...
            if (nesting_level > MAX_INCLUDE_NESTING) {
                std::cerr << "ERROR: Included files nested " << nesting_level
                          << " levels deep. Possible recursion!" << std::endl;
                exit(1);
            }
            include = cur;
            ...
            if (j_strcmp(xmlnode_get_localname(xmlnode_get_parent(cur)),
                ...
                if (j_strcmp(xmlnode_get_localname(xmlnode_get_parent(cur)),
                             xmlnode_get_localname(include_x)) == 0) {
                    xmlnode example_root_element =
                        xmlnode_dup(xmlnode_get_parent(cur));

                    while (xmlnode_get_firstchild(example_root_element) != NULL)
                        xmlnode_hide(
                            xmlnode_get_firstchild(example_root_element));

                    std::cerr << "WARNING (while including file '"
                              << include_file << "'):" << std::endl;
                    std::cerr
                        << "Local name (" << xmlnode_get_localname(include_x)
                        << ") of the included file's root element matches the"
                        << std::endl;
                    std::cerr << "parent element, but namespaces are "
                                 "different. This means the two elements"
                              << std::endl;
                    std::cerr << "are different and are handled as different "
                                 "elements. You might want this,"
                              << std::endl;
                    std::cerr << "and in that case you can just ignore this "
                                 "warning. But in most cases this"
                              << std::endl;
                    std::cerr << "is a configuration bug, and not what the "
                                 "writer of the configuration file"
                              << std::endl;
                    std::cerr << "intends. In that case you might want to "
                                 "update the root element of the"
                              << std::endl;
                    std::cerr
                        << "included file to declare the correct namespace."
                        << std::endl
                        << std::endl;
                    std::cerr
                        << "Currently the namespace of the parent element is: "
                        << xmlnode_get_namespace(xmlnode_get_parent(cur))
                        << std::endl;
                    std::cerr
                        << "and the namespace of the included root element is: "
                        << xmlnode_get_namespace(include_x) << std::endl;
                    std::cerr << "Waht you probably want is the following root "
                                 "element in the included file:"
                              << std::endl;
                    std::cerr
                        << xmlnode_serialize_string(example_root_element,
...
/**
 * replace &lt;jabberd:cmdline/&gt; elements in the configuration file with
...
static void _set_configured_debug(xmlnode x) {
    ...
    if (facility_str != NULL) {
        ...
        if (facility == -1) {
            log_alert(
                NULL,
                "debugging configuration error: unknown syslog facility: %s",
                facility);
        }
    } else {
    ...
    if (fd < 0) {
        if (errno == EEXIST) {
            ...
            if (fd < 0) {
                std::cerr
                    << "The pidfile " << path
                    << " already exists, and it cannot be opened for reading ("
            ...
            if (bytesread < 0) {
                std::cerr << "The pidfile " << path
                          << " already exists, but there is a problem reading "
                             "its content ("
                          << strerror(errno) << "). Exiting ..." << std::endl;
                _exit(1);
            } else if (bytesread == 0) {
                std::cerr
                    << "The pidfile " << path
                    << " already exists, but it has no content. Deleting it ..."
                    << std::endl;
            } else {
                pid_t filepid = 0;
                int killres = 0;
                ...
                if (filepid == 0) {
                    std::cerr << "The pidfile " << path
                              << " already exists, but does not contain a PID ("
                              << oldpid << "). Exiting ..." << std::endl;
                    _exit(1);
                }

                killres = kill(filepid, 0);
                if (killres == -1 && errno == ESRCH) {
                    std::cerr << "Stale pidfile " << path
                              << " found. No process with PID " << filepid
                              << " is running. Deleting pidfile ..."
                              << std::endl;
                } else {
                    std::cerr << "A pidfile already exists at " << path
                              << ", containing the PID (" << filepid
                              << ") of a running process. Exiting ..."
                              << std::endl;
                    _exit(1);
                }
            }
            ...
            fd = open(path, O_CREAT | O_EXCL | O_WRONLY, 0600);
            if (fd < 0) {
                std::cerr << "Still having problems accessing pidfile " << path
                          << ": " << strerror(errno) << std::endl;
                _exit(1);
            }
        } else {
            std::cerr << "Not writing pidfile " << path << ": "
                      << strerror(errno) << std::endl;
            return;
        }
    }
 ...
 * @param is_restart 0 if it is the initial configuration processing, 1 if it is
 * a restart
...
int configurate(char *file, xht cmd_line, int is_restart) {
    ...
    if ((realfile = static_cast<char *>(xhash_get(cmd_line, "i"))) != NULL) {
        while (realfile != NULL) {
            ...
            if ((incl = xmlnode_file(realfile)) == NULL) {
                fprintf(stderr,
                        "Configuration parsing included file %s failed: %s\n",
    ...
    if (import_spool != NULL) {
        ...
        service = xmlnode_insert_tag_ns(greymatter__, "service", NULL,
                                        NS_JABBERD_CONFIGFILE);
        xmlnode_put_attrib_ns(service, "id", NULL, NULL,
                              "spoolimporter.localhost");
        importspool = xmlnode_insert_tag_ns(service, "importspool", NULL,
                                            NS_JABBERD_CONFIGFILE);
    ...
    if (!is_restart) {
        ...
        locale =
            xmlnode_get_tags(greymatter__, "global/locales/locale", namespaces);
        xhash_free(namespaces);
        namespaces = NULL;

        xmlnode_vector::iterator iter;
        for (iter = locale.begin(); iter != locale.end(); ++iter) {
            messages_set_mapping(xmlnode_get_attrib_ns(*iter, "lang", NULL),
                                 xmlnode_get_attrib_ns(*iter, "locale", NULL));
        }
    }
...
static cfg cfget(char *node) {
    ...
    for (next = cfhandlers__; next != NULL && strcmp(node, next->node) != 0;
         next = next->next)
        ;
...
static void instance_cleanup_module_init_funcs(void *arg) {
    std::map<std::string, void *> *module_init_funcs =
        static_cast<std::map<std::string, void *> *>(arg);
...
static void instance_cleanup_static_hosts(void *arg) {
    std::set<Glib::ustring> *static_hosts =
        static_cast<std::set<Glib::ustring> *>(arg);
    delete static_hosts;
}

/**
 * handle a second-level configuration file element (beside the &lt;base/&gt;
 * element)
 *
...
static int instance_startup(xmlnode x, int exec) {
    ...
    if (type == p_NONE || xmlnode_get_attrib(x, "id") == NULL ||
        ...
        std::cerr << "Configuration error in:" << std::endl
                  << xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0)
                  << std::endl;
        if (type == p_NONE) {
            std::cerr << "ERROR: Invalid tag type: " << xmlnode_get_name(x)
                      << std::endl;
        }
    ...
    if (exec == 1) {
        newi = static_cast<instance_struct *>(
            xhash_get(instance__ids, xmlnode_get_attrib(x, "id")));
        if (newi != NULL) {
            std::cerr << "ERROR: Multiple instances with same id: "
                      << xmlnode_get_attrib_ns(x, "id", NULL) << std::endl;
            return -1;
        }
    }
    ...
    if (exec) {
        ...
        pool_cleanup(newi->p, instance_cleanup_module_init_funcs,
                     newi->module_init_funcs);
        newi->static_hosts = new std::set<Glib::ustring>();
        pool_cleanup(newi->p, instance_cleanup_static_hosts,
                     newi->static_hosts);
        /* make sure the id is valid for a hostname */
        temp = jid_new(p, newi->id);
        if (temp == NULL ||
            j_strcmp(temp->get_domain().c_str(), newi->id) != 0) {
    ...
    for (cur = xmlnode_get_firstchild(x); cur != NULL;
        ...
        /* if we don't have a handler, but we do have a namespace, we can just
         * be ignored */
        ...
        if (c == NULL || (c->f)(newi, cur, c->arg) == r_ERR) {
            char *error =
                pstrdup(xmlnode_pool(cur), xmlnode_get_attrib(cur, "error"));
            xmlnode_hide_attrib(cur, "error");
            std::cerr << "Invalid configuration in instance '"
                      << xmlnode_get_attrib_ns(x, "id", NULL) << ":"
                      << std::endl;
            std::cerr << xmlnode_serialize_string(cur, xmppd::ns_decl_list(), 0)
                      << std::endl;
            if (c == NULL)
                std::cerr << "ERROR: Unknown base tag: "
                          << xmlnode_get_name(cur) << std::endl;
            else if (error != NULL)
                std::cerr << "ERROR: Base handler returned an error:"
                          << std::endl
                          << error << std::endl;
            return -1;
        }
