    ...
    if (user == NULL) {
        ...
        if (!js_mapi_call(si, e_AUTH, p, user, NULL)) {
            if (jpacket_subtype(p) == JPACKET__GET) {
                /* if it's a type="get" for auth, everybody mods it and we
                 * result and return it */
                xmlnode_insert_tag_ns(
                    p->iq, "resource", NULL,
                    NS_AUTH); /* of course, resource is required :) */
                xmlnode_put_attrib_ns(p->x, "type", NULL, NULL, "result");
                jutil_tofrom(p->x);
            } else {
                /* type="set" that didn't get handled used to be a problem, but
                 * now auth_plain passes on failed checks so it might be normal
                 */
                jutil_error_xmpp(p->x, XTERROR_AUTH);
            }
        }
...
void _js_authreg_register(jpacket p) {
    ...
    if (jpacket_subtype(p) == JPACKET__GET) {
        ...
        if (!js_mapi_call(si, e_REGISTER, p, NULL, NULL)) {
            ...
            xmlnode_put_attrib_ns(p->x, "type", NULL, NULL, "result");
            jutil_tofrom(p->x);
            if (xmlnode_get_tags(p->iq, "register:username",
        ...
        if (!p->to->has_node() ||
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(p->iq, "register:password", namespaces), 0)) ==
                NULL) {
            log_debug2(ZONE, LOGT_AUTH,
                       "registration set request without a password ...");
            jutil_error_xmpp(p->x, XTERROR_NOTACCEPTABLE);
        } else if (js_user(si, p->to, NULL) != NULL) {
            jutil_error_xmpp(p->x, (xterror){409, N_("Username Not Available"),
                                             "cancel", "conflict"});
        } else {
            /* check if this account is blocked for registration as this account
             * already exited */
            xmlnode regtimeout_config = js_config(si, "jsm:regtimeout", NULL);
            int regtimeout = j_atoi(
                xmlnode_get_attrib_ns(regtimeout_config, "timeout", NULL),
                365 * 86400 / 2);
            xmlnode_free(regtimeout_config);
            regtimeout_config = NULL;

            /* if regtimeout is configured as 0, accounts can be reregistered
             * immediatelly */
            if (regtimeout != 0) {
                xmlnode last = NULL;

                last = xdb_get(si->xc, jid_user(p->to), NS_LAST);
                /* if last is NULL, no previous account existed. Further tests
                 * only if there is last data */
                if (last != NULL) {
                    time_t now = time(NULL);
                    int lasttime =
                        j_atoi(xmlnode_get_attrib_ns(last, "last", NULL), 0);

                    /* if regtimeout is set to -1, unregistered accounts are
                     * blocked forever ... */
                    if (regtimeout == -1 || now < (lasttime + regtimeout)) {
                        jutil_error_xmpp(
                            p->x, (xterror){409, N_("Username Not Available"),
                                            "cancel", "conflict"});
                        return;
                    }
                }
 ...
 *
 * If jsm is not configured to let an external component handle authentication
 * and registration of new users, it will let this function handle these both
 * jobs
 *
...
void js_authreg(void *arg) {
    ...
    if (p->to->has_node() &&
        (jpacket_subtype(p) == JPACKET__GET || p->to->has_resource()) &&
        NSCHECK(p->iq, NS_AUTH)) {
    ...
    xmlnode_put_attrib_ns(x, "type", NULL, NULL,
                          xmlnode_get_attrib_ns(p->x, "route", NULL));
    /* hide our uglies */
