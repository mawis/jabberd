 ...
 * session manager - DEPRICATED
 *
 * This module implements the handling of jabber:iq:browse to query client
 * information in the session manager. jabber:iq:browse is documented in
 * XEP-0011. Browsing is DEPRICATED, clients should use service discovery
 * instead.
 */

/**
 * Generate the browse result for a user by loading the information from xdb or
 * creating it if there is nothing stored yet.
 *
 * @param m the mapi structure
 * @param id the JID of the user for which the browse info should be build
 * @return the xml fragment containing the browse result (must be freed by the
 * caller)
 */
        ...
        if (!id->has_resource()) {
            ...
            if ((x = xdb_get(m->si->xc, m->user->id, NS_VCARD)) != NULL)
                xmlnode_put_attrib_ns(
                    browse, "name", NULL, NULL,
                    xmlnode_get_data(xmlnode_get_list_item(
                        xmlnode_get_tags(x, "vcard:FN",
                ...
                xmlnode_put_attrib_ns(
                    browse, "name", NULL, NULL,
                    xmlnode_get_data(xmlnode_get_list_item(
                        xmlnode_get_tags(x, "register:name",
                                         m->si->std_namespace_prefixes),
                        0)));
            xmlnode_free(x);
        } else {
 ...
 *
 * This callback is only inserted in the es_OUT handler list, therefore it is
 * only called for stanzas the user sents itself. It is NOT called for stanzas
 * arriving at the user's address. It only handles stanzas with NO to attribute.
 *
 * Handling iq stanzas of type 'set' is not documented in XEP-0011. I guess, it
 * is an undocumented extension in jabberd14.
 *
 * @param m the mapi structure containing the request
 * @param arg unused/ignored
 * @return M_IGNORE if it is no iq stanza, M_PASS if the packet has not been
 * processed, M_HANDLED if the packet has been processed
 */
static mreturn mod_browse_set(mapi m, void *arg) {
    ...
    log_debug2(
        ZONE, LOGT_DELIVER, "handling set request %s",
        xmlnode_serialize_string(m->packet->iq, xmppd::ns_decl_list(), 0));
    ...
    /* if we set to a resource, we need to make sure that resource's browse is
     * in the users browse */
    if (to->has_resource()) {
        browse = mod_browse_get(m, to); /* get our browse info */
        xmlnode_hide_attrib_ns(browse, "xmlns",
                               NS_XMLNS); /* don't need a ns as a child */
        for (cur = xmlnode_get_firstchild(browse); cur != NULL;
             cur = xmlnode_get_nextsibling(cur))
            xmlnode_hide(cur); /* erase all children */
        std::ostringstream xpath;
        xpath << "*[@jid='" << jid_full(to) << "']";
        xdb_act_path(m->si->xc, m->user->id, NS_BROWSE, "insert",
                     xpath.str().c_str(), m->si->std_namespace_prefixes,
                     browse); /* insert and match replace */
        xmlnode_free(browse);
    }
    ...
    xmlnode_hide_attrib_ns(
        cur, "xmlns", NS_XMLNS); /* just in case, to make sure it inserts */
    std::ostringstream xpath;
    xpath << "*[@jid='" << jid_full(id) << "']";
    if (xdb_act_path(m->si->xc, to, NS_BROWSE, "insert", xpath.str().c_str(),
                     m->si->std_namespace_prefixes, cur)) {
        js_bounce_xmpp(m->si, m->s, m->packet->x, XTERROR_UNAVAIL);
        return M_HANDLED;
    }

    /* if the new data we're inserting is to one of our resources, update that
     * resource's browse */
    if (jid_cmpx(m->user->id, id, JID_USER | JID_SERVER) == 0 &&
        ...
        xdb_set(m->si->xc, id, NS_BROWSE,
                cur); /* replace the resource's browse w/ this one */
        xmlnode_free(browse);
    }
...
/**
 * register a callback for stanzas the user sends (to be able to handle queries
 ...
 * Everything but iq stanzas are ignored, iq queries not in the jabber:iq:browse
 * namespace are not handled. Only iq stanzas of type 'get' are really
 * processed, stanzas of type 'set' are denied, other types are ignored.
 *
 * The result is build using the user's stored browse info. If the user that
 * sent the query is subscribed to the target's presence, a list of active
 * sessions of the users is added to the result as well.
 *
 * @param m the mapi instance containing the query
 * @param arg not used/ignored
 * @return M_IGNORE if the packet is no iq stanza, M_PASS if the stanza has not
 * been processed, M_HANDLED if the request has been handled
 */
static mreturn mod_browse_reply(mapi m, void *arg) {
    ...
    log_debug2(ZONE, LOGT_DELIVER, "handling query for user %s",
    ...
    for (cur = xmlnode_get_firstchild(ns); cur != NULL;
        ...
        if (xmlnode_get_attrib_ns(cur, "type", NULL) == NULL)
            xmlnode_insert_tag_node(
                browse, cur); /* only include the generic <ns>foo</ns> */
    xmlnode_free(ns);
    ...
    if (js_trust(m->user, m->packet->from)) {
        for (s = m->user->sessions; s != NULL; s = s->next) {
            ...
            if (xmlnode_get_list_item(
                    xmlnode_get_tags(browse, xpath.str().c_str(),
                                     m->si->std_namespace_prefixes),
                    0) != NULL)
 ...
 * Everything but iq stanzas are ignored. The iq stanzas have to be of type
 * 'get', the content must be in the jabber:iq:browse namespace and there must
 * be no resource, else the stanza won't be handled either.
 *
 * The result will be build using the content of the <browse/> element in the
 * session manager configuration. The the user that sent the query as read admin
 * privileges, and advertizement for serverdomain/admin will be sent as well.
 * (Queries to this address are handled by mod_admin.c.)
 *
 * @param m the mapi structure containing the request
 * @return M_IGNORE if the stanza is no iq, M_PASS if this module is not
 * responsible, M_HANDLED if the stanza has been processed
 */
static mreturn _mod_browse_server(mapi m) {
    ...
    if ((browse = js_config(m->si, "browse:browse",
    ...
    query = xmlnode_insert_tag_ns(jutil_iqresult(m->packet->x), "service", NULL,
                                  NS_BROWSE);
    xmlnode_put_attrib_ns(query, "type", NULL, NULL, "jabber");
    xmlnode_put_attrib_ns(query, "jid", NULL, NULL,
                          m->packet->to->get_domain().c_str());
    xmlnode_put_attrib_ns(
        query, "name", NULL, NULL,
        xmlnode_get_data(vcard_fn)); /* pull name from the server vCard */

    /* copy in the configured services */
    for (x = xmlnode_get_firstchild(browse); x != NULL;
 ...
 * registers three callbacks: if a session is started, mod_browse_session should
 * be called, if an offline user gets a stanza, mod_browse_reply should be
 * called, if a stanza is sent to the server, mod_browse_server should be
 * called.
 *
