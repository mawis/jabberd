 ...
 *
 * Check if the username is allowed to be registered. Generate an error reply if
 * not.
 *
 * @param m the mapi structure
 * @param arg unused/ignored
 * @return M_PASS if registration is not allowed, M_IGNORE if the module is not
 * configured, M_HANDLED else
 */
static mreturn mod_useridpolicy_new(mapi m, void *arg) {
    ...
    username = xmlnode_get_data(xmlnode_get_list_item(
        xmlnode_get_tags(m->packet->iq, "register:username",
                         m->si->std_namespace_prefixes),
        0));

    /* if there is no username, we don't care, mod_register will probably reject
     * it */
    if (username == NULL)
        return M_PASS;
    ...
    user_jid =
        jid_new(m->packet->p, "invalid"); /* for 'invalid' as domain see RFC
                                             2606, we just need any domain */
    ...
    for (x = xmlnode_get_firstchild(config); x != NULL;
        ...
        if (j_strcmp(xmlnode_get_data(x), username) == 0) {
            log_notice(m->packet->to->get_domain().c_str(),
                       "blocked account '%s' from being registered: forbidden "
                       "username",
                       username);
    ...
    if (j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                   xmlnode_get_tags(config, "jsm:minlen",
                                    m->si->std_namespace_prefixes),
                   0)),
               1) > (int)username_len) {
        log_notice(
            m->packet->to->get_domain().c_str(),
            "blocked account '%s' from being registered: username to short",
            username);
        jutil_error_xmpp(m->packet->x, XTERROR_NOTACCEPTABLE);
        xmlnode_free(config);
        return M_HANDLED;
    ...
    if (j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                   xmlnode_get_tags(config, "jsm:maxlen",
                                    m->si->std_namespace_prefixes),
                   0)),
               1023) < (int)username_len) {
        log_notice(
            m->packet->to->get_domain().c_str(),
            "blocked account '%s' from being registered: username to long",
            username);
        jutil_error_xmpp(m->packet->x, XTERROR_NOTACCEPTABLE);
        xmlnode_free(config);
        return M_HANDLED;
