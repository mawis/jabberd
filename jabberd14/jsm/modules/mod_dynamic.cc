 ...
 * But the chances made by this module are not presistant. To get them
 * persistant, you have to add them as well to the configuration file, so the
 * new hosts get (not) configured after a restart. (At least for now.)
 */
...
static mreturn mod_dynamic_server_disco_items(mapi m) {
    ...
    if (j_strcmp(node, NS_COMMAND) != 0 && j_strcmp(node, "host") != 0 &&
        j_strcmp(node, "unhost") != 0) {
        return M_PASS;
    }

    jutil_iqresult(m->packet->x);
    m->packet->iq =
        xmlnode_insert_tag_ns(m->packet->x, "query", NULL, NS_DISCO_ITEMS);
    xmlnode_put_attrib_ns(m->packet->iq, "node", NULL, NULL, NS_COMMAND);

    if (acl_check_access(m->si->xc, ADMIN_DYNAMIC, m->packet->from) &&
        j_strcmp(node, NS_COMMAND) == 0) {
        xmlnode item =
            xmlnode_insert_tag_ns(m->packet->iq, "item", NULL, NS_DISCO_ITEMS);
        ...
        xmlnode_put_attrib_ns(
            item, "name", NULL, NULL,
            messages_get(xmlnode_get_lang(m->packet->x), N_("Add host")));
        ...
        xmlnode_put_attrib_ns(
            item, "name", NULL, NULL,
            messages_get(xmlnode_get_lang(m->packet->x), N_("Remove host")));
    }
...
static mreturn mod_dynamic_server_disco_info(mapi m) {
    ...
    if (j_strcmp(disco_node, NS_COMMAND) == 0) {
        jutil_iqresult(m->packet->x);
        m->packet->iq =
            xmlnode_insert_tag_ns(m->packet->x, "query", NULL, NS_DISCO_INFO);

        xmlnode identity = xmlnode_insert_tag_ns(m->packet->iq, "identity",
                                                 NULL, NS_DISCO_INFO);
        xmlnode_put_attrib_ns(
            identity, "name", NULL, NULL,
            messages_get(xmlnode_get_lang(m->packet->x), N_("Commands")));
    ...
    if (j_strcmp(disco_node, "host") == 0) {
        ...
        xmlnode feature = xmlnode_insert_tag_ns(m->packet->iq, "feature", NULL,
                                                NS_DISCO_INFO);
        xmlnode_put_attrib_ns(feature, "var", NULL, NULL, NS_COMMAND);
        feature = xmlnode_insert_tag_ns(m->packet->iq, "feature", NULL,
                                        NS_DISCO_INFO);
    ...
    feature = xmlnode_insert_tag_ns(m->additional_result->iq, "feature", NULL,
                                    NS_DISCO_INFO);
    xmlnode_put_attrib_ns(feature, "var", NULL, NULL,
                          NS_COMMAND); // XXX check that feature has not already
                                       // been added by other module
...
static mreturn mod_dynamic_server_command(mapi m, jid config_jid) {
    ...
    enum our_nodes { host, unhost } given_command;
    ...
    xmlnode_vector form = xmlnode_get_tags(m->packet->iq, "data:x",
    ...
    if (form.empty()) {
        ...
        jutil_iqresult(m->packet->x);
        m->packet->iq =
            xmlnode_insert_tag_ns(m->packet->x, "command", NULL, NS_COMMAND);
        ...
        switch (given_command) {
            case host:
                form_title =
                    xmlnode_insert_tag_ns(form, "title", NULL, NS_DATA);
                xmlnode_insert_cdata(
                    ...
                    -1);
                form_instructions =
                    xmlnode_insert_tag_ns(form, "instructions", NULL, NS_DATA);
                xmlnode_insert_cdata(
                    form_instructions,
                    messages_get(xmlnode_get_lang(m->packet->x),
                                 N_("Please insert the hostname, that should "
                                    "get added.")),
                    -1);
                form_field =
                    xmlnode_insert_tag_ns(form, "field", NULL, NS_DATA);
                xmlnode_put_attrib_ns(form_field, "type", NULL, NULL,
                                      "text-single");
                xmlnode_put_attrib_ns(
                    form_field, "label", NULL, NULL,
                    messages_get(xmlnode_get_lang(m->packet->x),
                                 N_("Hostname")));
    ...
    if (j_strcmp(type, "submit") == 0) {
        ...
        xmlnode_vector hostname_node = xmlnode_get_tags(
            form[0], "data:field[@var='host']/data:value/text()",
            m->si->std_namespace_prefixes);
        if (!hostname_node.empty()) {
            ...
            if (hostname) {
                ...
                jutil_iqresult(m->packet->x);
                m->packet->iq = xmlnode_insert_tag_ns(m->packet->x, "command",
                                                      NULL, NS_COMMAND);
                xmlnode_put_attrib_ns(m->packet->iq, "status", NULL, NULL,
                                      "completed");
                xmlnode_put_attrib_ns(m->packet->iq, "node", NULL, NULL, node);

                xmlnode note = xmlnode_insert_tag_ns(m->packet->iq, "note",
                                                     NULL, NS_COMMAND);
                ...
                if (!preped_hostname) {
                    ...
                    xmlnode_insert_cdata(
                        note,
                        messages_get(xmlnode_get_lang(m->packet->x),
                                     N_("The entered hostname is invalid.")),
                        -1);
                } else if (j_strcmp(preped_hostname->get_domain().c_str(),
                                    m->si->i->id) == 0) {
                    xmlnode_put_attrib_ns(note, "type", NULL, NULL, "error");
                    xmlnode_insert_cdata(
                        note,
                        messages_get(xmlnode_get_lang(m->packet->x),
                                     N_("The main hostname of a session "
                    ...
                    replace_path << "dhost:host[@jid='"
                                 << preped_hostname->get_domain() << "']";
                    ...
                    switch (given_command) {
                        case host:
                            xmlnode_insert_cdata(
                                note,
                                messages_get(xmlnode_get_lang(m->packet->x),
                                             N_("Hostname has been added.")),
                                -1);
                            log_debug2(ZONE, LOGT_DYNAMIC,
                                       "registering hostname %s on server %s",
                                       preped_hostname->get_domain().c_str(),
                                       m->si->i->id);
                            register_instance(
                                m->si->i,
                                preped_hostname->get_domain().c_str());
                            ...
                            // store in xdb
                            newhost = xmlnode_new_tag_ns(
                                "host", NULL, NS_JABBERD_CONFIG_DYNAMICHOST);
                            xmlnode_put_attrib_ns(
                                newhost, "jid", NULL, NULL,
                                preped_hostname->get_domain().c_str());
                            xdb_act_path(m->si->xc, config_jid,
                                         NS_JABBERD_CONFIG_DYNAMICHOST,
                                         "insert", replace_path.str().c_str(),
                                         m->si->std_namespace_prefixes,
                                         newhost);
                            xmlnode_free(newhost);
                            break;
                        case unhost:
                            // XXX kick existing sessions and remove local data
                            // about this domain
                            xmlnode_insert_cdata(
                                note,
                                messages_get(xmlnode_get_lang(m->packet->x),
                                             N_("Hostname has been removed.")),
                                -1);
                            log_debug2(ZONE, LOGT_DYNAMIC,
                                       "unregistering hostname %s on server %s",
                                       preped_hostname->get_domain().c_str(),
                                       m->si->i->id);
                            unregister_instance(
                                m->si->i,
...
extern "C" void mod_dynamic(jsmi si) {
    ...
    // get additional hostnames of the session manager, that have been
    // dynamically added
    xmlnode dynhosts =
        xdb_get(si->xc, config_jid, NS_JABBERD_CONFIG_DYNAMICHOST);
    for (xmlnode iter = xmlnode_get_firstchild(dynhosts); iter;
         iter = xmlnode_get_nextsibling(iter)) {
        // skip what we do not care about
        if (xmlnode_get_type(iter) != NTYPE_TAG) {
            continue;
        }
        if (j_strcmp(xmlnode_get_namespace(iter),
                     NS_JABBERD_CONFIG_DYNAMICHOST)) {
            continue;
        }
        ...
        if (!dynhost) {
            log_notice(
                si->i->id, "ignoring empty dynamic host in configuration: %s",
                xmlnode_serialize_string(iter, xmppd::ns_decl_list(), 0));
            continue;
        }
