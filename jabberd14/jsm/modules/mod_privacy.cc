...
struct mod_privacy_compiled_list_item {
    ...
    int match_subscription; /**< 0: no match by subscription / 1: match 'none' /
                               3: match 'to' / 5: match 'from' / 7: match 'both'
                             */
    int do_deny; /**< 0 if the action is 'allow', 1 if the action is 'deny' */
    long int order; /**< order of this item, used when compiling the list; the
                       list gets ordered */
    struct mod_privacy_compiled_list_item
        *next; /**< pointer to the next list item (with a higher or the same
                  order) */
};
 ...
 *
 * @todo it would be nice if we could accept any name. But currently our xpath
 * implementation does not allow us to use some characters.
 *
 ...
 */
static int
mod_privacy_denied(const struct mod_privacy_compiled_list_item *privacy_list,
                   const udata user, const jid id) {
    /* sanity check */
    if (privacy_list == NULL || user == NULL || id == NULL)
        return 0;

    log_debug2(ZONE, LOGT_EXECFLOW, "mod_privacy_denied() check for %s",
               jid_full(id));
    ...
    for (; privacy_list != NULL; privacy_list = privacy_list->next) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "list item: jid=%s, parts=%i, subscription=%i, action=%s",
                   jid_full(privacy_list->match_jid), privacy_list->match_parts,
                   privacy_list->match_subscription,
                   privacy_list->do_deny ? "deny" : "allow");

        /* check if the JID matches */
        if (privacy_list->match_jid &&
            jid_cmpx(privacy_list->match_jid, id, privacy_list->match_parts) !=
                0) {
            /* no match */
        ...
        if (privacy_list->match_subscription) {
            ...
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "subscription tests, match when: %i = %i / %i = %i",
                       id_from, list_from, id_to, list_to);

            if (id_from && !list_from) /* no match */
                continue;
            if (!id_from && list_from) /* no match */
                continue;
            if (id_to && !list_to) /* no match */
                continue;
            if (!id_to && list_to) /* no match */
                continue;
        ...
        log_debug2(ZONE, LOGT_EXECFLOW, "Explicit result: %s",
                   privacy_list->do_deny ? "deny" : "accept");
 ...
 */
static void mod_privacy_send_notify_new_list(udata user,
                                             const char *edited_list) {
    session cur = NULL;
    ...
    for (cur = user->sessions; cur != NULL; cur = cur->next) {
        ...
        xmlnode_put_attrib_ns(
            xmlnode_insert_tag_ns(push_packet->iq, "list", NULL, NS_PRIVACY),
            "name", NULL, NULL, edited_list);
        xmlnode_put_attrib_ns(push_packet->x, "to", NULL, NULL,
                              jid_full(cur->id));
 ...
 * @return 0 if it is not in use by any other session, else number of sessions
 * using this list
 */
static int mod_privacy_list_in_use_by_other(session s, const char *list) {
    ...
    for (cur = s->u->sessions; cur != NULL; cur = cur->next) {
        ...
        if (j_strcmp(static_cast<char *>(
                         xhash_get(cur->aux_data, "mod_privacy_active")),
                     list) == 0)
            found++;
    }
...
static void mod_privacy_free_current_list_definitions(session s) {
    ...
    /* free the lists */
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(s->aux_data, "mod_privacy_list_message"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(s->aux_data, "mod_privacy_list_presence-in"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(s->aux_data, "mod_privacy_list_presence-out"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(s->aux_data, "mod_privacy_list_iq"));
    if (list != NULL) {
        pool_free(list->p);
    }
...
static void mod_privacy_free_current_offline_list_definitions(udata user) {
    ...
    /* free the lists */
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(user->aux_data, "mod_privacy_list_message"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(user->aux_data, "mod_privacy_list_presence-in"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(user->aux_data, "mod_privacy_list_presence-out"));
    if (list != NULL) {
        pool_free(list->p);
    }
    list = static_cast<struct mod_privacy_compiled_list_item *>(
        xhash_get(user->aux_data, "mod_privacy_list_iq"));
    if (list != NULL) {
        pool_free(list->p);
    }
 ...
 */
static jid mod_privacy_filter_jidlist(
    pool p, const jid jid_list, udata user,
    struct mod_privacy_compiled_list_item *privacy_list) {
    jid cur = NULL;
    jid result = NULL;
 ...
 * @return list of JIDs, that are subscribed to us, but have presence-out
 * currently blocked, NULL if none
 */
 ...
 * @param p pool to use for memory allocations
 * @param s for which session the list should get calculated
 * @return list of JIDs, we are subscribed to, but have presence-in currently
 * blocked, NULL if none
 */
...
static void mod_privacy_no_active_list(jsmi si, session s) {
    ...
    /* there are no blocked users now, send presence to trustees, that where
     * blocked before */
    ...
    /* there are no blocked users now, probe presence for seen jids, that where
     * blocked before */
...
/**
 * insert an item to a compiled privacy list at the position selected by the
 * order
 *
 ...
 * @param jid_str the JabberID to insert (NULL for a wildcard match of any
 * JabberID)
 * @param subscription if != NULL, match this subscription type
 * @param do_deny 1 if the action should be denial, 0 if teh action should be
 * acceptance
 */
static void
mod_privacy_insert_list_item(struct mod_privacy_compiled_list_item **list,
                             long order, const char *jid_str,
                             const char *subscription, int do_deny) {
    ...
    if (jid_str != NULL) {
        ...
        if (match_jid == NULL) {
            /* invalid JID */
            log_debug2(ZONE, LOGT_EXECFLOW, "Ignoring invalid JID: %s",
                       match_jid);
    ...
    /* create the new item */
    new_item = static_cast<struct mod_privacy_compiled_list_item *>(
        pmalloco(p, sizeof(struct mod_privacy_compiled_list_item)));
    new_item->p = p;
 ...
 * @param do_deny if the action of the roster group should be allow (0) or deny
 * (1)
 * @param roster the user's current roster
 * @param std_namespace_prefixes hash containing namespace prefixes for xpath
 * expressions
 */
static void
mod_privacy_insert_rostergroup(struct mod_privacy_compiled_list_item **list,
                               long order, const char *group, int do_deny,
                               xmlnode roster, xht std_namespace_prefixes) {
    /* iterate over all items */
    xmlnode_vector items =
        xmlnode_get_tags(roster, "roster:item", std_namespace_prefixes);
    ...
    for (item_iter = items.begin(); item_iter != items.end(); ++item_iter) {
        ...
        xmlnode_vector groups = xmlnode_get_tags(*item_iter, "roster:group",
        ...
        for (group_iter = groups.begin(); group_iter != groups.end();
            ...
            if (j_strcmp(xmlnode_get_data(*group_iter), group) == 0) {
                ...
                mod_privacy_insert_list_item(
                    list, order, xmlnode_get_attrib_ns(*item_iter, "jid", NULL),
                    NULL, do_deny);
            }
        }
    }
...
/**
 * 'compile' a privacy list: the list gets prepared for faster processing when
 * it is needed.
 *
 ...
 * @param list_type for which type of stanzas the privacy list should get
 * compiled ('iq', 'message', 'presence-in', or 'presence-out')
 * @return pointer to the first entry in the compiled list (NULL if no list)
 */
static struct mod_privacy_compiled_list_item *
mod_privacy_compile_list(jsmi si, xmlnode list, xmlnode roster,
                         const char *list_type) {
    struct mod_privacy_compiled_list_item *new_list = NULL;

    /* iterate over all items in the XML list */
    xmlnode_vector privacy_items =
        xmlnode_get_tags(list, "privacy:item", si->std_namespace_prefixes);
    xmlnode_vector::iterator list_iter;
    for (list_iter = privacy_items.begin(); list_iter != privacy_items.end();
         ++list_iter) {
        ...
        log_debug2(
            ZONE, LOGT_EXECFLOW, "Compiling privacy list item: %s",
            xmlnode_serialize_string(*list_iter, xmppd::ns_decl_list(), 0));

        /* check if this item is relevant for the list */
        xmlnode_vector child_element = xmlnode_get_tags(
            *list_iter, "privacy:*", si->std_namespace_prefixes);
        if (child_element.size() > 0) {
            int match = 0;
            xmlnode_vector::iterator child_iter;
            for (child_iter = child_element.begin();
                 child_iter != child_element.end(); ++child_iter) {
                if (j_strcmp(list_type, xmlnode_get_localname(*child_iter)) ==
                    0) {
                    match = 1;
                    break;
                }
            ...
            if (!match) {
                log_debug2(ZONE, LOGT_EXECFLOW,
                           "This item is not relevant for %s", list_type);
                continue;
            }
        }
        ...
        if (type == NULL) {
            mod_privacy_insert_list_item(&new_list, order_long, NULL, NULL,
                                         do_deny);
        } else if (j_strcmp(type, "jid") == 0) {
            mod_privacy_insert_list_item(&new_list, order_long, value, NULL,
                                         do_deny);
        } else if (j_strcmp(type, "subscription") == 0) {
            mod_privacy_insert_list_item(&new_list, order_long, NULL, value,
                                         do_deny);
        } else if (j_strcmp(type, "group") == 0) {
            mod_privacy_insert_rostergroup(&new_list, order_long, value,
                                           do_deny, roster,
                                           si->std_namespace_prefixes);
        } else {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "Skipping list item with unknown type.");
        }
    }
...
static int mod_privacy_activate_list(jsmi si, session s, xmlnode list) {
    ...
    /* keep the name of the list (compare with previous value and do not pstrdup
     * if it is the same)*/
    if (j_strcmp(list_name, static_cast<char *>(xhash_get(
                                s->aux_data, "mod_privacy_active"))) != 0)
    ...
    xmlnode_vector groups = xmlnode_get_tags(roster, "roster:item/roster:group",
                                             si->std_namespace_prefixes);
    ...
    for (group = groups.begin(); group != groups.end(); ++group) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "Checking normalization of roster group: %s", group_name);

        /* could the name be normalized? */
        if (normal_group == NULL || !normal_group->has_resource()) {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "Could not normalize group name in roster: %s",
                       group_name);
            xmlnode_hide(*group);
            continue;
        }
        ...
        if (j_strcmp(group_name, normal_group->get_resource().c_str()) != 0) {
            xmlnode_vector text_node =
                xmlnode_get_tags(*group, "text()", si->std_namespace_prefixes);

            log_debug2(ZONE, LOGT_EXECFLOW, "Normalized '%s' to '%s'",
                       group_name, normal_group->get_resource().c_str());

            if (text_node.size() > 0) {
                xmlnode_hide(text_node[0]);
            }
            xmlnode_insert_cdata(*group, normal_group->get_resource().c_str(),
                                 -1);
        }
    }

    /* normalize group names in a privacy list */
    groups = xmlnode_get_tags(list, "privacy:item[@type='group']",
                              si->std_namespace_prefixes);
    for (group = groups.begin(); group != groups.end(); ++group) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "Checking normalization of group on list: %s", group_name);

        /* could the name be normalized? */
        if (normal_group == NULL || !normal_group->has_resource()) {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "Could not normalize group name on list: %s",
                       group_name);
            xmlnode_hide(*group);
            continue;
        }
        ...
        if (j_strcmp(group_name, normal_group->get_resource().c_str()) != 0) {
            log_debug2(ZONE, LOGT_EXECFLOW, "Normalized '%s' to '%s'",
                       group_name, normal_group->get_resource().c_str());
            xmlnode_put_attrib_ns(*group, "value", NULL, NULL,
                                  normal_group->get_resource().c_str());
        }
    }
    ...
    for (cur = blocked_trustees_before; cur != NULL; cur = cur->next) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW, "trustee blocked before: %s",
                   jid_full(cur));
    ...
    for (cur = blocked_trustees_after; cur != NULL; cur = cur->next) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW, "trustee now blocked: %s",
                   jid_full(cur));
        ...
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "... not blocked before. Send presence unavailable.");
    ...
    for (cur = blocked_seen_jids_before; cur != NULL; cur = cur->next) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW, "seen JID blocked before: %s",
                   jid_full(cur));
    ...
    for (cur = blocked_seen_jids_after; cur != NULL; cur = cur->next) {
        ...
        log_debug2(ZONE, LOGT_EXECFLOW, "seen JID now blocked: %s",
                   jid_full(cur));
        ...
        xmlnode_put_attrib_ns(
            presence, "from", NULL, NULL,
            jid_full(cur)); /* XXX: well actually we should send it from the
                               resource, but we do not have it here */
        jp = jpacket_new(presence);
        jp->flag = PACKET_PASS_FILTERS_MAGIC;
        js_session_to(s, jp);
    }
...
static int mod_privacy_activate_named(jsmi si, session s, const char *name) {
    ...
    log_debug2(ZONE, LOGT_EXECFLOW,
               "mod_privacy_activate_named() for '%s' list '%s'",
               jid_full(s->id), name);

    /* get all privacy lists and select the correct one */
    all_lists = xdb_get(s->si->xc, s->u->id, NS_PRIVACY);
    if (name == NULL) {
        named_list = xmlnode_get_tags(all_lists, "*[@jabberd:default]",
                                      s->si->std_namespace_prefixes);

        if (named_list.size() == 0) {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "Activating default list, with declined default list: "
                       "disabling privacy lists for this session");
        ...
        xpath << "*[@name='" << name << "']";
        named_list = xmlnode_get_tags(all_lists, xpath.str().c_str(),
                                      s->si->std_namespace_prefixes);
        pool_free(p);

        if (named_list.size() == 0) {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "privacy list '%s' not found for user %s", name,
                       jid_full(s->id));
            xmlnode_free(all_lists);
            return 0;
        }
...
static void mod_privacy_load_offline_list(udata user) {
    ...
    log_debug2(ZONE, LOGT_EXECFLOW,
               "Loading (default) privacy list for offline handling of user %s",
               jid_full(user->id));
    ...
    xmlnode_vector default_list = xmlnode_get_tags(
        all_lists, "*[@jabberd:default]", user->si->std_namespace_prefixes);
    ...
    if (xhash_get(user->aux_data, "mod_privacy_lists_loaded") == NULL) {
        ...
        xhash_put(user->aux_data, "mod_privacy_lists_loaded",
                  const_cast<char *>("loaded"));
    }
    ...
    new_list =
        mod_privacy_compile_list(user->si, default_list[0], roster, "message");
    if (new_list)
        xhash_put(user->aux_data, "mod_privacy_list_message", new_list);
    new_list = mod_privacy_compile_list(user->si, default_list[0], roster,
                                        "presence-in");
    if (new_list)
    ...
    new_list =
        mod_privacy_compile_list(user->si, default_list[0], roster, "iq");
    if (new_list)
 ...
 * @param new_active_list the list that should become active (NULL to decline
 * active list)
 * @return M_HANDLED
 */
static mreturn mod_privacy_out_iq_set_active(mapi m,
                                             const char *new_active_list) {
    ...
    if (!mod_privacy_safe_name(new_active_list)) {
        js_bounce_xmpp(
            m->si, m->s, m->packet->x,
            (xterror){406,
                      N_("The server cannot accept that privacy list name."),
 ...
 *
 * @param m the mapi structure containing the request
 * @param new_default_list the list that should become default (NULL to decline
 * default list)
 * @return M_HANDLED
 */
static mreturn mod_privacy_out_iq_set_default(mapi m,
                                              const char *new_default_list) {
    xmlnode all_lists = NULL;
    const char *old_default_list = NULL;
    const char *current_active_list = NULL;

    log_debug2(ZONE, LOGT_EXECFLOW,
               "Updating default privacy list selection: %s", new_default_list);
    ...
    xmlnode_vector default_list =
        xmlnode_get_tags(all_lists, "privacy:list[@jabberd:default]",
    ...
    if (new_default_list != NULL) {
        ...
        xmlnode_vector default_list_new = xmlnode_get_tags(
            all_lists, xpath.str().c_str(), m->si->std_namespace_prefixes);
        ...
        xmlnode_put_attrib_ns(default_list_new[0], "default", "jabberd",
                              NS_JABBERD_WRAPPER, "default");
        xdb_act_path(m->si->xc, m->user->id, NS_PRIVACY, "insert",
    ...
    /* update the active list for the current session, if the default list was
     * in use */
    current_active_list =
        static_cast<char *>(xhash_get(m->s->aux_data, "mod_privacy_active"));
    if ((current_active_list == NULL && old_default_list == NULL) ||
        j_strcmp(current_active_list, old_default_list) == 0) {
 ...
 *
 * @todo RFC3920 requires that deleting a list fails if there is no such list.
 * We are confirming such a deletion as well for performance reasons. Think
 * about that.
 *
 * @param m the mapi structure containing the request
 * @param new_list the new list definition (empty list element, to delete a
 * list)
 * @return M_HANDLED
 */
static mreturn mod_privacy_out_iq_set_list(mapi m, xmlnode new_list) {
    ...
    log_debug2(ZONE, LOGT_EXECFLOW,
               "Setting new privacy list definition for list '%s'",
               edited_list);

    /* we cannot handle some names */
    if (!mod_privacy_safe_name(edited_list)) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "We cannot accept this privacy list name.");
        js_bounce_xmpp(
            m->si, m->s, m->packet->x,
            (xterror){406,
                      N_("The server cannot accept that privacy list name."),
    ...
    if (mod_privacy_list_in_use_by_other(m->s, edited_list) > 0) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "Privacy list is in use and cannot be updated.");
        js_bounce_xmpp(m->si, m->s, m->packet->x, XTERROR_CONFLICT);
        return M_HANDLED;
    }
    ...
    xmlnode_vector previous_list =
        xmlnode_get_tags(previous_lists, edited_list_xpath.str().c_str(),
                         m->si->std_namespace_prefixes);
    if (previous_list.size() > 0 &&
        xmlnode_get_attrib_ns(previous_list[0], "default",
                              NS_JABBERD_WRAPPER) != NULL) {
        xmlnode_put_attrib_ns(new_list, "default", "jabberd",
                              NS_JABBERD_WRAPPER, "default");
    }

    /* is it an edit or a delete request? */
    log_debug2(ZONE, LOGT_EXECFLOW, "Checking the new list: %s",
               xmlnode_serialize_string(new_list, xmppd::ns_decl_list(), 0));
    xmlnode_vector new_items = xmlnode_get_tags(new_list, "privacy:item",
                                                m->si->std_namespace_prefixes);
    xmlnode_vector::iterator new_item;
    for (new_item = new_items.begin(); new_item != new_items.end();
         ++new_item) {
        ...
        if (order == NULL) {
            ...
            js_bounce_xmpp(
                ...
                (xterror){
                    400,
                    N_("Privacy list is invalid: item without order attribute"),
                    "modify", "bad-request"});
            return M_HANDLED;
        }
        if (j_strcmp(action, "deny") != 0 && j_strcmp(action, "allow") != 0) {
            ...
            js_bounce_xmpp(m->si, m->s, m->packet->x,
                           (xterror){400,
                                     N_("Privacy list is invalid: item action "
                                        "has to be either 'deny' or 'allow'"),
                                     "modify", "bad-request"});
            return M_HANDLED;
        }
        if (type == NULL) {
            if (value != NULL) {
                ...
                js_bounce_xmpp(
                    m->si, m->s, m->packet->x,
                    (xterror){400,
                              N_("Privacy list is invalid: fall-through item "
            ...
            if (test_jid == NULL) {
                ...
                js_bounce_xmpp(
                    m->si, m->s, m->packet->x,
                    (xterror){400,
                              N_("Privacy list is invalid: if type is 'jid', "
                                 "value has to be a valid JID"),
                              "modify", "bad-request"});
                return M_HANDLED;
            }
        } else if (j_strcmp(type, "group") == 0) {
            if (j_strlen(value) <= 0) {
                ...
                js_bounce_xmpp(
                    m->si, m->s, m->packet->x,
                    (xterror){400,
                              N_("Privacy list is invalid: if type is 'group', "
                                 "value has to be a roster group name"),
                              "modify", "bad-request"});
                return M_HANDLED;
            }
            /* XXX TODO: check that roster group exists */
        } else if (j_strcmp(type, "subscription") == 0) {
            if (j_strcmp(value, "both") != 0 && j_strcmp(value, "none") != 0 &&
                j_strcmp(value, "to") != 0 && j_strcmp(value, "from") != 0) {
                xmlnode_free(previous_lists);
                js_bounce_xmpp(
                    m->si, m->s, m->packet->x,
                    (xterror){400,
                              N_("Privacy list is invalid: if type is "
                                 "'subscription', value has to be one of "
                                 "'none', 'from', 'to', or 'both'"),
                              "modify", "bad-request"});
                return M_HANDLED;
            }
        } else {
            ...
            js_bounce_xmpp(
                ...
                (xterror){
                    400,
                    N_("Privacy list is invalid: if type is present, it has to "
                       "be one of 'jid', 'group', or 'subscription'"),
                    "modify", "bad-request"});
            return M_HANDLED;
        }
    ...
    /* save the new list */
    xdb_result = xdb_act_path(m->si->xc, m->user->id, NS_PRIVACY, "insert",
                              edited_list_xpath.str().c_str(),
                              m->si->std_namespace_prefixes,
                              list_items > 0 ? new_list : NULL);
    if (xdb_result) {
        xmlnode_free(previous_lists);
    ...
    log_debug2(ZONE, LOGT_STORAGE,
               "New privacy list definition has been stored.");

    /* if it has been the default list, do we have to update the user's offline
     * list? */
    if (xhash_get(m->user->aux_data, "mod_privacy_lists_loaded")) {
        mod_privacy_load_offline_list(m->user);
    }
    ...
    /* do we use this list? */
    if (j_strcmp(static_cast<char *>(
                     xhash_get(m->s->aux_data, "mod_privacy_active")),
                 edited_list) == 0) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "The edited list was in use by us. Updating used list.");
        if (list_items > 0) {
...
static mreturn mod_privacy_out_iq_set(mapi m) {
    /* check the type of request: exactly one child element in the NS_PRIVACY
     * namespace required */
    xmlnode_vector child_element = xmlnode_get_tags(
        m->packet->iq, "privacy:*", m->si->std_namespace_prefixes);
    ...
    if (j_strcmp(xmlnode_get_localname(child_element[0]), "active") == 0) {
        return mod_privacy_out_iq_set_active(
            m, xmlnode_get_attrib_ns(child_element[0], "name", NULL));
    } else if (j_strcmp(xmlnode_get_localname(child_element[0]), "default") ==
               0) {
        return mod_privacy_out_iq_set_default(
            m, xmlnode_get_attrib_ns(child_element[0], "name", NULL));
...
static mreturn mod_privacy_out_iq_get(mapi m) {
    ...
    xmlnode_vector query_data = xmlnode_get_tags(m->packet->iq, "privacy:*",
                                                 m->si->std_namespace_prefixes);
    if (query_data.size() == 0) {
        ...
        /* user wants to get the names of the lists, and which list is default
         * or active */
        jutil_iqresult(m->packet->x);
        m->packet->iq =
            xmlnode_insert_tag_ns(m->packet->x, "query", NULL, NS_PRIVACY);
        if (storedlists != NULL) {
            log_debug2(ZONE, LOGT_EXECFLOW, "we have stored lists to copy in");
            xmlnode_insert_node(m->packet->iq,
                                xmlnode_get_firstchild(storedlists));
            xmlnode_free(storedlists);
            storedlists = NULL;
        }

        /* iterate the lists and hide their child nodes */
        xmlnode_vector lists = xmlnode_get_tags(m->packet->iq, "privacy:list",
                                                m->si->std_namespace_prefixes);
        xmlnode_vector::iterator list_iter;
        for (list_iter = lists.begin(); list_iter != lists.end(); ++list_iter) {
            /* check if it is the default list, and add <active/> element in
             * that case */
            const char *default_list = xmlnode_get_attrib_ns(
                *list_iter, "default", NS_JABBERD_WRAPPER);
            if (default_list != NULL) {
                default_list = xmlnode_get_attrib_ns(*list_iter, "name", NULL);
                xmlnode default_element = xmlnode_insert_tag_ns(
                    m->packet->iq, "default", NULL, NS_PRIVACY);
                xmlnode_put_attrib_ns(default_element, "name", NULL, NULL,
                                      default_list);
                xmlnode_hide_attrib_ns(*list_iter, "default",
                                       NS_JABBERD_WRAPPER);
                log_debug2(ZONE, LOGT_EXECFLOW, "default list is: %s",
                           default_list);
            }
        ...
        active_list = static_cast<char *>(
            xhash_get(m->s->aux_data, "mod_privacy_active"));
        if (active_list != NULL) {
            xmlnode active_element = xmlnode_insert_tag_ns(
                m->packet->iq, "active", NULL, NS_PRIVACY);
            xmlnode_put_attrib_ns(active_element, "name", NULL, NULL,
                                  active_list);
    ...
    /* if query has more than one child in the privacy namespace, we deny the
     * request */
    if (query_data.size() > 1 ||
        j_strcmp(xmlnode_get_localname(query_data[0]), "list") != 0) {
        if (storedlists)
            xmlnode_free(storedlists);
        js_bounce_xmpp(m->si, m->s, m->packet->x, XTERROR_BAD);
        return M_HANDLED;
    ...
    log_debug2(ZONE, LOGT_EXECFLOW, "Client requested privacy list: %s",
               requested_list);

    /* get the requested list */
    std::ostringstream xpath;
    xpath << "privacy:list[@name='" << requested_list << "']";
    xmlnode_vector lists = xmlnode_get_tags(storedlists, xpath.str().c_str(),
                                            m->si->std_namespace_prefixes);
...
static mreturn mod_privacy_serialize(mapi m, void *arg) {
    ...
    /* serialize our data */
    mod_privacy_data = xmlnode_insert_tag_ns(
        m->serialization_node, "modPrivacy", NULL, NS_JABBERD_STOREDSTATE);
    xmlnode_put_attrib_ns(
        xmlnode_insert_tag_ns(mod_privacy_data, "active", NULL, NS_PRIVACY),
        "name", NULL, NULL, active_list);
 ...
 * filter stanzas
 *
 * if m->s is NULL, it is a stanza to a offline user; else it is data to a
 * session
 *
...
static mreturn mod_privacy_filter(mapi m, void *arg) {
    ...
    if (m == NULL || m->packet == NULL || m->packet->to == NULL ||
        m->packet->from == NULL || m->user == NULL)
        return M_PASS;

    /* don't filter packets from the user itself (for outgoing filtering this
     * has been already checked) */
    if (arg == (void *)0 &&
        jid_cmpx(m->packet->to, m->packet->from, JID_USER | JID_SERVER) == 0)
        return M_PASS;

    log_debug2(
        ZONE, LOGT_EXECFLOW, "filtering %s packet %s: %s",
        arg ? "outgoing" : "incoming",
        m->s ? "for session" : "for offline account",
        xmlnode_serialize_string(m->packet->x, xmppd::ns_decl_list(), 0));

    /* if it is to an offline user, we might have to load the privacy lists
     * first */
    if (m->s == NULL &&
        !xhash_get(m->user->aux_data, "mod_privacy_lists_loaded")) {
        mod_privacy_load_offline_list(m->user);
    }
    ...
    switch (m->packet->type) {
        case JPACKET_IQ:
            do_bounce = 1;
            affected_list =
                static_cast<struct mod_privacy_compiled_list_item *>(
                    xhash_get(m->s ? m->s->aux_data : m->user->aux_data,
                              "mod_privacy_list_iq"));
            break;
        ...
        case JPACKET_PRESENCE:
            ...
            affected_list =
                static_cast<struct mod_privacy_compiled_list_item *>(
                    xhash_get(m->s ? m->s->aux_data : m->user->aux_data,
                              arg ? "mod_privacy_list_presence-out"
                                  : "mod_privacy_list_presence-in"));
            break;
    }
    ...
    if (affected_list == NULL) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "there is no privacy list for this ... accept");
        return M_PASS;
    }
    ...
    if (mod_privacy_denied(affected_list, m->user,
                           arg ? m->packet->to : m->packet->from)) {
        log_debug2(ZONE, LOGT_EXECFLOW, "... and denied");

        if (do_bounce) {
            xterror err =
                (xterror){503, N_("Blocked by recipient's privacy list"),
                          "cancel", "service-unavailable"};
            if (arg) {
                snprintf(err.msg, sizeof(err.msg), "%s",
                         N_("Blocked by your own privacy list"));
            }
...
static mreturn mod_privacy_rosterchange(mapi m, void *arg) {
    ...
    log_debug2(ZONE, LOGT_EXECFLOW, "received rosterchange event for user '%s'",
               jid_full(m->user->id));

    /* check if the default list has been loaded for offline delivery, and
     * reload if yes */
    if (xhash_get(m->user->aux_data, "mod_privacy_lists_loaded")) {
        log_debug2(ZONE, LOGT_EXECFLOW,
                   "reloading default list for offline handling");
        mod_privacy_load_offline_list(m->user);
    }

    /* reload the sessions' lists */
    for (cur = m->user->sessions; cur != NULL; cur = cur->next) {
        const char *active_list =
            static_cast<char *>(xhash_get(cur->aux_data, "mod_privacy_active"));

        if (active_list != NULL) {
            log_debug2(ZONE, LOGT_EXECFLOW,
                       "Reloading list '%s' for session '%s'", active_list,
                       jid_full(cur->id));
            mod_privacy_activate_named(m->si, cur, active_list);
        } else {
            log_debug2(ZONE, LOGT_EXECFLOW, "No active list for session '%s'",
                       jid_full(cur->id));
        }
    }
...
static mreturn mod_privacy_deserialize(mapi m, void *arg) {
    xmlnode_vector active_list = xmlnode_get_tags(
        m->serialization_node, "state:modPrivacy/privacy:active",
        m->si->std_namespace_prefixes);
    if (active_list.size() > 0)
        mod_privacy_activate_named(
            m->si, m->s, xmlnode_get_attrib_ns(active_list[0], "name", NULL));
