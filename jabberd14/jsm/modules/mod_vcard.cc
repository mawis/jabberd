 ...
 *
 * This module allows publishing of vcard data, replies queries for the vcard
 * data of a user, responds to queries for the server vcard, and my forward
 * published vcards to a configured Jabber users directory.
 */
...
static mreturn mod_vcard_jud(mapi m) {
    ...
    if (vcard != NULL) {
        log_debug2(ZONE, LOGT_DELIVER, "sending registration for %s",
        ...
        regq = xmlnode_get_list_item(
            xmlnode_get_tags(reg, "register:query",
                             m->si->std_namespace_prefixes),
            0);

        xmlnode_insert_cdata(
            xmlnode_insert_tag_ns(regq, "name", NULL, NS_REGISTER),
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(vcard, "vcard:FN",
                                 m->si->std_namespace_prefixes),
                0)),
            -1);
        xmlnode_insert_cdata(
            xmlnode_insert_tag_ns(regq, "first", NULL, NS_REGISTER),
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(vcard, "vcard:N/vcard:GIVEN",
                                 m->si->std_namespace_prefixes),
                0)),
            -1);
        xmlnode_insert_cdata(
            xmlnode_insert_tag_ns(regq, "last", NULL, NS_REGISTER),
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(vcard, "vcard:N/vcard:FAMILY",
                                 m->si->std_namespace_prefixes),
                0)),
            -1);
        xmlnode_insert_cdata(
            xmlnode_insert_tag_ns(regq, "nick", NULL, NS_REGISTER),
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(vcard, "vcard:NICKNAME",
                                 m->si->std_namespace_prefixes),
                0)),
            -1);
        xmlnode_insert_cdata(
            xmlnode_insert_tag_ns(regq, "email", NULL, NS_REGISTER),
            xmlnode_get_data(xmlnode_get_list_item(
                xmlnode_get_tags(vcard, "vcard:EMAIL",
                                 m->si->std_namespace_prefixes),
                0)),
            -1);
 ...
 * @return M_IGNORE if not an iq stanza, M_HANDLED if the packet has been
 * handled, M_PASS else
 */
static mreturn mod_vcard_set(mapi m, void *arg) {
    ...
    switch (jpacket_subtype(m->packet)) {
        case JPACKET__GET:
            ...
            break;
        case JPACKET__SET:
            log_debug2(ZONE, LOGT_DELIVER, "handling set request %s",
            ...
            xmlnode_hide(xmlnode_get_list_item(
                xmlnode_get_tags(m->packet->x, "vcard:vcard",
                                 m->si->std_namespace_prefixes),
                0));
            jpacket_reset(m->packet);
            js_session_to(m->s, m->packet);
            ...
            /* handle putting the vcard to the configured jud: send a get
             * request to the jud services */
            browse = js_config(m->si, "browse:browse",
                               xmlnode_get_lang(m->packet->x));
            for (cur = xmlnode_get_firstchild(browse); cur != NULL;
                 cur = xmlnode_get_nextsibling(cur)) {
                if (j_strcmp(xmlnode_get_attrib_ns(cur, "type", NULL),
                             "user") != 0)
                    continue;
                if (j_strcmp(xmlnode_get_attrib_ns(cur, "category", NULL),
                             "directory") != 0)
                    continue;

                judreg = jutil_iqnew(JPACKET__GET, NS_REGISTER);
                xmlnode_put_attrib_ns(judreg, "to", NULL, NULL,
                                      xmlnode_get_attrib_ns(cur, "jid", NULL));
                xmlnode_put_attrib_ns(judreg, "id", NULL, NULL,
                                      "mod_vcard_jud");
 ...
 * @param arg unused/ignored
 * @return M_IGNORE if not an iq stanza, M_HANDLED if the packet is handled,
 * M_PASS else
 */
static mreturn mod_vcard_reply(mapi m, void *arg) {
    ...
    if (NSCHECK(m->packet->iq, NS_REGISTER) &&
        j_strcmp(xmlnode_get_attrib_ns(m->packet->x, "id", NULL),
                 "mod_vcard_jud") == 0)
 ...
 * @param arg unused/ignored
 * @return M_IGNORE if not a iq stanza, M_HANDLED if the packet has been
 * handled, M_PASS else
 */
static mreturn mod_vcard_server(mapi m, void *arg) {
    ...
    if (jpacket_subtype(m->packet) != JPACKET__GET ||
        !NSCHECK(m->packet->iq, NS_VCARD) || m->packet->to->has_resource())
        return M_PASS;

    /* get data from the config file */
    if ((vcard = js_config(m->si, "vcard:vCard",
 ...
 * register mod_vcard_server to be called for packets sent to the server
 * address.
 *
