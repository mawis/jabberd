...
static xmlnode mod_roster_get(mapi m, udata u) {
    ...
    log_debug2(ZONE, LOGT_ROSTER, "getting %s's roster",
               u->id->get_node().c_str());
    ...
    xmlnode_vector items =
        xmlnode_get_tags(ret, "roster:item", m->si->std_namespace_prefixes);
    for (xmlnode_vector::iterator p = items.begin(); p != items.end(); ++p) {
        ...
        if (!item_jid) {
            log_debug2(ZONE, LOGT_ROSTER,
                       "removing this item, that has no JID");
 ...
 * @param newflag where to store 1 if the item did not exist and has just been
 * created
 * @return the roster item
 */
static xmlnode mod_roster_get_item(mapi m, xmlnode roster, jid id,
                                   int *newflag) {
    ...
    xmlnode_vector ret_v = xmlnode_get_tags(roster, xpath.str().c_str(),
...
static void mod_roster_push(udata user, xmlnode item) {
    ...
    log_debug2(ZONE, LOGT_ROSTER, "pushing %s",
               xmlnode_serialize_string(item, xmppd::ns_decl_list(), 0));
    ...
    xmlnode_hide_attrib_ns(xmlnode_get_firstchild(query), "subscribe",
                           NULL); /* hide the server tirds */
...
static void mod_roster_set_s10n(int from, int to, xmlnode item) {
    xmlnode_put_attrib_ns(item, "subscription", NULL, NULL,
                          from ? (to ? "both" : "from") : (to ? "to" : "none"));
}
...
static mreturn mod_roster_out_s10n(mapi m) {
    ...
    if (j_strcmp(xmlnode_get_attrib_ns(item, "subscription", NULL), "to") == 0)
        to = 1;
    if (j_strcmp(xmlnode_get_attrib_ns(item, "subscription", NULL), "from") ==
        0)
        from = 1;
    if (j_strcmp(xmlnode_get_attrib_ns(item, "subscription", NULL), "both") ==
        0)
        to = from = 1;
    if (xmlnode_get_attrib_ns(item, "subscribe", NULL) != NULL)
        p_in = 1;

    /* ask='unsubscribe' can be in xdb from old data written by jabberd up to
     * version 1.4.3 */
    if (j_strcmp(xmlnode_get_attrib_ns(item, "ask", NULL), "unsubscribe") ==
        0) {
        to = 0;
        xmlnode_put_attrib_ns(item, "subscription", NULL, NULL,
                              from ? "from" : "none");
    ...
    switch (jpacket_subtype(m->packet)) {
        ...
        case JPACKET__SUBSCRIBED:
            if (force_sent || (!from && p_in)) {
                /* XMPP IM, sect. 9 states "None + Pending In", "None + Pending
                 * Out/In", and "To + Pending In" */
                route = 1;
                mod_roster_set_s10n(1, to, item); /* update subscription */
                jid_append(js_trustees(m->user),
                           m->packet->to); /* make them trusted now */
                xmlnode_hide_attrib_ns(item, "subscribe",
                                       NULL); /* reset "Pending In" */
                xmlnode_hide_attrib_ns(
                    item, "hidden",
                    NULL); /* make it visible on the user's roster */
                mod_roster_pforce(m->user, m->packet->to,
                                  0); /* they are now subscribed to us, send
                                         them our presence */
                rosterchange = 1;
                mod_roster_push(
                    m->user,
                    item); /* new roster to the user's other sessions */

                /* delete stored subscription request from xdb */
                std::ostringstream xpath;
                xpath << "presence[@from='" << jid_full(m->packet->to) << "']";
                xdb_act_path(m->si->xc, m->user->id, NS_JABBERD_STOREDREQUEST,
                             "insert", xpath.str().c_str(),
                             m->si->std_namespace_prefixes, NULL);
            } else {
        ...
        case JPACKET__UNSUBSCRIBE:
            if (to) {
                /* changed behaviour since after version 1.4.3 ... we now
                 * immediatelly change to state "From" or "None" jabberd up to
                 * version 1.4.3 only set the flag ask="unsubscribe" and waited
                 * for an unsubscribed configirmation from the contact's server
                 */
                mod_roster_set_s10n(from, 0, item);
                xmlnode_hide_attrib_ns(item, "ask",
                                       NULL); /* reset Pending Out */
                rosterchange = 1;
                mod_roster_push(m->user, item);
                js_remove_seen(m->user, m->packet->to);
            } else if (newflag) {
                /* the contact was not on the roster and should not become a
                 * roster item */
                xmlnode_hide(item);
            }
            /* always route the packet, the user might have unsubscribed a
             * second time because the contact's server is out of sync */
            route = 1;
            break;
        case JPACKET__UNSUBSCRIBED:
            if (!from && !p_in && !force_sent) {
                /* XMPP IM, sect. 9 states "None", "None + Pending Out", and
                 * "To" */
                route = 0;
                /* no state change */
            } else {
            ...
            if (((!route || (!from && !p_in && force_sent)) && newflag) ||
                xmlnode_get_attrib_ns(item, "hidden", NULL)) {
                /* the contact was not on the roster and should not become a
                 * roster item */
                xmlnode_hide(item);
            }
...
static mreturn mod_roster_out_iq(mapi m) {
    ...
    switch (jpacket_subtype(m->packet)) {
        case JPACKET__GET:
            ...
            elements = xmlnode_get_tags(m->packet->iq, "roster:item",
                                        m->si->std_namespace_prefixes);
            ...
            elements = xmlnode_get_tags(roster, "roster:item",
                                        m->si->std_namespace_prefixes);
            for (iter = elements.begin(); iter != elements.end(); ++iter) {
                if (xmlnode_get_attrib_ns(*iter, "subscribe", NULL) != NULL) {
                    /* is there a stored version of the subscription request in
                     * xdb?
                     */
                    xmlnode stored_subscribes = xdb_get(
                        m->si->xc, m->user->id, NS_JABBERD_STOREDREQUEST);
                    std::ostringstream xpath;
                    xpath << "presence[@from='"
                          << xmlnode_get_attrib_ns(*iter, "jid", NULL) << "']";
                    pres = xmlnode_dup(xmlnode_get_list_item(
                        xmlnode_get_tags(stored_subscribes, xpath.str().c_str(),
                                         m->si->std_namespace_prefixes),
                        0));
                    ...
                    if (pres == NULL) {
                        ...
                        xmlnode_put_attrib_ns(pres, "type", NULL, NULL,
                                              "subscribe");
                        xmlnode_put_attrib_ns(
                            pres, "from", NULL, NULL,
                            xmlnode_get_attrib_ns(*iter, "jid", NULL));
                        if (j_strlen(xmlnode_get_attrib_ns(*iter, "subscribe",
                                                           NULL)) > 0)
                            xmlnode_insert_cdata(
                                xmlnode_insert_tag_ns(pres, "status", NULL,
                                                      NS_SERVER),
                                xmlnode_get_attrib_ns(*iter, "subscribe", NULL),
                                -1);
                    }
        ...
        case JPACKET__SET:
            ...
            elements = xmlnode_get_tags(m->packet->iq, "roster:item[@jid]",
                                        m->si->std_namespace_prefixes);
            for (iter = elements.begin(); iter != elements.end(); ++iter) {
                id = jid_new(m->packet->p,
                             xmlnode_get_attrib_ns(*iter, "jid", NULL));
                if (id == NULL || jid_cmpx(jid_user(m->s->id), id,
                                           JID_USER | JID_SERVER) == 0)
                    continue;

                /* zoom to find the existing item in the current roster, and
                 * hide it
                 */
                ...
                /* drop you sukkah */
                if (j_strcmp(xmlnode_get_attrib_ns(*iter, "subscription", NULL),
                             "remove") == 0) {
                    /* cancel our subscription to them */
                    if (j_strcmp(
                            xmlnode_get_attrib_ns(item, "subscription", NULL),
                            "both") == 0 ||
                        j_strcmp(
                            xmlnode_get_attrib_ns(item, "subscription", NULL),
                            "to") == 0 ||
                        j_strcmp(xmlnode_get_attrib_ns(item, "ask", NULL),
                                 "subscribe") == 0) {
                        jpacket jp = jpacket_new(jutil_presnew(
                            JPACKET__UNSUBSCRIBE,
                            xmlnode_get_attrib_ns(*iter, "jid", NULL), NULL));
                        jp->flag =
                            PACKET_FORCE_SENT_MAGIC; /* force to sent it, as
                ...
                /* copy the old stuff into the new one and insert it into the
                 * roster
                 */
                xmlnode_put_attrib_ns(
                    *iter, "subscription", NULL, NULL,
                    xmlnode_get_attrib_ns(item, "subscription", NULL));
                xmlnode_put_attrib_ns(
                    *iter, "ask", NULL, NULL,
                    xmlnode_get_attrib_ns(
                        item, "ask",
                        NULL)); /* prolly not here, but just in case */
                xmlnode_put_attrib_ns(
                    *iter, "subscribe", NULL, NULL,
                    xmlnode_get_attrib_ns(item, "subscribe", NULL));
            ...
            /* save the changes */
            log_debug2(
                ZONE, LOGT_ROSTER, "SROSTER: %s",
                xmlnode_serialize_string(roster, xmppd::ns_decl_list(), 0));
            /* XXX what do we do if the set fails?  hrmf... */
            xdb_set(m->si->xc, m->user->id, NS_ROSTER, roster);
 ...
 * @return M_IGNORE if the packet is not an iq or subscription packet, else
 * M_PASS if packet not handled or M_HANDLED if packet handled
 */
...
/**
 * register mod_roster_out callback for outgoing stanzas when a new session
 * starts
 *
 ...
 * @param m the mapi instance
 * @param arg not used/ignored
 * @return I_IGNORE if not a subscription stanza, else M_PASS if not handled or
 * M_HANDLED if handled
 */
static mreturn mod_roster_s10n(mapi m, void *arg) {
    ...
    reply2 = reply = NULL;
    jid_set(m->packet->to, NULL,
            JID_RESOURCE); /* make sure we're only dealing w/ the user id */

    log_debug2(ZONE, LOGT_ROSTER,
               "s10n %s request from %s with existing item %s",
               xmlnode_get_attrib_ns(m->packet->x, "type", NULL),
    ...
    switch (jpacket_subtype(m->packet)) {
        case JPACKET__SUBSCRIBE:
            if (from) {
                ...
                reply = jutil_presnew(
                    JPACKET__SUBSCRIBED, jid_full(m->packet->from),
                    messages_get(xmlnode_get_lang(m->packet->x),
                                 N_("Already Subscribed")));
                jid_set(m->packet->to, NULL, JID_RESOURCE);
                xmlnode_put_attrib_ns(reply, "from", NULL, NULL,
                                      jid_full(m->packet->to));
                drop = 1;

                /* the other person obviously is re-adding them to their roster,
                 * and should be told of the current presence */
                reply2 = jutil_presnew(JPACKET__PROBE, jid_full(m->packet->to),
                                       NULL);
                xmlnode_put_attrib_ns(reply2, "from", NULL, NULL,
                                      jid_full(m->packet->from));
            } else if (p_in) {
                ...
                drop = 0;
                status = xmlnode_get_data(xmlnode_get_list_item(
                    xmlnode_get_tags(m->packet->x, "status",
                                     m->si->std_namespace_prefixes),
                    0));
                if (status == NULL)
                    ...
                    xmlnode_put_attrib_ns(item, "subscribe", NULL, NULL,
                                          status);
                if (newflag) /* SPECIAL CASE: special flag so that we can hide
                                these incoming subscribe requests */
        ...
        case JPACKET__SUBSCRIBED:
            if (to || !p_out) {
                ...
                push = 1;
                jid_append(js_seen_jids(m->user),
                           m->packet->from); /* make them seen now */
            }
            break;
        case JPACKET__UNSUBSCRIBE:
            if (from || p_in) {
                ...
                reply = jutil_presnew(
                    JPACKET__UNSUBSCRIBED, jid_full(m->packet->from),
                    messages_get(xmlnode_get_lang(m->packet->x),
                                 N_("Autoreply")));
                jid_set(m->packet->to, NULL, JID_RESOURCE);
                xmlnode_put_attrib_ns(reply, "from", NULL, NULL,
    ...
    if (store_request) {
        ...
        xdb_act_path(m->si->xc, m->user->id, NS_JABBERD_STOREDREQUEST, "insert",
                     xpath.str().c_str(), m->si->std_namespace_prefixes,
                     request);
    }

    /* these are delayed until after we check the roster back in, avoid rancid
     * race conditions */
    if (reply != NULL)
...
static mreturn mod_roster_delete(mapi m, void *arg) {
    ...
    xmlnode_vector roster_items = xmlnode_get_tags(
        roster, "roster:item[@subscription]", m->si->std_namespace_prefixes);
    for (xmlnode_vector::iterator roster_item = roster_items.begin();
         roster_item != roster_items.end(); ++roster_item) {
        ...
        subscription =
            xmlnode_get_attrib_ns(*roster_item, "subscription", NULL);

        log_debug2(ZONE, LOGT_ROSTER, "removing subscription %s (%s)",
                   subscription, jid_full(peer));
        ...
        if (unsubscribe) {
            xmlnode pp =
                jutil_presnew(JPACKET__UNSUBSCRIBE, jid_full(peer), NULL);
            xmlnode_put_attrib_ns(pp, "from", NULL, NULL,
                                  jid_full(m->user->id));
            jp = jpacket_new(pp);
            jp->flag = PACKET_FORCE_SENT_MAGIC; /* we are removing the roster,
                                                   sent anyway */
            js_deliver(m->si, jp, m->s);
        }
        if (unsubscribed) {
            xmlnode pp =
                jutil_presnew(JPACKET__UNSUBSCRIBED, jid_full(peer), NULL);
            xmlnode_put_attrib_ns(pp, "from", NULL, NULL,
                                  jid_full(m->user->id));
            jp = jpacket_new(pp);
            jp->flag = PACKET_FORCE_SENT_MAGIC; /* we are removing the roster,
