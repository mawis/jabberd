 ...
 * to a configured entity
 *
 * This module only logs ending sessions. (It expects that the beginning of a
 * session is logged by the client connection manager which is able to log the
 * IP address if required as well.)
 *
 * The module can be configured to forward all message stanzas to a configured
 * entity on the router. The message forwarding is configured using the
 * &lt;archive/&gt; element containing a &lt;service/&gt; element for each
 * destination having the destination's address as the CDATA node in it. The
 * messages are then forwarded using a &lt;route/&gt; element, therefore you
 * cannot forward messages to entities not connected to your router (you cannot
 * use s2s).
 *
 * The message forwarding functionallity is disabled if there is no
 * &lt;archive/&gt; element in the configuration of the session manager.
 *
 * mod_log should be the last in the list of modules in the session manager.
 */

/**
 * logs the characteristics of a closed session (when it started, stanzas
 * received, stanzas sent, resource)
 *
...
static mreturn mod_log_session_end(mapi m, void *arg) {
    ...
    log_record(jid_full(m->user->id), "session", "end", "%d %d %d %s",
               (int)(t - m->s->started), m->s->c_in, m->s->c_out, m->s->res);
 ...
 *
 * If message forwarding is enabled this callback will register the
 * mod_log_archiver callback for incoming and outgoing messages.
 *
 * In any case it will register the mod_log_session_end callback for ending
 * sessions
 *
 * @param m the mapi structure
 * @param arg NULL if message forwarding is disabled, pointer to a list of
 * destination JIDs otherwise
 * @return always M_PASS
 */
 ...
 *
 * build the list of forwarding destinations for the message logging
 * functionallity (using the &lt;service/&gt; elements inside the
 * &lt;archive/&gt; element in the configuration of the session manager)
 *
...
extern "C" void mod_log(jsmi si) {
    ...
    for (cfg = xmlnode_get_firstchild(cfg); cfg != NULL;
        ...
        if (xmlnode_get_type(cfg) != NTYPE_TAG ||
            j_strcmp(xmlnode_get_localname(cfg), "service") != 0)
            continue;
        if (svcs == NULL)
