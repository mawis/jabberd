 ...
 * manager
 *
 * This module implements service discovery to get the list of available
 * services on the jabber server installation. Only handling of requests
 * addressed to the server is implemented. Handling requests to a user's address
 * are not handled by this module. Neither does this module handle service
 * discovery requests sent to a node.
 */

/**
 * This function handles iq stanzas of type 'get' in the
 * http://jabber.org/protocol/disco#info namespace.
 *
 * The result is build using the content of the <disco/> element in the session
 ...
 * @return M_PASS if the packet has not been processed, M_HANDLED if the packet
 * has been handled
 */
static mreturn mod_disco_server_info(mapi m, void *arg) {
    ...
    if (identity != NULL) {
        ...
        xmlnode generated_identity = xmlnode_insert_tag_ns(
            m->additional_result->iq, "identity", NULL, NS_DISCO_INFO);
        xmlnode vcard_fn = js_config(m->si, "vcard:vCard/vcard:FN",
                                     xmlnode_get_lang(m->packet->x));
        xmlnode_put_attrib_ns(generated_identity, "category", NULL, NULL,
                              "server");
        xmlnode_put_attrib_ns(generated_identity, "type", NULL, NULL, "im");
        xmlnode_put_attrib_ns(generated_identity, "name", NULL, NULL,
                              xmlnode_get_data(vcard_fn));
...
/**
 * This function handles iq stanzas of type 'get' in the
 * http://jabber.org/protocol/disco#items namespace.
 *
 * The result is build using the content of the <browse/> element in the session
 * manager's configruration.
 *
 * The packet is not processed if it is addressed to a specific node.
 *
 * @param m the mapi structure containing the request
...
static mreturn mod_disco_server_items(mapi m, void *arg) {
    ...
    /* build the result IQ */
    query = xmlnode_insert_tag_ns(jutil_iqresult(m->packet->x), "query", NULL,
                                  NS_DISCO_ITEMS);
    ...
    for (admin_iter = admins; admin_iter != NULL;
        ...
        xmlnode_put_attrib_ns(
            item, "name", NULL, NULL,
            messages_get(xmlnode_get_lang(m->packet->x), N_("Administrator")));
    }
    if (admins != NULL) {
 ...
 *
 * Everything but iq stanzas are ignored. Only iq stanzas of type 'get' are
 * processed. Processing of queries in the namespace
 * http://jabber.org/protocol/disco#items are delegated to
 * mod_disco_server_items. Processing of queries in the namespace
 * http://jabber.org/protocol/disco#info are delegated to mod_disco_server_info.
 * No other namespaces are processed.
 *
 * @param m the mapi structure containing the request
 * @param arg unused/ignored (but passed to mod_disco_server_items and
 * mod_disco_server_info)
 ...
 *
 * @param m the mapi structure containing the request
 * @return M_PASS if the request has not been processed, M_HANDLED if the
    ...
    if (js_trust(m->user, m->packet->from)) {
        for (s = m->user->sessions; s != NULL; s = s->next) {
            ...
            if (xmlnode_get_list_item(
                    xmlnode_get_tags(m->packet->iq, xpath.str().c_str(),
...
static mreturn mod_disco_user_info(mapi m) {
    ...
    xmlnode_put_attrib_ns(x, "type", NULL, NULL,
                          is_admin ? "admin" : "registered");

    vcard = xdb_get(m->si->xc, m->user->id, NS_VCARD);
    vcard_fn =
        xmlnode_get_tags(vcard, "vcard:FN", m->si->std_namespace_prefixes);
    if (vcard_fn.size() > 0) {
        std::ostringstream name;
        name << xmlnode_get_data(vcard_fn[0]);
        if (is_admin)
            name << messages_get(xmlnode_get_lang(m->packet->x),
                                 N_(" (administrator)"));
        ...
        xmlnode_put_attrib_ns(
            ...
            messages_get(xmlnode_get_lang(m->packet->x),
                         is_admin ? N_("Administrator") : N_("User")));
    }

    if (vcard != NULL) {
        x = xmlnode_insert_tag_ns(m->packet->iq, "feature", NULL,
                                  NS_DISCO_INFO);
 ...
 * Processing of queries in the namespace http://jabber.org/protocol/disco#items
 * are delegated to mod_disco_user_items. Processing of queries in the namespace
 * http://jabber.org/protocol/disco#info are delegated to mod_disco_user_info.
 * No other namespaces are processed.
 *
 * @param m the mapi structure containing the request
 * @param arg unused/ignored
