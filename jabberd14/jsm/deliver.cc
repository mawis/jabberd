...
static void js_deliver_local(jsmi si, jpacket p, xht ht) {
    ...
    /* the packet has neither been handled by the e_DELIVER modules nor was for
     * the server */

    /* if the stanza is a message, we reroute to the top session, if we have no
     * session yet */
    ...
    if (p->type == JPACKET_PRESENCE && jpacket_subtype(p) == JPACKET__PROBE) {
        jpacket jp = NULL;
        xmlnode presence_unsubscribed =
            jutil_presnew(JPACKET__UNSUBSCRIBED, jid_full(p->from), NULL);
        xmlnode_put_attrib_ns(presence_unsubscribed, "from", NULL, NULL,
                              jid_full(jid_user(p->to)));
        jp = jpacket_new(presence_unsubscribed);
        jp->flag = PACKET_FORCE_SENT_MAGIC;
        js_deliver(si, jp, NULL);

        log_notice(si->i->id,
                   "got presence probe from '%s' for non-existant user '%s' => "
                   "sent unsubscribed",
                   jid_full(p->from), jid_full(p->to));
    } else if (p->type == JPACKET_PRESENCE &&
               jpacket_subtype(p) != JPACKET__ERROR &&
               jpacket_subtype(p) != JPACKET__UNAVAILABLE) {
        /* presence to an unexistant user ... send unsubscribe */
        jpacket jp = NULL;
        xmlnode presence_unsubscribe =
            jutil_presnew(JPACKET__UNSUBSCRIBE, jid_full(p->from), NULL);
        ...
        log_notice(si->i->id,
                   "got presence from '%s' for non-existant user '%s' => sent "
                   "unsubscribe",
                   jid_full(p->from), jid_full(p->to));
    }
...
/**
 * handle incoming &lt;route type='session'/&gt; packets, we get passed from
 * jabberd: create a new session
 *
...
result _js_routed_session_packet(instance i, dpacket p, jsmi si) {
    ...
    if ((s = js_session_new(si, p)) == NULL) {
        ...
        xmlnode_put_attrib_ns(
            p->x, "error", NULL, NULL,
            messages_get(xmlnode_get_lang(p->x), N_("Session Failed")));
    } else {
...
/**
 * handle incoming &lt;route type='auth'/&gt; packets, we get passed from
 * jabberd: authentication or registration requests
 *
...
result _js_routed_auth_packet(instance i, dpacket p, jsmi si, jpacket jp) {
    /* check and see if we're configured to forward auth packets for processing
     * elsewhere */
    if (si->auth != NULL) {
        xmlnode_put_attrib_ns(
            p->x, "oto", NULL, NULL,
            xmlnode_get_attrib_ns(p->x, "to", NULL)); /* preserve original to */
    ...
    xmlnode_put_attrib_ns(jp->x, "to", NULL, NULL,
                          xmlnode_get_attrib_ns(p->x, "to", NULL));
    xmlnode_put_attrib_ns(jp->x, "from", NULL, NULL,
                          xmlnode_get_attrib_ns(p->x, "from", NULL));
    xmlnode_put_attrib_ns(jp->x, "route", NULL, NULL,
...
/**
 * handle incoming &lt;route type='error'/&gt; packets, we get passed from
 * jabberd: most likely returned packets we sent to the client connection
 * manager - the user seems to have disconnected (or the component has crashed
 * *g*)
 *
 ...
 * @return always r_DONE
 */
result _js_routed_error_packet(instance i, dpacket p, jsmi si, xht ht,
                               jpacket jp, session s, udata u) {
    /* ooh, incoming routed errors in reference to this session, the session is
     * kaput */
    if (s != NULL) {
        s->sid = NULL; /* they generated the error, no use in sending there
                          anymore! */
        ...
        /* ... removing pass from udata, authentication should always fetch a
        new copy of the password from xdb u->pass = NULL; */ /* so they can't log back in */
        xmlnode_free(p->x);
        return r_DONE;
    }

    /* if this was a message, it should have been delievered to that session,
     * store offline */
    if (jp != NULL && jp->type == JPACKET_MESSAGE) {
        js_deliver_local(si, jp, ht); /* (re)deliver it locally again, should go
                                         to another session or offline */
        return r_DONE;
    }
 ...
 * @return always r_DONE
 */
result _js_routed_session_control_packet(instance i, dpacket p,
                                         xmlnode sc_session, jsmi si) {
    ...
    if (j_strcmp(action, "start") == 0) {
        ...
        if (s == NULL) {
            /* session start failed */
            log_warn(p->host, "Unable to create session %s", jid_full(p->id));
            xmlnode_put_attrib_ns(sc_session, "failed", NULL, NULL,
                                  messages_get(xmlnode_get_lang(sc_session),
                                               N_("Session Failed")));
        } else {
            /* confirm the session */
        ...
        /* notify modules */
        jid user_id =
            jid_new(p->p, xmlnode_get_attrib_ns(sc_session, "target", NULL));
        if (user_id != NULL) {
        ...
        /* unknown action */
        log_warn(p->host, "Session control packet with unknown action: %s",
                 action);
        xmlnode_put_attrib_ns(
            sc_session, "failed", NULL, NULL,
            messages_get(xmlnode_get_lang(sc_session),
                         N_("Unknown session control action")));
    }
...
/**
 * handle incoming &lt;route/&gt; packets, we get passed from the @link jabberd
 * XML router (jabberd)@endlink
 *
 * &lt;route/&gt; packets are normally received from the client connection
 * manager, that puts the stanzas received from the client inside the
 * &lt;route/&gt; packet to ensure, that the packet is routed to the session
 * manager component instead of directly to the destination address. This is
 * necessary as the session manager might want to trigger actions, drop the
 * stanza, or just has to ensure, that the client is not spoofing the source
 * address.
 *
 * The function checks if it is a special stanza used to establish sessions of a
 * user (in which case the client connection manager also tells the session
 * manager, how it has to identify the connection on packets it sends to the
 * client connection manager to get delivered to the client), or if it is a
 * forwarded stanza of a user for an existing connection.
 *
 * In case of a special stanza to establish sessions, this function has to check
 * which protocol is used by this session. This version of the session manager
 * supports two protocols for this. On the one hand the traditional jabberd 1.x
 * protocol (handled by _js_routed_session_packet()), and on the other hand the
 * session control protocol introduced by jabberd2 (handled by
 * _js_routed_session_control_packet()).
 *
 * In case of the traditional jabberd 1.x protocol establishing a session
 * consists of multiple steps and before the session is really established, the
 * authentication has to be done by the session manager (for the new jabberd2
 * protocol, authentication has to be done by the client connection manager
 * itself). For handling packets of the traditional protocol until
 * authentication has been done, the packet is passed to
 * _js_routed_auth_packet().
 *
 * If it is no session establishment packet, it is then checked if the stanza is
 * an error stanza, in which case it is passed to _js_routed_error_packet(). If
 * it is no error stanza, the session of this packet is identified. If there is
 * no such session, the stanza gets bounced back to the forwarder of it (which
 * normally will be the client connection manager, which then closes the
 * corresponding connection to the user). If there is such a session, the stanza
 * is passed together with the corresponding session data to the function
 * js_session_from().
 *
 * @param i the jsm instance we are running in
 * @param p the packet we should receive
...
result _js_routed_packet(instance i, dpacket p, jsmi si, xht ht) {
    ...
    char *type =
        NULL; /* the type of the route packet: NULL, auth, session, or error */
    session s = NULL; /* the session this packet is for */
    udata u = NULL;   /* the user's data */
    char *sc_sm =
        NULL; /* sc:sm attribute value for new session control protocol */
    ...
    if (child != NULL &&
        j_strcmp(xmlnode_get_localname(child), "session") == 0 &&
        j_strcmp(xmlnode_get_namespace(child), NS_SESSION) == 0)
    ...
    /* get user data */
    u = sc_sm == NULL ? js_user(si, p->id, ht)
                      : (udata)xhash_get(si->sc_sessions, sc_sm);
    /* is it a packet the c2s bounced back because there is no session anymore?
    ...
    if (u == NULL) {
        /* no user!?!?! */
        log_notice(p->host, "Bouncing packet intended for non-existant %s: %s",
                   sc_sm == NULL ? "user" : "session",
    ...
    if (jp == NULL) {
        ...
        log_notice(p->host, "Dropping an invalid or empty route packet: %s",
                   xmlnode_serialize_string(p->x, xmppd::ns_decl_list(), 0),
                   jid_full(p->id));
        xmlnode_free(p->x);
        return r_DONE;
    ...
    if (s != NULL) {
        ...
        log_notice(p->host, "Bouncing %s packet intended for session %s",
                   xmlnode_get_localname(jp->x), jid_full(p->id));
...
/**
 * handle packets we get passed from the @link jabberd XML router
 * (jabberd)@endlink
 *
 * The JSM component receives the packets from the XML router by a call to this
 * function. Therefore each incoming stanza (either from other servers received
 * by the @link dialback dialback component@endlink or from a client connection
 * received by the @link pthsock client connection manager@endlink) generates a
 * call of this function.
 *
 * This function ensures, that there is a hash table for the destination domain
 * containing all users, that are currently online on this host (it might be the
 * first packet addresses to this host in which case this hash is not present
 * before). After this is done, it checks if it is either a stanza, that has
 * been forwarded by an other component on the XML router (typically this is the
 * client connection manager) in which case the stanza is passed to
 * _js_routed_packet(), or if it is a non-forwarded packet, which typically
 * means it is an incoming stanza from either a remote server or another session
 * manager instance on the same XML router, or a stanza from a
 * transport/gateway. In the second case of a non-forwarded stanza, the stanza
 * is passed to the function js_deliver_local().
 *
 * @param i the jsm instance we are running in
 * @param p the packet we should receive
...
result js_packet(instance i, dpacket p, void *arg) {
    ...
    log_debug2(ZONE, LOGT_DELIVER, "(%X)incoming packet %s", si,
               xmlnode_serialize_string(p->x, xmppd::ns_decl_list(), 0));
    ...
    if ((ht = (xht)xhash_get(si->hosts, p->host)) == NULL) {
        ...
        log_debug2(ZONE, LOGT_DELIVER, "checking %X",
                   xhash_get(si->hosts, p->host));
    }
    ...
    /* normal server-server packet, should we make sure it's not spoofing us?
     * if so, if xhash_get(p->to->server) then bounce w/ security error */
...
/**
 * take a packet and deliver it either locally if it has ourself as the
 * destination or deliver it using jabberd if it is for a non-instance-local
 * host
 *
 * This function checks if the packet is local, and will then call
 * j_deliver_local() to deliver it. If it is not instance-local it will call
 * deliver() to deliver it using jabberd.
 *
 ...
 * @param p the packet to deliver
 * @param sending_s the sending session of the packet (used to select the
 * correct outgoing filter), NULL if packet is not related to a session
 */
void js_deliver(jsmi si, jpacket p, session sending_s) {
    ...
    /* only filter if packet is from a user, and if the packet is not to the
     * user himself */
    if (p->flag != PACKET_PASS_FILTERS_MAGIC && p->from->has_node() &&
        jid_cmpx(p->to, p->from, JID_USER | JID_SERVER) != 0) {
    ...
    log_debug2(ZONE, LOGT_DELIVER,
               "deliver(to[%s],from[%s],type[%d],packet[%s])", jid_full(p->to),
               jid_full(p->from), p->type,
               xmlnode_serialize_string(p->x, xmppd::ns_decl_list(), 0));
