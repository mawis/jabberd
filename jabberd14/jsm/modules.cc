 ...
 *
 * This is like js_mapi_register except that the call only applies to the
 * specified session.
 *
 * @param e the event type for which to register the callback
 ...
 *
 * checks if there is already an additional result in the mapi structure, if
 * not, it generates an iq result for the current query in the packet inside the
 * mapi strucutre (needs to by an iq query of type get or set).
 *
 * If name, prefix, and ns_iri is not NULL, an element in inserted in the iq
 * result having this data.
 *
...
void js_mapi_create_additional_iq_result(mapi m, const char *name,
    ...
    m->additional_result =
        jpacket_new(jutil_iqresult(xmlnode_dup(m->packet->x)));

    /* insert element in the result? */
    if (name != NULL) {
        m->additional_result->iq = xmlnode_insert_tag_ns(
            m->additional_result->x, name, prefix, ns_iri);
    }
}
 ...
 *
 * To let several modules generate a common result, mapi has been enhanced with
 * the additional_result element. Modules can create a result there, which jsm
 * will return after having called all modules. This function checks if such a
 * result has been created by the modules, and sends the result.
 *
 * @note Other than modules, that have to free the packet, that they processed,
 * if the return, that they handle the packet (usually this is just done by
 * sending the packet back as a result). This function will never free the
 * packet inside the mapi structure!
 *
 ...
 *
 * Addes callbacks to the ignore mask for a given packet type if they return
 * M_IGNORE.
 *
 ...
 * @param user the user data for the current session (or the sender for e_SERVER
 * if it is local), may be NULL
 ...
 * call all the module callbacks for a phase
 *
 * Only needed for the events es_SERIALIZE and es_DESERIALIZE. Other events can
 * use the shorter interface of js_mapi_call()
 *
 * Addes callbacks to the ignore mask for a given packet type if they return
 * M_IGNORE.
 *
 ...
 * if it is local), may be NULL
 * @param s the session for which to call the event, may be NULL
 * @param serialization_node the xmlnode to pass for es_SERIALIZE, and
 * es_DESERIALIZE event
 * @return 1 if the call was handled by a module, 0 if it wasn't handled
 */
