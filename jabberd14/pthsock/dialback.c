...
void pthsock_server_outread(mio s, int flags, void *arg, xmlnode x)
{
    ...
    case MIO_XML_ROOT:
        ...
        if(xmlnode_get_attrib(x,"xmlns:db") == NULL)
        {
            ...
            { /* Muahahaha!  you suck! *click* */
                log_notice(c->legacy_to,"Legacy server access denied due to configuration");
    ...
    case MIO_CLOSED:
        ...
        if(c->s == NULL && c->ipn != NULL)
        {
            ...
            mio_connect(ip, port, pthsock_server_outread, (void *)c, 20, MIO_CONNECT_XML);
            return;
        }

        log_debug(ZONE, "MIO_CLOSED: freeing c->p (watch for more than 1 of these in a row)");
...
void pthsock_server(instance i, xmlnode x)
{
    ...
    {
        mio m;
        m = mio_listen(5269,NULL,pthsock_server_inread,(void*)si, MIO_LISTEN_XML);
        mio_rate(m, j_atoi(xmlnode_get_attrib(xmlnode_get_tag(cfg,"rate"),"time"),5), j_atoi(xmlnode_get_attrib(xmlnode_get_tag(cfg,"rate"),"points"),25));
        mio_karma2(m, &k);
    }
