...
typedef struct smi_st {
    ...
    xht aliases;
    xht users;
    xht std_namespace_prefixes; /**< standard prefixes for using with
                                   xmlnode_get_tags() */
    xmlnode cfg;
    char *host;
...
static xmlnode pthsock_make_route(xmlnode x, char *to, char *from,
                                  char const *type) {
    xmlnode newx;
    newx = x ? xmlnode_wrap_ns(x, "route", NULL, NS_SERVER)
             : xmlnode_new_tag_ns("route", NULL, NS_SERVER);
...
static result pthsock_client_packets(instance id, dpacket p, void *arg) {
    ...
    if (p->type != p_ROUTE || fd == 0) {
        ...
        log_warn(p->host, "pthsock_client bouncing invalid %s packet from %s",
                 xmlnode_get_localname(p->x),
                 xmlnode_get_attrib_ns(p->x, "from", NULL));
        deliver_fail(p, N_("invalid client packet"));
        return r_DONE;
    }

    if ((cdcur = static_cast<cdata>(xhash_get(
             s__i->users, xmlnode_get_attrib_ns(p->x, "to", NULL)))) == NULL) {
    ...
    if (fd != cdcur->m->fd) {
        log_debug2(ZONE, LOGT_SESSION,
                   "Unexpected fd: packet for fd #%i while expecting fd #%i",
                   fd, cdcur->m->fd);
        m = NULL;
        ...
        log_debug2(ZONE, LOGT_SESSION,
                   "Unexpected resource: packet for %s, expecting %s",
                   p->id->get_resource().c_str(), cdcur->res);
    ...
    if (m == NULL) {
        if (j_strcmp(xmlnode_get_attrib_ns(p->x, "type", NULL), "error") == 0) {
            ...
            log_debug2(
                ZONE, LOGT_SESSION,
                "[%s] received Session close for non-existant session: %s",
                ZONE, xmlnode_get_attrib_ns(p->x, "from", NULL));
            xmlnode_free(p->x);
            return r_DONE;
        }

        log_debug2(ZONE, LOGT_SESSION,
                   "[%s] connection not found for %s, closing session", ZONE,
                   xmlnode_get_attrib_ns(p->x, "from", NULL));
    ...
    log_debug2(ZONE, LOGT_DELIVER,
               "[%s] %s has an active session, delivering packet", ZONE,
               xmlnode_get_attrib_ns(p->x, "from", NULL));
    if (j_strcmp(xmlnode_get_attrib_ns(p->x, "type", NULL), "error") == 0) {
        /* <route type="error" means we were disconnected */
        log_debug2(ZONE, LOGT_SESSION,
                   "[%s] closing down session %s at request of session manager",
                   ZONE, xmlnode_get_attrib_ns(p->x, "from", NULL));
        mio_write(
            m, NULL,
            ...
            "xml:lang='en'>Disconnected</text></stream:error></stream:stream>",
            -1);
        mio_close(m);
        xmlnode_free(p->x);
        return r_DONE;
    } else if (cdcur->state == state_UNKNOWN &&
               j_strcmp(xmlnode_get_attrib_ns(p->x, "type", NULL), "auth") ==
                   0) {
        /* look for our auth packet back */
        char *type =
            xmlnode_get_attrib_ns(xmlnode_get_firstchild(p->x), "type", NULL);
        char *id = xmlnode_get_attrib_ns(
            xmlnode_get_list_item(
                xmlnode_get_tags(p->x, "iq", s__i->std_namespace_prefixes), 0),
            "id", NULL);
        if ((j_strcmp(type, "result") == 0) &&
            j_strcmp(cdcur->auth_id, id) == 0) {
            /* update the cdata status if it's a successfull auth */
            xmlnode x;
            log_debug2(ZONE, LOGT_AUTH | LOGT_SESSION,
                       "[%s], auth for user successful", ZONE);
            /* notify SM to start a session */
            x = pthsock_make_route(NULL, jid_full(cdcur->session_id),
                                   cdcur->client_id, "session");
            log_debug2(ZONE, LOGT_SESSION,
                       "[%s] requesting Session Start for %s", ZONE,
                       xmlnode_get_attrib_ns(p->x, "from", NULL));
            deliver(dpacket_new(x), s__i->i);
        } else if (j_strcmp(type, "error") == 0) {
            log_record(jid_full(jid_user(cdcur->session_id)), "login", "fail",
                       "%s %s %s", mio_ip(cdcur->m),
                       xmlnode_get_attrib_ns(
                           xmlnode_get_list_item(
                               xmlnode_get_tags(p->x, "iq/error",
                                                s__i->std_namespace_prefixes),
                               0),
                           "code", NULL),
                       cdcur->session_id->get_resource().c_str());
        }
    } else if (cdcur->state == state_UNKNOWN &&
               j_strcmp(xmlnode_get_attrib_ns(p->x, "type", NULL), "session") ==
                   0) {
        /* got a session reply from the server */
        mio_wbq q;

        cdcur->state = state_AUTHD;
        log_record(jid_full(jid_user(cdcur->session_id)), "login", "ok",
                   "%s %s", mio_ip(cdcur->m),
                   cdcur->session_id->get_resource().c_str());
        /* change the host id */
        cdcur->session_id =
            jid_new(m->p, xmlnode_get_attrib_ns(p->x, "from", NULL));
        xmlnode_free(p->x);
        /* if we have packets in the queue, write them */
        while ((q = (mio_wbq)pth_msgport_get(cdcur->pre_auth_mp)) != NULL) {
            q->x = pthsock_make_route(q->x, jid_full(cdcur->session_id),
                                      cdcur->client_id, NULL);
    ...
    if (xmlnode_get_firstchild(p->x) == NULL ||
        xhash_get(s__i->users, xmlnode_get_attrib_ns(p->x, "to", NULL)) ==
            NULL) {
        xmlnode_free(p->x);
    } else {
        log_debug2(ZONE, LOGT_IO, "[%s] Writing packet to MIO: %s", ZONE,
                   xmlnode_serialize_string(xmlnode_get_firstchild(p->x),
...
static void pthsock_client_read(mio m, int flag, void *arg, xmlnode x,
    ...
    log_debug2(ZONE, LOGT_IO,
               "[%s] pthsock_client_read called with: m:%X flag:%d arg:%X",
               ZONE, m, flag, arg);
    switch (flag) {
        case MIO_CLOSED:

            log_debug2(ZONE, LOGT_IO,
                       "[%s] io_select Socket %d close notification", ZONE,
                       m->fd);
            xhash_zap(cd->si->users, cd->client_id);
            if (cd->state == state_AUTHD) {
                h = pthsock_make_route(NULL, jid_full(cd->session_id),
                                       cd->client_id, "error");
            ...
            if (cd->pre_auth_mp != NULL) {
                ...
                while ((q = (mio_wbq)pth_msgport_get(cd->pre_auth_mp)) !=
                       NULL) {
                    log_debug2(
                        ZONE, LOGT_IO,
                        "[%s] freeing unsent packet due to disconnect with no "
                        "auth: %s",
                        ZONE,
                        xmlnode_serialize_string(q->x, xmppd::ns_decl_list(),
                                                 0));
                    xmlnode_free(q->x);
                }
        ...
        case MIO_ERROR:
            ...
            break;
        case MIO_XML_ROOT:
            log_debug2(ZONE, LOGT_IO, "[%s] root received for %d: %s", ZONE,
                       m->fd,
            ...
            log_debug2(ZONE, LOGT_IO, "[%s] Recieved connection to: %s", ZONE,
                       jid_full(cd->sending_id));
            alias = static_cast<char *>(xhash_get(cd->si->aliases, to));
            alias = alias ? alias
                          : static_cast<char *>(
                                xhash_get(cd->si->aliases, "default"));
            ...
            if (j_strcmp(jid_full(cd->session_id), jid_full(cd->sending_id)) !=
                0)
                cd->aliased = 1;
            if (cd->aliased)
                log_debug2(ZONE, LOGT_SESSION, "[%s] using alias %s --> %s",
                           ZONE, jid_full(cd->sending_id),
                           jid_full(cd->session_id));
            ...
            if (j_strcmp(xmlnode_get_namespace(x), NS_FLASHSTREAM) == 0) {
                h = xmlnode_new_tag_pool_ns(xmlnode_pool(h), "stream", "flash",
                                            NS_FLASHSTREAM);
                xmlnode_put_attrib_ns(h, "id", NULL, NULL, cd->sid);
                xmlnode_put_attrib_ns(h, "from", NULL, NULL,
                                      jid_full(cd->session_id));
                xmlnode_put_attrib_ns(h, "xmlns", NS_XMLNS, NULL, NS_SERVER);
                xmlnode_put_attrib_ns(h, "stream", NS_XMLNS, "xmlns",
                                      NS_STREAM);
            }
            if (version >= 1) {
            ...
            if (cd->session_id == NULL) {
                ...
                mio_write(m, NULL,
                          ...
                          "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                          "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                          "xml:lang='en'>Did not specify a valid to "
                          "argument</text></stream:error></stream:stream>",
                          -1);
                mio_close(m);
            }
            ...
            if (version >= 1) {
                xmlnode features =
                    xmlnode_new_tag_ns("features", "stream", NS_STREAM);
                /* TLS possible on this connection? */
                if (mio_ssl_starttls_possible(
                        m, cd->session_id->get_domain().c_str())) {
                    xmlnode_insert_tag_ns(features, "starttls", NULL,
                                          NS_XMPP_TLS);
                }

                /* advertise registration of accounts? */
                if (cd->si->register_feature != 0) {
                    xmlnode_insert_tag_ns(features, "register", NULL,
                                          NS_REGISTER_FEATURE);
                }
        ...
        case MIO_XML_NODE:
            ...
            if (cd->state == state_UNKNOWN) {
                ...
                xmlnode q_auth = xmlnode_get_list_item(
                    xmlnode_get_tags(x, "auth:query",
                                     cd->si->std_namespace_prefixes),
                    0);
                xmlnode q_register = xmlnode_get_list_item(
                    xmlnode_get_tags(x, "register:query",
                                     cd->si->std_namespace_prefixes),
                    0);
                if (j_strcmp(xmlnode_get_localname(x), "starttls") == 0 &&
                    ...
                    if (mio_ssl_starttls_possible(
                        ...
                        /* ACK the start */
                        xmlnode proceed =
                            xmlnode_new_tag_ns("proceed", NULL, NS_XMPP_TLS);
                        mio_write(m, proceed, NULL, 0);

                        /* start TLS on this connection */
                        if (mio_xml_starttls(
                                m, 0, cd->session_id->get_domain().c_str()) !=
                            0) {
                            /* starttls failed */
                            mio_close(m);
                        }
                    ...
                    q = static_cast<mio_wbq>(
                        pmalloco(xmlnode_pool(x), sizeof(_mio_wbq)));
                    q->x = x;
                    pth_msgport_put(cd->pre_auth_mp,
                                    reinterpret_cast<pth_message_t *>(q));
                    return;
                } else if (q_auth != NULL) {
                    if (j_strcmp(xmlnode_get_attrib_ns(x, "type", NULL),
                                 "set") == 0) {
                        ...
                        xmlnode_put_attrib_ns(
                            xmlnode_get_list_item(
                                xmlnode_get_tags(
                                    ...
                                    cd->si->std_namespace_prefixes),
                                0),
                            "sid", NULL, NULL, cd->sid);
                        cd->auth_id =
                            pstrdup(m->p, xmlnode_get_attrib_ns(x, "id", NULL));
                        if (cd->auth_id == NULL) {
                            cd->auth_id =
                                pstrdup(m->p, "pthsock_client_auth_ID");
                            xmlnode_put_attrib_ns(x, "id", NULL, NULL,
                                                  "pthsock_client_auth_ID");
                        }
                        jid_set(cd->session_id,
                                xmlnode_get_data(xmlnode_get_list_item(
                                    xmlnode_get_tags(
                                        x, "auth:query/auth:username",
                                        cd->si->std_namespace_prefixes),
                                    0)),
                                JID_USER);
                        jid_set(cd->session_id,
                                xmlnode_get_data(xmlnode_get_list_item(
                                    xmlnode_get_tags(
                                        x, "auth:query/auth:resource",
                                        cd->si->std_namespace_prefixes),
                                    0)),
                                JID_RESOURCE);

                        x = pthsock_make_route(x, jid_full(cd->session_id),
                                               cd->client_id, "auth");
                        deliver(dpacket_new(x), cd->si->i);
                    } else if (j_strcmp(xmlnode_get_attrib_ns(x, "type", NULL),
                                        "get") == 0) {
                    ...
                    jid_set(cd->session_id,
                            xmlnode_get_data(xmlnode_get_list_item(
                                xmlnode_get_tags(
                                    x, "register:query/register:username",
                                    cd->si->std_namespace_prefixes),
                                0)),
                            JID_USER);
                    x = pthsock_make_route(x, jid_full(cd->session_id),
...
static void pthsock_client_listen(mio m, int flag, void *arg, xmlnode x,
                                  char *unused1, int unused2) {
...
static void _pthsock_client_timeout(xht h, const char *key, void *data,
                                    void *arg) {
    time_t timeout;
    cdata cd = (cdata)data;
    if (cd->state == state_AUTHD)
    ...
    log_debug2(ZONE, LOGT_IO, "[%s] timeout: %d, connect time %d: fd %d", ZONE,
               timeout, cd->connect_time, cd->m->fd);
    ...
    if (cd->connect_time < timeout) {
        mio_write(
            ...
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
            "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>Timeout "
            "waiting for authentication</text></stream:error></stream:stream>",
            -1);
...
static void _pthsock_client_heartbeat(xht h, const char *key, void *data,
                                      void *arg) {
    time_t skipbeat;
    cdata cd = (cdata)data;

    skipbeat = time(NULL) - cd->si->heartbeat;
    if (cd->state == state_AUTHD && cd->last_activity < skipbeat) {
...
static void _pthsock_client_shutdown(xht h, const char *key, void *data,
                                     void *arg) {
    cdata cd = (cdata)data;
    log_debug2(ZONE, LOGT_CLEANUP, "[%s] closing down user %s from ip: %s",
               ZONE, jid_full(cd->session_id), mio_ip(cd->m));
    mio_close(cd->m);
}
...
extern "C" void pthsock_client(instance i, xmlnode x) {
    ...
    xmlnode cur;
    int set_rate =
        0; /* Default false; did they want to change the rate parameters */
    int rate_time = 0;
    int rate_points = 0;
    char *host;
    struct karma *k = karma_new(i->p); /* Get new inialized karma */
    int set_karma =
        0; /* Default false; Did they want to change the karma parameters */
    xmlnode_vector item;
    ...
    xhash_put(s__i->std_namespace_prefixes, "auth",
              const_cast<char *>(NS_AUTH));
    xhash_put(s__i->std_namespace_prefixes, "pthcsock",
              const_cast<char *>(NS_JABBERD_CONFIG_PTHCSOCK));
    xhash_put(s__i->std_namespace_prefixes, "register",
              const_cast<char *>(NS_REGISTER));

    /* get the config */
    xc = xdb_cache(i);
    s__i->cfg = xdb_get(xc, jid_new(xmlnode_pool(x), "config@-internal"),
                        NS_JABBERD_CONFIG_PTHCSOCK);
    ...
    for (cur = xmlnode_get_firstchild(s__i->cfg); cur != NULL;
        ...
        if (j_strcmp(xmlnode_get_localname(cur), "alias") == 0) {
            ...
            k->val = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                                xmlnode_get_tags(cur, "pthcsock:init",
                                                 s__i->std_namespace_prefixes),
                                0)),
                            KARMA_INIT);
            k->max = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                                xmlnode_get_tags(cur, "pthcsock:max",
                                                 s__i->std_namespace_prefixes),
                                0)),
                            KARMA_MAX);
            k->inc = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                                xmlnode_get_tags(cur, "pthcsock:inc",
                                                 s__i->std_namespace_prefixes),
                                0)),
                            KARMA_INC);
            k->dec = j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                                xmlnode_get_tags(cur, "pthcsock:dec",
                                                 s__i->std_namespace_prefixes),
                                0)),
                            KARMA_DEC);
            k->restore =
                j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                           xmlnode_get_tags(cur, "pthcsock:restore",
                                            s__i->std_namespace_prefixes),
                           0)),
                       KARMA_RESTORE);
            k->penalty =
                j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                           xmlnode_get_tags(cur, "pthcsock:penalty",
                                            s__i->std_namespace_prefixes),
                           0)),
                       KARMA_PENALTY);
            k->reset_meter =
                j_atoi(xmlnode_get_data(xmlnode_get_list_item(
                           xmlnode_get_tags(cur, "pthcsock:resetmeter",
                                            s__i->std_namespace_prefixes),
                           0)),
                       KARMA_RESETMETER);
    ...
    /* start listening */
    item = xmlnode_get_tags(s__i->cfg, "pthcsock:ip",
                            s__i->std_namespace_prefixes);
    for (xmlnode_vector::iterator iter = item.begin(); iter != item.end();
         ++iter) {
        mio m;
        m = mio_listen(j_atoi(xmlnode_get_attrib_ns(*iter, "port", NULL), 5222),
                       xmlnode_get_data(*iter), pthsock_client_listen,
                       (void *)s__i, MIO_LISTEN_XML);
        if (m == NULL)
            return;
    ...
    item = xmlnode_get_tags(s__i->cfg, "pthcsock:tls",
                            s__i->std_namespace_prefixes);
    if (item.size() == 0) {
        item = xmlnode_get_tags(s__i->cfg, "pthcsock:ssl",
                                s__i->std_namespace_prefixes);
        if (item.size() > 0)
            log_warn(
                NULL,
                "Processing legacy <ssl/> element(s) inside pthsock_client "
                "configuration. The element has been renamed to <tls/>.");
    }
    ...
    for (xmlnode_vector::iterator iter = item.begin(); iter != item.end();
        ...
        m = mio_listen(j_atoi(xmlnode_get_attrib_ns(*iter, "port", NULL), 5223),
                       xmlnode_get_data(*iter), pthsock_client_listen,
                       (void *)s__i, mh);
        if (m == NULL)
            return;
    ...
    if (s__i->heartbeat) {
        log_debug2(ZONE, LOGT_INIT, "Registering heartbeat: %d",
                   s__i->heartbeat);
