...
namespace xmppd {
	...
	class connected_sockets {
		 ...
		 * @param bufferlen for MIO_BUFFER events this is the length of the read data
		 */
	...
	class socks5stub {
		 ...
		 */
		sigc::signal<void, socks5stub*>& event_protocol_done() { return protocol_done; }
		 ...
		 * @return reference to the signal
		 */
		sigc::signal<void, socks5stub*>& event_disconnected() { return disconnected; }
		 ...
		 *
		 * The signal handler gets passed the instance that represents the client connection.
		 */
		 ...
		 *
		 * This signal handler gets passed the intance that represents the client connection.
		 */
		 ...
		 */
		enum state {
		    state_connected,		/**< connection just accepted, no (complete) version identifier from client received yet */
		    state_accepted_auth,	/**< received version identifier from client and client supports the X'00' auth */
		    state_accepted_connect,	/**< received a connect request from the client */
		    state_error			/**< there has been a problem with this connection, we had to reject it */
		};
		...
		/**
		 * data that has been received on this socket, but that is not yet consumed/processed
		 */
		 ...
		 *
		 * This is the 'destination address' the connecting entity used. The protocol specifies, that this is the
		 * sha1 hash of the two JIDs we will interconnect and the session id. Therefore this id is not unique, normally
		 * we will get twice the same id.
		 *
	 ...
	 *
	 * This class implements a proxy to be used with XEP-0065. The main use-case currently is
	 * to allow users to establish a file transfer if both sides are behind a NAT router or
	 * if one side is IPv4-only and the other side is IPv6-only.
	 */
	class proxy65 : public instance_base {
	    ...
	    private:
		/**
		 * Signal handler that handles disconnected connections that where currently doing the socks5 protocol
		 *
		...
		/**
		 * the connections, that have finished the socks5 protocol but are not yet interconnected
		 *
		 * Key is the connection id, value is a pair of the socks5stub instance and the time when the socks5 protocol has been finished
		 */
		...
		/**
		 * callback we register with mio for notifying us, that a new connection has been accepted
		 *
		 ...
		 */
		static void conn_accepted_wrapper(mio m, int state, void *arg, xmlnode unused1, char* unused2, int unused3);
