 ...
 *
 * This is a module for jabberd14 that implements a proxy for XEP-0065
 * bytestreams.
 */
...
proxy65::proxy65(instance i, xmlnode x) : instance_base(i, x) {
    ...
    xhash_put(std_namespace_prefixes, "bytestreams",
              const_cast<char *>(NS_BYTESTREAMS));

    // open the socket to listen on
    // XXX IP and port needs to be configurable, fixed for now
    mio_listen(6565, "::", proxy65::conn_accepted_wrapper, this,
               MIO_LISTEN_RAW);
}

void proxy65::conn_accepted_wrapper(mio m, int state, void *arg,
...
void proxy65::connection_accepted(mio m) {
    log_debug2(ZONE, LOGT_IO, "New connection has been accepted on fd #%i",
               m->fd);
    ...
    // connect signals
    stub->event_disconnected().connect(
        sigc::mem_fun(*this, &proxy65::connecting_connection_disconnected));
    stub->event_protocol_done().connect(
        sigc::mem_fun(*this, &proxy65::connecting_connection_protocol_done));
...
void proxy65::connecting_connection_disconnected(socks5stub *stub) {
    ...
    log_debug2(
        ZONE, LOGT_IO,
        "XXX deleting stub in proxy65::connecting_connection_disconnected");
    delete stub;
}

void proxy65::connecting_connection_protocol_done(socks5stub *stub) {
    log_debug2(ZONE, LOGT_IO,
               "Connection has finished the SOCKS5 part of the protocol");

    connecting_connections.erase(stub);

    waiting_connections.insert(
        std::pair<std::string, std::pair<socks5stub *, time_t>>(
            stub->get_connection_id(),
            std::pair<socks5stub *, time_t>(stub, std::time(NULL))));
}
...
result proxy65::iq_bytestreams(jpacket p) {
    ...
    switch (jpacket_subtype(p)) {
        case JPACKET__GET:
            ...
            x = xmlnode_insert_tag_ns(p->iq, "streamhost", NULL,
                                      NS_BYTESTREAMS);
            xmlnode_put_attrib_ns(x, "jid", NULL, NULL,
                                  get_instance_id().c_str());
            xmlnode_put_attrib_ns(x, "host", NULL, NULL,
                                  get_instance_id().c_str());
            xmlnode_put_attrib_ns(x, "port", NULL, NULL,
                                  "6565"); // XXX should get configured
            deliver(p->x);
            return r_DONE;
        case JPACKET__SET:
...
result proxy65::iq_bytestreams_activate(jpacket p) {
    log_debug2(ZONE, LOGT_EXECFLOW, "Got request to activate a bytestream: %s",
    ...
    xmlnode_vector activate =
        xmlnode_get_tags(p->iq, "bytestreams:activate", std_namespace_prefixes);
    if (!activate.size()) {
        bounce_stanza(p->x, XTERROR_BAD);
        return r_DONE;
    }
    ...
    for (iter = waiting_connections.begin(); iter != waiting_connections.end();
         ++iter) {
    ...
    log_debug2(ZONE, LOGT_EXECFLOW,
               "We have %i waiting connections with hash %s",
               matched_connections.size(), session_id.c_str());

    // we should have found exactly two connections
    if (matched_connections.size() != 2) {
        std::vector<socks5stub *>::iterator iter;
        for (iter = matched_connections.begin();
             iter != matched_connections.end(); ++iter) {
            delete *iter;
        }
    ...
    connected_sockets *new_conn = new connected_sockets(
        matched_connections[0], matched_connections[1], identifier.str());
    new_conn->event_closed().connect(
        sigc::mem_fun(*this, &proxy65::active_connection_disconnected));
    active_connections.insert(
        new_conn); // XXX protect this for the case that it
                   // diconnectes before it gets inserted
...
void proxy65::active_connection_disconnected(connected_sockets *conn) {
    ...
    log(notice) << "Reflected bytestream has been closed after transfering "
                << conn->get_forwarded_traffic()
                << " B: " << conn->get_identifier();
    delete conn;
}
...
socks5stub::~socks5stub() { close_socket(); }
...
void socks5stub::mio_event_wrapper(mio m, int state, void *arg, xmlnode unused1,
    ...
    switch (state) {
        ...
        case MIO_BUFFER:
            if (bufferlen > 0 && buffer != NULL)
                static_cast<socks5stub *>(arg)->on_data(
                    std::string(buffer, bufferlen));
            break;
    }
}
...
void socks5stub::on_data(const std::string &received_data) {
    ...
    log_debug2(ZONE, LOGT_IO,
               "socks5stub::on_data(), state is %i, unprocessed bytes are %i",
               current_state, unprocessed_data.length());

    // what we do depends on our current state
    switch (current_state) {
        case state_connected:
            // if no data yet available, we cannot process anything (should not
            // happen as we just received data)
            ...
            if (unprocessed_data.length() >=
                (size_t)(2 + (uint8_t)unprocessed_data[1])) {
                // yes we received version and all methods the client proposes

                // check if client supports 'no authentication'
                if (unprocessed_data.substr(2, unprocessed_data[1])
                        .find(static_cast<char>(0)) != std::string::npos) {
        ...
        case state_accepted_auth:
            ...
            // check version
            if (unprocessed_data[0] != 5) {
                mio_write(m, NULL, "\x05\x07\0x00\x01\x00\x00\x00\x00\x00\x00",
                          10);
                mio_close(m);
                current_state = state_error;
                unprocessed_data = "";
            ...
            if (unprocessed_data.length() <
                (size_t)(7 + (uint8_t)unprocessed_data[4]))
                return;
            ...
            // change the connection_id to lowercase
            {
                ...
                std::transform(connection_id.begin(), connection_id.end(),
                               connection_id.begin(), to_lowercase);
            }

            // send reply message
            mio_write(m, NULL,
                      (std::string("\x05\x00\x00\x03", 4) +
                       unprocessed_data.substr(4, unprocessed_data[4] + 3))
                          .c_str(),
                      7 + unprocessed_data[4]);
        ...
        case state_accepted_connect:
            // we are only collecting the data, the responsibility for the
            // connection gets transfered
            break;
        case state_error:
...
connected_sockets::connected_sockets(socks5stub *socket1, socks5stub *socket2,
    ...
    : identifier(identifier), forwarded_traffic(0) {
    // sanity checks
    if (!socket1 || !socket2)
        throw std::invalid_argument(
            "NULL socket passed to connected_sockets constructor");
    if (!socket1->m || !socket2->m)
        throw std::invalid_argument("socks5stub passed to connected_sockets "
                                    "constructor, that has no connection");
    ...
    log_debug2(ZONE, LOGT_IO, "connecting two sockets together: #%i #%i",
               this->sockets[0]->fd, this->sockets[1]->fd);
    ...
    if (socket1->unprocessed_data.length() > 0) {
        mio_write(this->sockets[1], NULL, socket1->unprocessed_data.c_str(),
                  socket1->unprocessed_data.length());
        socket1->unprocessed_data = "";
    }
    if (socket2->unprocessed_data.length() > 0) {
        mio_write(this->sockets[0], NULL, socket2->unprocessed_data.c_str(),
                  socket2->unprocessed_data.length());
...
void connected_sockets::mio_event_wrapper(mio m, int state, void *arg,
                                          xmlnode unused1, char *buffer,
                                          int bufferlen) {
    // sanity check
    if (arg == NULL || m == NULL)
        return;
    ...
    switch (state) {
        ...
        case MIO_BUFFER:
            if (bufferlen > 0 && buffer != NULL)
                static_cast<connected_sockets *>(arg)->on_data(
                    socketindex, std::string(buffer, bufferlen));
            break;
    }
}
...
void connected_sockets::on_data(int socketindex,
    ...
    if (sockets[othersocket]) {
        ...
        mio_write(sockets[othersocket], NULL, received_data.c_str(),
