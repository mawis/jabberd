 ...
 *
 * The dialback directory contains the module, that implements server to server
 * (s2s) connections.
 */
...
typedef struct db_struct {
    ...
    xht out_connecting; /**< where unvalidated in-progress connections are, key
                           is to/from */
    xht out_ok_db; /**< hash table of all connected dialback hosts, key is same
                      to/from */
    xht in_id; /**< all the incoming connections waiting to be checked, rand id
                  attrib is key */
    xht in_ok_db;   /**< all the incoming dialback connections that are ok,
                       ID\@to/from is key */
    xht hosts_xmpp; /**< hash containing the XMPP version configuration for
                       peers */
    xht hosts_tls;  /**< hash containing the STARTTLS configuration for peers */
    xht hosts_auth; /**< hash containing the authentiction configuration for
                       peers */
    char *secret;   /**< our dialback secret */
    int timeout_packets; /**< configuration option &lt;queuetimeout/&gt;: how
                            long should stanzas wait for an established
                            connection */
    int timeout_idle; /**< configuration option &lt;idletimeout/&gt;: after how
                         many seconds an authorized connection should be timed
                         out */
    int timeout_auth; /**< configuration option &lt;authtimeout/&gt;: after how
                         many seconds a connection should be closed, if it is
                         not authorized for any domain */
    xht std_ns_prefixes; /**< standard prefixes used inside the dialback
                            component for xpath expressions */
    xdbcache xc; /**< pointer to the ::xdbcache_cache structure used to access
                    the ACL and configuration */
} * db, _db;
...
void dialback_in_read(mio s, int flags, void *arg, xmlnode x, char *unused1,
                      int unused2);
 ...
 */
typedef enum {
    not_requested, /**< there was no packet yet, for that we want to request
                      doing dialback (just sending db:verifys), and we could not
                      yet send them  */
    could_request, /**< there was no packet yet, that requested doing dialback,
                      but we could send out dialback requests */
    want_request,  /**< we want to send a dialback request */
    sent_request   /**< we did sent a dialback request */
} db_request;

/**
 * enumeration of connection establishment states an outgoing connection can
 * have
 *
 * used for more detailed logging of failed connections
 */
typedef enum {
    created,    /**< outgoing connection request created, but not yet started to
                   connect */
    ...
    waiting_features, /**< we are waiting for the stream features on a XMPP1.0
                         connection */
 ...
 */
typedef struct {
    char *ip; /**< where to connect to (list of comma separated addresses of the
                 format [ip]:port, [ip], ip:port, or ip) */
    ...
    jid key;   /**< destination and source for this connection, format: dest/src
                */
    xmlnode
        verifies; /**< waiting db:verify elements we have to send to the peer */
    ...
    /* original comment: for that short time when we're connected and open, but
     * haven't auth'd ourselves yet */
    int xmpp_version;    /**< version the peer supports, -1 not yet known, 0
                            preXMPP */
    int settings_failed; /**< 1 if the connection has been droped as configured
                            settings where not fulfilled (e.g. TLS required), 0
                            else */
    char *stream_id;     /**< the stream id the connected entity assigned */
    db_request db_state; /**< if we want to send a <db:result/> and if we
                            already did */
    db_connection_state connection_state; /**< how far did we proceed in
                                             connecting to the other host */
    std::ostringstream
        *connect_results; /**< result messages for the connection attempts */
    struct {
...
typedef struct dbic_struct {
    ...
    char *other_domain; /**< who the other end told to be in its stream root
                           from attribute (if present) */
