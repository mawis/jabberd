 ...
 *
 * This is where the server to server connection manager handles outgoing
 * connections.
 *
 ...
 * - An initial db:result element to tell the peer, that we want to authorize
 * for the use of a domain, that the peer should verify that we are allowed to
 * use this sending domain - and watch for the results to this
 * - db:verify elements to verify if a peer is allowed to use a domain as
 *   sender - and watch for the results
 * - After we are authorized by the peer to send stanzas from a domain, we send
 * them.
 ...
 *
 * Tell mio to connect to the peer and make dialback_out_read() the first mio
 * handler
 *
...
void dialback_out_connect(dboc c) {
    ...
    log_debug2(ZONE, LOGT_IO, "Attempting to connect to %s at %s",
               jid_full(c->key), ip);
...
/**
 * helper function used to register deleting a std::ostringstream as a
 * pool_cleaner function
 *
...
dboc dialback_out_connection(db d, jid key, char *ip, db_request db_state) {
    ...
    if ((c = static_cast<dboc>(xhash_get(d->out_connecting, jid_full(key)))) !=
        NULL) {
        /* db:request now wanted? */
        if (db_state == want_request) {
            if (c->db_state == not_requested) {
                log_debug2(ZONE, LOGT_IO,
                           "packet for existing connection: state change "
                           "not_requested -> want_request");
                c->db_state = want_request;
            } else if (c->db_state == could_request) {
                /* send <db:result/> to request dialback */
                xmlnode db_result =
                    xmlnode_new_tag_ns("result", "db", NS_DIALBACK);
                xmlnode_put_attrib_ns(db_result, "to", NULL, NULL,
                                      c->key->get_domain().c_str());
                xmlnode_put_attrib_ns(db_result, "from", NULL, NULL,
                                      c->key->get_resource().c_str());
                xmlnode_insert_cdata(
                    db_result,
                    dialback_merlin(xmlnode_pool(db_result), c->d->secret,
                                    c->key->get_domain().c_str(),
                                    c->key->get_resource().c_str(),
                                    c->stream_id),
                    -1);
 ...
 */
static char const *
dialback_out_connection_state_string(db_connection_state state) {
    switch (state) {
        case created:
...
/**
 * handle failed connection attempts, bounce pending stanzas and db:verify
 * elements
 *
 * either we're connected, or failed, or something like that, but the connection
 * process is kaput
 *
...
void dialback_out_connection_cleanup(dboc c) {
    ...
    /* if there was never any ->m set but there's a queue yet, then we probably
     * never got connected, just make a note of it */
    if (c->m == NULL && c->q != NULL) {
        log_notice(c->d->i->id, "failed to establish connection to %s, %s: %s",
                   c->key->get_domain().c_str(),
                   dialback_out_connection_state_string(c->connection_state),
                   connect_results.c_str());
    }
    ...
    if (cur != NULL) {
        ...
        if (c->settings_failed) {
            errmsg << messages_get(
                lang, N_("Failed to deliver stanza to other server because of "
                         "configured stream parameters."));
        } else {
            errmsg << messages_get(
                lang, N_("Failed to deliver stanza to other server while "));
            errmsg << messages_get(lang,
                                   N_(dialback_out_connection_state_string(
                                       c->connection_state)))
                   << ": " << connect_results;
        }
    ...
    while (cur != NULL) {
        ...
        deliver_fail(
            ...
            bounce_reason
                ? bounce_reason
                : messages_get(lang,
                               N_("Could not send stanza to other server")));
        cur = next;
    ...
    for (x = xmlnode_get_firstchild(c->verifies); x != NULL;
        ...
        dialback_in_verify(
            c->d,
            xmlnode_dup(x)); /* it'll take these verifies and trash them */
    }
 ...
 *
 * (packets to our instances address are not handled here, but in
 * dialback_in_verify())
 *
 * We have to:
 * - revert some magic we are using to talk to the dns resolver for db:verify
 * packets
 * - check if there is already a connection and establish one else
 * - send or queue the packet (depending if we already authorized and if it's a
 * db:verify)
 *
 * @param d the dialback instance
 * @param x the packet
...
void dialback_out_packet(db d, xmlnode x, char *ip) {
    ...
    if (to == NULL || from == NULL) {
        log_warn(d->i->id, "dropping packet, invalid to or from: %s",
                 xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));
        xmlnode_free(x);
        return;
    }

    log_debug2(ZONE, LOGT_IO, "dbout packet[%s]: %s", ip,
               xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));

    /* db:verify packets come in with us as the sender */
    if ((j_strcmp(xmlnode_get_localname(x), "verify") == 0 &&
         j_strcmp(xmlnode_get_namespace(x), NS_DIALBACK) == 0) ||
        j_strcmp(from->get_domain().c_str(), d->i->id) == 0) {
        verify = 1;
        /* fix the headers, restore the real from */
        /* (I think we wouldn't need to from/ofrom thing anymore because we have
         * dnsqueryby, that we need for s2s clustering) */
    ...
    if (md != NULL) {
        /* if we've got an ip sent, and a connected host, we should be
         * registered! */
        if (ip != NULL)
    ...
    c = dialback_out_connection(d, key, dialback_ip_get(d, key, ip),
                                verify ? not_requested : want_request);
    log_debug2(ZONE, LOGT_IO, "got connection %x for request %s (%s)", c,
               jid_full(key), verify ? "not_requested" : "want_request");

    /* verify requests can't be queued, they need to be sent outright */
    if (verify) {
        if (c == NULL) {
            jutil_tofrom(x);          /* pretend it bounced */
            dialback_in_verify(d, x); /* no connection to send db:verify to,
                                         bounce back to in to send failure */
            return;
        }
...
/**
 * handle the events (incoming stanzas) on an outgoing dialback socket, which
 * isn't much of a job
 *
 * The only packets we have to expect on an outgoing dialback socket are
 * db:verify and maybe stream:error
 *
 ...
 */
void dialback_out_read_db(mio m, int flags, void *arg, xmlnode x, char *unused1,
                          int unused2) {
    db d = (db)arg;

    if (flags != MIO_XML_NODE)
        return;
    ...
    if (j_strcmp(xmlnode_get_localname(x), "error") == 0 &&
        ...
        switch (errstruct->severity) {
            case normal:
                log_debug2(ZONE, LOGT_IO,
                           ...
                           mio_ip(m), errmsg.str().c_str());
                break;
            case configuration:
            case feature_lack:
            case unknown:
                log_warn(d->i->id,
                         "received stream error on outgoing db conn to %s: %s",
                         mio_ip(m), errmsg.str().c_str());
                break;
            case error:
            default:
        ...
        mio_write(
            ...
            "xml:lang='en'>Received data on a send-only socket. You are not "
            "Allowed to send data on this socket!</text></stream:error>",
            -1);
    }
...
static void dialback_out_send_verifies(mio m, dboc c) {
    ...
    for (cur = xmlnode_get_firstchild(c->verifies); cur != NULL;
...
void dialback_out_read(mio m, int flags, void *arg, xmlnode x, char *unused1,
    ...
    log_debug2(ZONE, LOGT_IO, "dbout read: fd %d flag %d key %s", m->fd, flags,
               jid_full(c->key));

    switch (flags) {
        case MIO_NEW:
            log_debug2(ZONE, LOGT_IO,
                       "NEW outgoing server socket connected at %d", m->fd);
            ...
            cur = xstream_header(c->key->get_domain().c_str(),
                                 c->key->get_resource().c_str());
            xmlnode_hide_attrib_ns(
                cur, "id", NULL); /* no, we don't need the id on this stream */
            if (j_strcmp(static_cast<char *>(xhash_get_by_domain(
                             c->d->hosts_auth, c->key->get_domain().c_str())),
                         "sasl") != 0)
                xmlnode_put_attrib_ns(
                    cur, "db", "xmlns", NS_XMLNS,
                    NS_DIALBACK); /* flag ourselves as dialback capable */
            if (j_strcmp(static_cast<char *>(xhash_get_by_domain(
                             c->d->hosts_xmpp, c->key->get_domain().c_str())),
                         "no") != 0) {
                /* we flag support for XMPP 1.0 */
                xmlnode_put_attrib_ns(cur, "version", NULL, NULL, "1.0");
            }
            xmlnode_put_attrib_ns(cur, "check", "loop", NS_JABBERD_LOOPCHECK,
                                  dialback_get_loopcheck_token(c->d));
            mio_write_root(m, cur, 0);
            return;

        case MIO_XML_ROOT:
            log_debug2(ZONE, LOGT_IO, "Incoming root %s",
                       xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));
            if (c->connection_state != sasl_success)
                c->connection_state = got_streamroot;
            else {
                if (dialback_check_settings(c->d, m,
                                            c->key->get_domain().c_str(), 1, 1,
                                            c->xmpp_version) == 0) {
                    c->settings_failed = 1;
                    break;
                }
            ...
            /* remember the stream id the connected entity assigned ... required
             * to do dialback */
            c->stream_id = pstrdup(c->p, xmlnode_get_attrib_ns(x, "id", NULL));
            if (c->stream_id == NULL) {
                mio_write(
                    m, NULL,
                    "<stream:error><invalid-id "
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                    "xml:lang='en'>You are missing the id attribute in your "
                    "stream header!</text></stream:error>",
                    -1);
                mio_close(m);
                break;
            }
            ...
            if (xhash_get(c->d->in_id, c->stream_id) != NULL) {
                log_alert(
                    c->key->get_domain().c_str(),
                    "hostname maps back to ourselves!- No service defined "
                    "for this hostname, can not handle request. Check "
                ...
                mio_write(
                    ...
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                    "xml:lang='en'>Mirror Mirror on the wall (we connected "
                    "to ourself)</text></stream:error>",
                    -1);
                mio_close(m);
                break;
            }
            ...
            /* check version */
            c->xmpp_version =
                j_atoi(xmlnode_get_attrib_ns(x, "version", NULL), 0);
            try {
            ...
            if (c->xmpp_version < 1 && !c->flags.db) {
                ...
                log_notice(c->key->get_domain().c_str(),
                           "Legacy server access denied");
                mio_write(
                    m, NULL,
                    "<stream:error><not-authorized "
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                    "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                    "xml:lang='en'>Legacy Access Denied!</text></stream:error>",
                    -1);
                mio_close(m);
                break;
            }

            /* create and send our result request to initiate dialback for non
             * XMPP-sessions (XMPP has to wait for stream features) */
            if (c->xmpp_version < 1) {
                /* check the require-tls setting */
                if (dialback_check_settings(c->d, m,
                                            c->key->get_domain().c_str(), 1, 0,
                                            c->xmpp_version) == 0) {
                    c->settings_failed = 1;
                    break;
                }
                ...
                if (j_strcmp(static_cast<char *>(
                                 xhash_get_by_domain(c->d->hosts_auth, "sasl")),
                             "sasl") == 0) {
                    log_warn(
                        c->d->i->id,
                        "pre-XMPP 1.0 peer %s cannot support SASL, but we are "
                    ...
                    mio_write(
                        ...
                        "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                        "xml:lang='en' "
                        "xmlns='urn:ietf:params:xml:ns:xmpp-streams'>Sorry, "
                        "but we require SASL auth, but you seem to only "
                        "support dialback.</text></stream:error>",
                        -1);
                    mio_close(m);
                    break;
                }
                ...
                log_debug2(ZONE, LOGT_IO,
                           "pre-XMPP 1.0 stream could now send <db:result/>");
                if (c->db_state == want_request) {
                    /* send db request */
                    cur = xmlnode_new_tag_ns("result", "db", NS_DIALBACK);
                    xmlnode_put_attrib_ns(cur, "to", NULL, NULL,
                                          c->key->get_domain().c_str());
                    xmlnode_put_attrib_ns(cur, "from", NULL, NULL,
                                          c->key->get_resource().c_str());
                    xmlnode_insert_cdata(
                        cur,
                        dialback_merlin(xmlnode_pool(cur), c->d->secret,
                                        c->key->get_domain().c_str(),
                                        c->key->get_resource().c_str(),
                                        c->stream_id),
                        -1);
                    ...
                    log_debug2(
                        ZONE, LOGT_IO,
                        "... and we wanted ... and we sent <db:result/>");
            ...
            /* well, we're connected to a dialback server, we can at least send
             * verify requests now */
            c->m = m;
            if (c->xmpp_version < 1) {
                for (cur = xmlnode_get_firstchild(c->verifies); cur != NULL;
        ...
        case MIO_XML_NODE:
            ...
            if (j_strcmp(xmlnode_get_localname(x), "error") == 0 &&
                ...
                switch (errstruct->severity) {
                    case normal:
                        log_debug2(
                            ZONE, LOGT_IO,
                            "stream error on outgoing%s conn to %s (%s): %s",
                            c->xmpp_version < 0
                                ? ""
                                : c->xmpp_version == 0 ? " preXMPP"
                                                       : " XMPP1.0",
                            mio_ip(m), jid_full(c->key), errstr.c_str());
                        break;
                    case configuration:
                    case feature_lack:
                    case unknown:
                        log_warn(c->d->i->id,
                                 "received stream error on outgoing%s conn to "
                                 "%s (%s): %s",
                                 c->xmpp_version < 0
                                     ? ""
                                     : c->xmpp_version == 0 ? " preXMPP"
            ...
            if (j_strcmp(xmlnode_get_localname(x), "features") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_STREAM) == 0) {
                ...
                if (c->connection_state == sasl_success) {
                    mio_reset(m, dialback_out_read_db,
                              (void *)(c->d)); /* different handler now */
                    md =
                        dialback_miod_new(c->d, m); /* set up the mio wrapper */
                    dialback_miod_hash(md, c->d->out_ok_db,
                                       c->key); /* this registers us to get
                                                   stuff directly now */
                ...
                if (xmlnode_get_list_item(
                        xmlnode_get_tags(x, "tls:starttls",
                                         c->d->std_ns_prefixes),
                        0) != NULL) {
                    /* don't start if forbidden by caller (configuration) */
                    if (j_strcmp(
                            static_cast<char *>(xhash_get_by_domain(
                                c->d->hosts_tls, c->key->get_domain().c_str())),
                            "no") == 0) {
                        log_notice(
                            c->d->i->id,
                            "Server %s advertized starttls, but disabled by "
                            "our configuration.",
                            c->key->get_domain().c_str());
                    } else if (mio_ssl_starttls_possible(
                                   m, c->key->get_resource().c_str())) {
                        /* our side is prepared for starttls */
                        xmlnode starttls = NULL;

                        /* request to start tls on this connection */
                        log_debug2(ZONE, LOGT_IO,
                                   "requesting starttls for an outgoing "
                                   "connection to %s",
                                   c->key->get_domain().c_str());

                        starttls =
                            xmlnode_new_tag_ns("starttls", NULL, NS_XMPP_TLS);
                ...
                mechanisms = xmlnode_get_list_item(
                    xmlnode_get_tags(x, "sasl:mechanisms",
                                     c->d->std_ns_prefixes),
                    0);
                if (mechanisms != NULL) {
                    ...
                    for (mechanism = xmlnode_get_firstchild(mechanisms);
                         mechanism != NULL;
                         mechanism = xmlnode_get_nextsibling(mechanism)) {
                        if (xmlnode_get_type(mechanism) != NTYPE_TAG)
                            continue;
                        if (j_strcasecmp(xmlnode_get_data(mechanism),
                                         "EXTERNAL") != 0)
                            continue;

                        /* SASL EXTERNAL is supported: use it */
                        log_debug2(ZONE, LOGT_IO,
                                   "SASL EXTERNAL seems to be supported: %s",
                                   xmlnode_serialize_string(
                                       mechanisms, xmppd::ns_decl_list(), 0));
                        auth = xmlnode_new_tag_ns("auth", NULL, NS_XMPP_SASL);
                        xmlnode_put_attrib_ns(auth, "mechanism", NULL, NULL,
                                              xmlnode_get_data(mechanism));

                        /* add our id as base64 encoded CDATA */
                        base64_source_domain_len =
                            (c->key->get_resource().length() + 2) / 3 * 4 + 1;
                        base64_source_domain = static_cast<char *>(pmalloco(
                            xmlnode_pool(x), base64_source_domain_len));
                        base64_encode(
                            (unsigned char *)c->key->get_resource().c_str(),
                            c->key->get_resource().length(),
                            base64_source_domain, base64_source_domain_len);
                        xmlnode_insert_cdata(auth, base64_source_domain, -1);

                        /* send the initial exchange */
                        log_debug2(ZONE, LOGT_IO, "trying authentication: %s",
                                   xmlnode_serialize_string(
                                       auth, xmppd::ns_decl_list(), 0));
                ...
                /* new connection established, we can now send the outstanding
                 * db:result - or we could, if we did not want */
                log_debug2(ZONE, LOGT_IO,
                           "XMPP-stream: we could now send <db:result/>s");
                if (c->db_state == want_request) {
                    ...
                    log_debug2(
                        ZONE, LOGT_IO,
                        "... and we wanted ... and we did sent a <db:result/>");
                } else if (c->db_state == not_requested) {
                    c->db_state = could_request;
            ...
            if (j_strcmp(xmlnode_get_localname(x), "proceed") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_XMPP_TLS) == 0) {
                /* start tls on our side */
                if (mio_xml_starttls(m, 1, c->key->get_resource().c_str())) {
                    /* starting tls failed */
                    log_warn(
                        c->d->i->id,
                        "Starting TLS on an outgoing s2s to %s failed on our "
                        "side (%s).",
                        c->key->get_domain().c_str(),
                        c->key->get_resource().c_str());
                    mio_close(m);
            ...
            if (j_strcmp(xmlnode_get_localname(x), "failure") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_XMPP_TLS) == 0) {
                log_warn(c->d->i->id,
                         "Starting TLS on an outgoing s2s to %s failed on the "
                         "other side.",
                         c->key->get_domain().c_str());
                mio_close(m);
                break;
            ...
            if (j_strcmp(xmlnode_get_localname(x), "success") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_XMPP_SASL) == 0) {
                log_debug2(
                    ZONE, LOGT_IO, "SASL success response: %s",
                    xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));
            ...
            if (j_strcmp(xmlnode_get_localname(x), "failure") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_XMPP_SASL) == 0) {
                log_debug2(
                    ZONE, LOGT_IO, "SASL failure response: %s",
                    xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));
                ...
                if (c->connect_results != NULL) {
                    *c->connect_results
                        << " (SASL EXTERNAL auth failed: "
                        << xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0)
                        << ")";
                }
                log_alert(
                    c->d->i->id,
                    "SASL EXTERNAL authentication failed on authenticating "
                    "ourselfs to %s (sending name: %s)",
                    c->key->get_domain().c_str(),
                    c->key->get_resource().c_str());
                /* close the stream (in former times we sent a stream error, but
                 * I think we shouldn't. There is stream fault by the other
                 * entity!)
                 */
            ...
            if (j_strcmp(xmlnode_get_localname(x), "result") == 0 &&
                j_strcmp(xmlnode_get_namespace(x), NS_DIALBACK) == 0) {
                if (j_strcmp(xmlnode_get_attrib_ns(x, "from", NULL),
                             c->key->get_domain().c_str()) != 0 ||
                    j_strcmp(xmlnode_get_attrib_ns(x, "to", NULL),
                             c->key->get_resource().c_str()) != 0) {
                    /* naughty... *click* */
                    log_warn(
                        c->d->i->id,
                        "Received illegal dialback validation remote %s != %s "
                        "or to %s != %s",
                        c->key->get_domain().c_str(),
                        xmlnode_get_attrib_ns(x, "from", NULL),
                        c->key->get_resource().c_str(),
                        xmlnode_get_attrib_ns(x, "to", NULL));
                    mio_write(
                        m, NULL,
                        "<stream:error><not-authorized "
                        "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                        "xmlns='urn:ietf:params:xml:ns:xmpp-streams' "
                        "xml:lang='en'>Invalid Dialback "
                        "Result</text></stream:error>",
                        -1);
                    mio_close(m);
                    break;
                }
                ...
                if (j_strcmp(xmlnode_get_attrib_ns(x, "type", NULL), "valid") ==
                    0) {
                ...
                if (c->connect_results != NULL) {
                    char const *type_attribute =
                        xmlnode_get_attrib_ns(x, "type", NULL);
                    *c->connect_results
                        << " (dialback result: "
                ...
                log_alert(
                    c->d->i->id,
                    "We were told by %s that our sending name %s is invalid, "
                    "either something went wrong on their end, we tried using "
                    "that "
            ...
            log_warn(c->d->i->id,
                     "Dropping connection due to illegal incoming packet on an "
                     "unverified socket from %s to %s (%s): %s",
                     c->key->get_resource().c_str(),
                     c->key->get_domain().c_str(), mio_ip(m),
                     xmlnode_serialize_string(x, xmppd::ns_decl_list(), 0));
            mio_write(
                m, NULL,
                "<stream:error><not-authorized "
                "xmlns='urn:ietf:params:xml:ns:xmpp-streams'/><text "
                "xmlns='urn:ietf:params:xml:ns:xmpp-streams' xml:lang='en'>Not "
                "Allowed to send data on this socket!</text></stream:error>",
                -1);
            mio_close(m);
            break;
        ...
        case MIO_CLOSED:
            /* add the connect error message to the list of messages for the
             * tried hosts */
            if (c->connect_results != NULL) {
...
/**
 * callback for walking the connecting hash tree: timing out connections that
 * did not get authorized in time (default is 30 seconds, can be configured with
 * &lt;queuetimeout/&gt; in the configuration file)
 *
...
void _dialback_out_beat_packets(xht h, const char *key, void *data, void *arg) {
    ...
    while (cur != NULL) {
        ...
        if (bounce_reason == NULL) {
            ...
            errmsg << messages_get(lang, dialback_out_connection_state_string(
                                             c->connection_state));
        ...
        deliver_fail(dpacket_new(cur->x),
                     bounce_reason
                         ? bounce_reason
                         : messages_get(lang, N_("Server Connect Timeout")));
        cur = next;
    }
}

/**
 * start walking the connection hash tree, to see if connections dig not get
 * authorizsed in time
 *
 * @param arg the dialback instance
