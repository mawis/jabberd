...
resolver::resolver(instance i, xmlnode x)
    : instance_base(i, x), lwresd_socket(NULL), queue_timeout(60),
      lwresd_host("localhost"), lwresd_service("921") {
    configurate();
...
void resolver::open_lwresd_socket() {
    int udp_socket =
        make_netsocket2(lwresd_service, lwresd_host, NETSOCKET_UDP);

    lwresd_socket = mio_new(udp_socket, mio_callback, this, MIO_CONNECT_RAW);
}

void resolver::mio_callback(mio m, int state, void *arg, xmlnode x,
    ...
    static_cast<resolver *>(arg)->mio_event(
        m, state, buffer ? std::string(buffer, bufsz) : std::string());
}
...
void resolver::mio_event_buffer(mio m, std::string const &buffer) {
    ...
    xmppd::xhash<std::shared_ptr<resolver_job>> pending_jobs_lock =
        pending_jobs;
    ...
    std::map<uint32_t,
             std::pair<time_t,
                       sigc::signal<void, xmppd::lwresc::lwresult const &>>>::
        iterator result_listener = result_listeners.find(serial);
...
void resolver::configurate() {
    ...
    xmlnode_vector resend_elements =
        xmlnode_get_tags(config, "dnsrv:resend", namespaces);
    for (xmlnode_vector::iterator p = resend_elements.begin();
    ...
    char const *queuetimeout_attrib =
        xmlnode_get_attrib_ns(config, "queuetimeout", NULL);
...
void resolver::send_query(xmppd::lwresc::lwquery const &query) {
    ...
    // send it
    mio_write(lwresd_socket, NULL, query_bin.str().c_str(),
              query_bin.str().length());
}

sigc::connection resolver::register_result_callback(
    uint32_t serial,
    sigc::signal<void, xmppd::lwresc::lwresult const &>::slot_type const
        &callback) {
    if (result_listeners.find(serial) == result_listeners.end()) {
        result_listeners[serial] =
            std::pair<time_t,
                      sigc::signal<void, xmppd::lwresc::lwresult const &>>(
                std::time(NULL),
                sigc::signal<void, xmppd::lwresc::lwresult const &>());
...
result resolver::on_stanza_packet(dpacket dp) {
    ...
    // check if the packet already has been resolved (in the case of looping
    // packets)
    if (xmlnode_get_attrib_ns(dp->x, "ip", NULL) ||
        xmlnode_get_attrib_ns(dp->x, "iperror", NULL)) {
        ...
        if (packet_type && Glib::ustring(packet_type) == "error") {
            log(xmppd::warn)
                << "Looping DNS request. Dropped: "
                << xmlnode_serialize_string(dp->x, xmppd::ns_decl_list(), 0);
            xmlnode_free(dp->x);
        } else {
    ...
    // is there already a resolve request pending for this domain? Just add to
    // queue for this resolving
    if (pending_jobs.find(dp->host) != pending_jobs.end()) {
        pending_jobs[dp->host]->add_packet(dp);
        return r_DONE;
    }

    // store the packet, so that we can forward it when it has been resolved,
    // and start resolving
    pending_jobs[dp->host] =
        std::shared_ptr<resolver_job>(new resolver_job(*this, dp));
    log(xmppd::notice) << "Created new resolver job: "
                       << *(pending_jobs[dp->host]);
    pending_jobs[dp->host]->register_result_callback(
        sigc::mem_fun(*this, &xmppd::resolver::resolver::handle_completed_job));
    return r_DONE;
}

result resolver::on_route_packet(dpacket dp) {
    // only packets addressed to us directly (no default route) are accepted as
    // routed packets
    ...
    jid to = jid_new(dp->p, xmlnode_get_attrib_ns(xmlnode_get_firstchild(dp->x),
                                                  "to", NULL));
    if (to == NULL) {
        return r_ERR;
    }
...
void resolver::resend_packet(xmlnode pkt, Glib::ustring ips, Glib::ustring to) {
    if (ips.empty()) {
        ...
        char const *dnsresultto =
            xmlnode_get_attrib_ns(pkt, "dnsqueryby", NULL);
        if (!dnsresultto) {
        ...
        // XXX
        if (std::string("verify") == xmlnode_get_localname(pkt) &&
            std::string(NS_DIALBACK) == xmlnode_get_namespace(pkt)) {
            log(xmppd::notice)
                << "DB resend: "
                << xmlnode_serialize_string(pkt, xmppd::ns_decl_list(), 0);
        }
    }
    deliver(pkt);
}
...
void resolver::handle_completed_job(resolver_job &job) {
    ...
    log(xmppd::notice) << (ips.empty()
                               ? "Finished resolver job without result: "
                               : "Finished resolver job: ")
                       << job;

    // resend the packets
    for (std::list<dpacket>::const_iterator p = packets.begin();
         p != packets.end(); ++p) {
