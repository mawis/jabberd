...
dnl   JADC2S_LIB_OPENSSL(major, minor, fix, patch)
dnl
...
dnl   appropriate, and set the shell variable `jadc2s_lib_openssl' to
dnl   `yes'.  Otherwise, set `jadc2s_lib_openssl' to `no'.
dnl
...
[
  ...
  JADC2S_LIB_OPENSSL_save_libs="$LIBS"
  JADC2S_LIB_OPENSSL_save_cppflags="$CPPFLAGS"
  ...
  for place in $places; do
    ...
    LIBS="$JADC2S_LIB_OPENSSL_save_libs"
    CPPFLAGS="$JADC2S_LIB_OPENSSL_save_cppflags"
    case "$place" in
      "std" )
    ...
    # switches.
    changequote(,)
    cache_id="`echo jadc2s_cv_lib_openssl_$1_$2_$3_$4_in_${place} \
    ...
    AC_CACHE_VAL($cache_id,
      [
       JADC2S_LIB_OPENSSL_TRY($1, $2, $3, $4)
         eval "$cache_id=$jadc2s_have_openssl"
      ])
  ...
  case "$found" in
	...
	jadc2s_lib_openssl=no
    ;;
    "std" )
        JADC2S_SSL_INCLUDES=
        JADC2S_SSL_LIBS="-lssl -lcrypto"
        jadc2s_lib_openssl=yes
    ;;
    *":"* )
	header="`echo $found | sed -e 's/:.*$//'`"
	lib="`echo $found | sed -e 's/^.*://'`"
      JADC2S_SSL_INCLUDES="-I$header"
      JADC2S_SSL_LIBS="-L$lib -R$lib -lssl -lcrypto"
      jadc2s_lib_openssl=yes
    ;;
    * )
      JADC2S_SSL_INCLUDES="-I$found/include"
      JADC2S_SSL_LIBS="-L$found/lib -R$found/lib -lssl -lcrypto"
	jadc2s_lib_openssl=yes
    ;;
  esac
])
...
dnl   JADC2S_LIB_OPENSSL_TRY(major, minor, fix, patch)
dnl
dnl   A subroutine of JADC2S_LIB_OPENSSL.
dnl
...
dnl
dnl   Set the shell variable `jadc2s_have_openssl' to `yes' if we found
...
AC_DEFUN(JADC2S_LIB_OPENSSL_TRY,
  [
    jadc2s_lib_openssl_try_save_libs="$LIBS"

    jadc2s_check_openssl_major=$1
    jadc2s_check_openssl_minor=$2
    jadc2s_check_openssl_fix=$3
    jadc2s_check_openssl_patch=$4
...
main ()
{
  ...
  if(major < $jadc2s_check_openssl_major)
    exit(1);
  if(major > $jadc2s_check_openssl_major)
    exit(0);

  if(minor < $jadc2s_check_openssl_minor)
    exit(1);
  if(minor > $jadc2s_check_openssl_minor)
    exit(0);

  if(fix > $jadc2s_check_openssl_fix)
    exit(0);

  if(patch >= $jadc2s_check_openssl_patch)
    exit(0);

  exit(1);
}
      ],
      [jadc2s_have_openssl=yes],
      [jadc2s_have_openssl=no],
      [jadc2s_have_openssl=yes]
    )

  LIBS="$jadc2s_lib_openssl_try_save_libs"
  ]
)
