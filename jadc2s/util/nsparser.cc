...
namespace xmppd {
	...
	for (p2 = ns_attributes.begin(); p2 != ns_attributes.end(); ++p2) {
		...
		throw xmlpp::parse_error(std::string("Found attribute using undefined namespace prefix ")+p2->ns_prefix);
	    }
	...
	if (current_ns_mappings.find(ns_prefix) == current_ns_mappings.end()) {
	    throw xmlpp::parse_error(std::string("Found start element using undefined namespace prefix ")+ns_prefix);
	}
